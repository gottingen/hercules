...
TEST_F(HIRCoreTest, AssignInstrQueryAndReplace) {
...
TEST_F(HIRCoreTest, AssignInstrCloning) {
    auto *var = module->Nr<Var>(module->getIntType());
    auto *val = module->Nr<IntConst>(1, module->getIntType());
    auto *instr = module->Nr<AssignInstr>(var, val);
...
TEST_F(HIRCoreTest, ExtractInstrQueryAndReplace) {
    auto FIELD = "foo";
...
TEST_F(HIRCoreTest, ExtractInstrCloning) {
    auto FIELD = "foo";
    auto *type = cast<types::RecordType>(module->unsafeGetMemberedType("**internal**"));
    type->realize({module->getIntType()}, {FIELD});
    auto *var = module->Nr<Var>(type);
...
TEST_F(HIRCoreTest, InsertInstrQueryAndReplace) {
    auto FIELD = "foo";
    auto *type =
...
TEST_F(HIRCoreTest, InsertInstrCloning) {
    auto FIELD = "foo";
    auto *type =
            cast<types::RefType>(module->unsafeGetMemberedType("**internal**", true));
    type->realize({module->getIntType()}, {FIELD});
...
TEST_F(HIRCoreTest, CallInstrQueryAndReplace) {
...
TEST_F(HIRCoreTest, CallInstrCloning) {
...
TEST_F(HIRCoreTest, StackAllocInstrQueryAndReplace) {
    auto COUNT = 1;
...
TEST_F(HIRCoreTest, StackAllocInstrCloning) {
    auto COUNT = 1;
    auto *arrayType = module->unsafeGetArrayType(module->getIntType());
    auto *instr = module->Nr<StackAllocInstr>(arrayType, COUNT);
    ASSERT_TRUE(util::match(instr, cv->clone(instr)));
...
TEST_F(HIRCoreTest, TypePropertyInstrQueryAndReplace) {
...
TEST_F(HIRCoreTest, TypePropertyInstrCloning) {
    auto *type = module->unsafeGetArrayType(module->getIntType());
    auto *instr =
            module->Nr<TypePropertyInstr>(type, TypePropertyInstr::Property::IS_ATOMIC);
    ASSERT_TRUE(util::match(instr, cv->clone(instr)));
...
TEST_F(HIRCoreTest, YieldInInstrQueryAndReplace) {
    auto *type = module->unsafeGetArrayType(module->getIntType());
...
TEST_F(HIRCoreTest, YieldInInstrCloning) {
    auto *type = module->unsafeGetArrayType(module->getIntType());
    auto *instr = module->Nr<YieldInInstr>(type);
    ASSERT_TRUE(util::match(instr, cv->clone(instr)));
}

TEST_F(HIRCoreTest, TernaryInstrQueryAndReplace) {
...
TEST_F(HIRCoreTest, TernaryInstrCloning) {
    auto *trueValue = module->Nr<BoolConst>(true, module->getBoolType());
    auto *falseValue = module->Nr<BoolConst>(false, module->getBoolType());
    auto *cond = module->Nr<BoolConst>(true, module->getBoolType());
    auto *instr = module->Nr<TernaryInstr>(cond, trueValue, falseValue);
...
TEST_F(HIRCoreTest, ContinueInstrQueryReplaceAndCloning) {
    auto *instr = module->Nr<ContinueInstr>();
    ASSERT_TRUE(util::match(instr, cv->clone(instr)));
}

TEST_F(HIRCoreTest, BreakInstrQueryReplaceAndCloning) {
    auto *instr = module->Nr<BreakInstr>();
    ASSERT_TRUE(util::match(instr, cv->clone(instr)));
}

TEST_F(HIRCoreTest, ReturnInstrQueryReplaceAndCloning) {
    auto *val = module->Nr<IntConst>(1, module->getIntType());
...
TEST_F(HIRCoreTest, YieldInstrQueryReplaceAndCloning) {
    auto *val = module->Nr<IntConst>(1, module->getIntType());
...
TEST_F(HIRCoreTest, ThrowInstrQueryReplaceAndCloning) {
    auto *val = module->Nr<IntConst>(1, module->getIntType());
...
TEST_F(HIRCoreTest, FlowInstrQueryAndReplace) {
...
TEST_F(HIRCoreTest, FlowInstrCloning) {
    auto *flow = module->Nr<SeriesFlow>();
    auto *val = module->Nr<IntConst>(1, module->getIntType());
    auto *instr = module->Nr<FlowInstr>(flow, val);
    ASSERT_TRUE(util::match(instr, cv->clone(instr)));
