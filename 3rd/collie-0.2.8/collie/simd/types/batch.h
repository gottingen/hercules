...
#ifndef COLLIE_SIMD_TYPES_BATCH_H_
#define COLLIE_SIMD_TYPES_BATCH_H_
...
namespace collie::simd {
    ...
    class batch : public types::simd_register<T, A>, public types::integral_only_operators<T, A> {
        static_assert(!std::is_same<T, bool>::value, "use collie::simd::batch_bool<T, A> instead of collie::simd::batch<bool, A>");
    ...
    public:
        ...
        template<class U>
        [[nodiscard]] static inline batch broadcast(U val) noexcept;
        ...
        template<class U>
        [[nodiscard]] static inline batch load_aligned(U const *mem) noexcept;

        template<class U>
        [[nodiscard]] static inline batch load_unaligned(U const *mem) noexcept;

        template<class U>
        [[nodiscard]] static inline batch load(U const *mem, aligned_mode) noexcept;

        template<class U>
        [[nodiscard]] static inline batch load(U const *mem, unaligned_mode) noexcept;

        template<class U, class V>
        [[nodiscard]] static inline batch gather(U const *src, batch<V, arch_type> const &index) noexcept;
        ...
        /** Shorthand for collie::simd::add() */
        friend inline batch operator+(batch const &self, batch const &other) noexcept {
            return batch(self) += other;
        }

        /** Shorthand for collie::simd::sub() */
        friend inline batch operator-(batch const &self, batch const &other) noexcept {
            return batch(self) -= other;
        }

        /** Shorthand for collie::simd::mul() */
        friend inline batch operator*(batch const &self, batch const &other) noexcept {
            return batch(self) *= other;
        }

        /** Shorthand for collie::simd::div() */
        friend inline batch operator/(batch const &self, batch const &other) noexcept {
            return batch(self) /= other;
        }

        /** Shorthand for collie::simd::bitwise_and() */
        friend inline batch operator&(batch const &self, batch const &other) noexcept {
            return batch(self) &= other;
        }

        /** Shorthand for collie::simd::bitwise_or() */
        friend inline batch operator|(batch const &self, batch const &other) noexcept {
            return batch(self) |= other;
        }

        /** Shorthand for collie::simd::bitwise_xor() */
        friend inline batch operator^(batch const &self, batch const &other) noexcept {
            return batch(self) ^= other;
        }

        /** Shorthand for collie::simd::logical_and() */
        friend inline batch operator&&(batch const &self, batch const &other) noexcept {
            return batch(self).logical_and(other);
        }

        /** Shorthand for collie::simd::logical_or() */
        friend inline batch operator||(batch const &self, batch const &other) noexcept {
    ...
    public:
        ...
        [[nodiscard]] static inline batch_bool load_aligned(bool const *mem) noexcept;

        [[nodiscard]] static inline batch_bool load_unaligned(bool const *mem) noexcept;
    ...
    public:
        ...
        // memory operators
        [[nodiscard]] static inline batch load_aligned(const T *real_src, const T *imag_src = nullptr) noexcept;

        [[nodiscard]] static inline batch load_unaligned(const T *real_src, const T *imag_src = nullptr) noexcept;
        ...
        [[nodiscard]] static inline batch load_aligned(const value_type *src) noexcept;

        [[nodiscard]] static inline batch load_unaligned(const value_type *src) noexcept;
...
#include <collie/simd/arch/isa.h>
...
namespace collie::simd {
    ...
    inline batch<T, A> &types::integral_only_operators<T, A>::operator%=(batch<T, A> const &other) noexcept {
        ::collie::simd::detail::static_check_supported_config<T, A>();
    ...
    namespace details {
        ...
        template<typename T, class Arch, bool BatchExists = collie::simd::types::has_simd_register<T, Arch>::value>
        struct batch_trait;

        template<typename T, class Arch>
        struct batch_trait<T, Arch, true> {
            using type = collie::simd::batch<T, Arch>;
            static constexpr std::size_t size = collie::simd::batch<T, Arch>::size;
        };
...
#endif  // COLLIE_SIMD_TYPES_BATCH_H_
