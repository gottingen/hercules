...
#ifndef COLLIE_SIMD_ARCH_AVX512BW_H_
#define COLLIE_SIMD_ARCH_AVX512BW_H_
...
namespace collie::simd {
    ...
    namespace kernel {
...
#if defined(COLLIE_SIMD_AVX512_SHIFT_INTRINSICS_IMM_ONLY)
        ...
        eq(batch<T, A> const &self, batch<T, A> const &other, requires_arch<avx512bw>) noexcept {
        ...
        template<class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
        inline batch_bool<T, A>
        ge(batch<T, A> const &self, batch<T, A> const &other, requires_arch<avx512bw>) noexcept {
        ...
        template<class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
        inline batch_bool<T, A>
        gt(batch<T, A> const &self, batch<T, A> const &other, requires_arch<avx512bw>) noexcept {
        ...
        template<class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
        inline batch_bool<T, A>
        le(batch<T, A> const &self, batch<T, A> const &other, requires_arch<avx512bw>) noexcept {
        ...
        template<class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
        inline batch_bool<T, A>
        lt(batch<T, A> const &self, batch<T, A> const &other, requires_arch<avx512bw>) noexcept {
        ...
        template<class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
        inline batch_bool<T, A>
        neq(batch<T, A> const &self, batch<T, A> const &other, requires_arch<avx512bw>) noexcept {
        ...
        namespace detail {
            template<size_t... Is>
            constexpr std::array<uint64_t, sizeof...(Is)> make_slide_perm_hi(::collie::simd::detail::index_sequence<Is...>) {
                return {(Is == 0 ? 8 : Is - 1)...};
            }

            template<size_t N, size_t... Is>
            constexpr std::array<uint16_t, sizeof...(Is)>
            make_slide_left_pattern(::collie::simd::detail::index_sequence<Is...>) {
                return {(Is >= N ? Is - N : 0)...};
            }

            template<size_t N, size_t... Is>
            constexpr std::array<uint16_t, sizeof...(Is)> make_slide_left_mask(::collie::simd::detail::index_sequence<Is...>) {
        ...
        inline batch<T, A> slide_left(batch<T, A> const &x, requires_arch<avx512bw>) noexcept {
            ...
            if (N & 1) {
                ...
                alignas(A::alignment()) auto slide_perm = detail::make_slide_perm_hi(
                        ::collie::simd::detail::make_index_sequence<512 / 64>());
            ...
            alignas(A::alignment()) auto slide_pattern = detail::make_slide_left_pattern<N / 2>(
                    ::collie::simd::detail::make_index_sequence<512 / 16>());
            alignas(A::alignment()) auto slide_mask = detail::make_slide_left_mask<N / 2>(
                    ::collie::simd::detail::make_index_sequence<512 / 16>());
            return _mm512_and_si512(_mm512_permutexvar_epi16(_mm512_load_epi32(slide_pattern.data()), xx),
                                    _mm512_load_epi32(slide_mask.data()));
        }

        // slide_right
        namespace detail {
            template<size_t... Is>
            constexpr std::array<uint64_t, sizeof...(Is)> make_slide_perm_low(::collie::simd::detail::index_sequence<Is...>) {
                return {(Is + 1)...};
            }

            template<size_t N, size_t... Is>
            constexpr std::array<uint16_t, sizeof...(Is)>
            make_slide_right_pattern(::collie::simd::detail::index_sequence<Is...>) {
                return {(Is < (32 - N) ? Is + N : 0)...};
            }

            template<size_t N, size_t... Is>
            constexpr std::array<uint16_t, sizeof...(Is)>
            make_slide_right_mask(::collie::simd::detail::index_sequence<Is...>) {
        ...
        inline batch<T, A> slide_right(batch<T, A> const &x, requires_arch<avx512bw>) noexcept {
            ...
            batch<T, A> xx;
            if (N & 1) {
                alignas(A::alignment()) auto slide_perm = detail::make_slide_perm_low(
                        ::collie::simd::detail::make_index_sequence<512 / 64>());
            ...
            alignas(A::alignment()) auto slide_pattern = detail::make_slide_right_pattern<N / 2>(
                    ::collie::simd::detail::make_index_sequence<512 / 16>());
            alignas(A::alignment()) auto slide_mask = detail::make_slide_right_mask<N / 2>(
                    ::collie::simd::detail::make_index_sequence<512 / 16>());
            return _mm512_and_si512(_mm512_permutexvar_epi16(_mm512_load_epi32(slide_pattern.data()), xx),
                                    _mm512_load_epi32(slide_mask.data()));
        }
        ...
        swizzle(batch<uint16_t, A> const &self, batch<uint16_t, A> mask, requires_arch<avx512bw>) noexcept {
            return _mm512_permutexvar_epi16(mask, self);
        }

        template<class A>
        inline batch<int16_t, A>
        swizzle(batch<int16_t, A> const &self, batch<uint16_t, A> mask, requires_arch<avx512bw>) noexcept {
            return bitwise_cast<int16_t>(swizzle(bitwise_cast<uint16_t>(self), mask, avx512bw{}));
        }

        template<class A>
        inline batch<uint8_t, A>
        swizzle(batch<uint8_t, A> const &self, batch<uint8_t, A> mask, requires_arch<avx512bw>) noexcept {
            return _mm512_shuffle_epi8(self, mask);
        }

        template<class A>
        inline batch<int8_t, A>
        swizzle(batch<int8_t, A> const &self, batch<uint8_t, A> mask, requires_arch<avx512bw>) noexcept {
        ...
        inline batch<uint16_t, A>
        swizzle(batch<uint16_t, A> const &self, batch_constant<batch<uint16_t, A>, Vs...> mask,
                requires_arch<avx512bw>) noexcept {
            return swizzle(self, (batch<uint16_t, A>) mask, avx512bw{});
        }

        template<class A, uint16_t... Vs>
        inline batch<int16_t, A> swizzle(batch<int16_t, A> const &self, batch_constant<batch<uint16_t, A>, Vs...> mask,
                                         requires_arch<avx512bw>) noexcept {
            return swizzle(self, (batch<uint16_t, A>) mask, avx512bw{});
        }

        template<class A, uint8_t... Vs>
        inline batch<uint8_t, A> swizzle(batch<uint8_t, A> const &self, batch_constant<batch<uint8_t, A>, Vs...> mask,
                                         requires_arch<avx512bw>) noexcept {
            return swizzle(self, (batch<uint8_t, A>) mask, avx512bw{});
        }

        template<class A, uint8_t... Vs>
        inline batch<int8_t, A> swizzle(batch<int8_t, A> const &self, batch_constant<batch<uint8_t, A>, Vs...> mask,
                                        requires_arch<avx512bw>) noexcept {
            return swizzle(self, (batch<uint8_t, A>) mask, avx512bw{});
        }
...
#endif  // COLLIE_SIMD_ARCH_AVX512BW_H_
