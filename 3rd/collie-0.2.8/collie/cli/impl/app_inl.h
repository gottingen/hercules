...
namespace collie {
    ...
    inline App::App(std::string app_description, std::string app_name, App *parent)
    ...
    [[nodiscard]] inline char **App::ensure_utf8(char **argv) {
#ifdef _WIN32
        (void)argv;
    ...
    inline App *App::name(std::string app_name) {
    ...
    inline App *App::alias(std::string app_name) {
    ...
    inline App *App::immediate_callback(bool immediate) {
    ...
    inline App *App::ignore_case(bool value) {
    ...
    inline App *App::ignore_underscore(bool value) {
    ...
    inline Option *App::add_option(std::string option_name,
    ...
    inline Option *App::set_help_flag(std::string flag_name, const std::string &help_description) {
    ...
    inline Option *App::set_help_all_flag(std::string help_name, const std::string &help_description) {
    ...
    App::set_version_flag(std::string flag_name, const std::string &versionString, const std::string &version_help) {
        ...
        if (!flag_name.empty()) {
            version_ptr_ = add_flag_callback(
                    flag_name, [versionString]() { throw (collie::CallForVersion(versionString, 0)); }, version_help);
    ...
    App::set_version_flag(std::string flag_name, std::function<std::string()> vfunc, const std::string &version_help) {
        ...
        if (!flag_name.empty()) {
            version_ptr_ = add_flag_callback(
                    flag_name, [vfunc]() { throw (collie::CallForVersion(vfunc(), 0)); }, version_help);
            version_ptr_->configurable(false);
        }
    ...
    inline Option *
    App::_add_flag_internal(std::string flag_name, collie::callback_t fun, std::string flag_description) {
        Option *opt = nullptr;
    ...
    inline Option *App::add_flag_callback(std::string flag_name,
                                          std::function<void(void)> function,  ///< A function to call, void(void)
                                          std::string flag_description) {

        collie::callback_t fun = [function](const collie::results_t &res) {
    ...
    inline Option *App::set_config(std::string option_name,
    ...
    inline bool App::remove_option(Option *opt) {
    ...
    inline App *App::add_subcommand(std::string subcommand_name, std::string subcommand_description) {
        ...
        collie::App_p subcom = std::shared_ptr<App>(new App(std::move(subcommand_description), subcommand_name, this));
        return add_subcommand(std::move(subcom));
    }

    inline App *App::add_subcommand(collie::App_p subcom) {
        if (!subcom)
    ...
    inline bool App::remove_subcommand(App *subcom) {
        // Make sure no links exist
    ...
    inline App *App::get_subcommand(const App *subcom) const {
        if (subcom == nullptr)
    ...
    [[nodiscard]] inline App *App::get_subcommand(std::string subcom) const {
    ...
    [[nodiscard]] inline App *App::get_subcommand(int index) const {
        if (index >= 0) {
    ...
    inline collie::App_p App::get_subcommand_ptr(App *subcom) const {
        if (subcom == nullptr)
            throw OptionNotFound("nullptr passed");
        for (const App_p &subcomptr: subcommands_)
            if (subcomptr.get() == subcom)
    ...
    [[nodiscard]] inline collie::App_p App::get_subcommand_ptr(std::string subcom) const {
        for (const App_p &subcomptr: subcommands_)
    ...
    [[nodiscard]] inline collie::App_p App::get_subcommand_ptr(int index) const {
        if (index >= 0) {
            auto uindex = static_cast<unsigned>(index);
            if (uindex < subcommands_.size())
    ...
    [[nodiscard]] inline collie::App *App::get_option_group(std::string group_name) const {
    ...
    [[nodiscard]] inline std::size_t App::count_all() const {
        std::size_t cnt{0};
    ...
    inline void App::parse(int argc, const char *const *argv) { parse_char_t(argc, argv); }

    inline void App::parse(int argc, const wchar_t *const *argv) { parse_char_t(argc, argv); }

    namespace detail {

// Do nothing or perform narrowing
        inline const char *maybe_narrow(const char *str) { return str; }

        inline std::string maybe_narrow(const wchar_t *str) { return narrow(str); }

    }  // namespace detail

    template<class CharT>
    inline void App::parse_char_t(int argc, const CharT *const *argv) {
    ...
    inline void App::parse(std::string commandline, bool program_name_included) {
        ...
        try {
            ...
            throw collie::ParseError(arg.what(), collie::ExitCodes::InvalidError);
        }
        std::reverse(args.begin(), args.end());
        parse(std::move(args));
    }

    inline void App::parse(std::wstring commandline, bool program_name_included) {
        parse(narrow(commandline), program_name_included);
    }

    inline void App::parse(std::vector<std::string> &args) {
        // Clear if parsed
        if (parsed_ > 0)
            clear();
    ...
    inline void App::parse_from_stream(std::istream &input) {
        if (parsed_ == 0) {
            _validate();
            _configure();
            // set the parent as nullptr as this object should be the top now
    ...
    inline int App::exit(const Error &e, std::ostream &out, std::ostream &err) const {

        /// Avoid printing anything if this is a collie::RuntimeError
    ...
    inline std::vector<const App *> App::get_subcommands(const std::function<bool(const App *)> &filter) const {
    ...
    inline std::vector<App *> App::get_subcommands(const std::function<bool(App *)> &filter) {
    ...
    inline bool App::remove_excludes(Option *opt) {
    ...
    inline bool App::remove_excludes(App *app) {
    ...
    inline bool App::remove_needs(Option *opt) {
    ...
    inline bool App::remove_needs(App *app) {
    ...
    [[nodiscard]] inline std::string App::help(std::string prev, AppFormatMode mode) const {
        if (prev.empty())
            prev = get_name();
        else
            prev += " " + get_name();
    ...
    [[nodiscard]] inline std::string App::version() const {
        std::string val;
        if (version_ptr_ != nullptr) {
            ...
            } catch (const collie::CallForVersion &cfv) {
                val = cfv.what();
            }
            version_ptr_->clear();
    ...
    inline std::vector<const Option *> App::get_options(const std::function<bool(const Option *)> filter) const {
    ...
    inline std::vector<Option *> App::get_options(const std::function<bool(Option *)> filter) {
    ...
    inline Option *App::get_option_no_throw(std::string option_name) noexcept {
    ...
    [[nodiscard]] inline const Option *App::get_option_no_throw(std::string option_name) const noexcept {
        for (const Option_p &opt: options_) {
            if (opt->check_name(option_name)) {
                return opt.get();
            }
    ...
    [[nodiscard]] inline std::string App::get_display_name(bool with_aliases) const {
        if (name_.empty()) {
    ...
    [[nodiscard]] inline bool App::check_name(std::string name_to_check) const {
    ...
    [[nodiscard]] inline std::vector<std::string> App::get_groups() const {
    ...
    [[nodiscard]] inline std::vector<std::string> App::remaining(bool recurse) const {
    ...
    [[nodiscard]] inline std::vector<std::string> App::remaining_for_passthrough(bool recurse) const {
        std::vector<std::string> miss_list = remaining(recurse);
        std::reverse(std::begin(miss_list), std::end(miss_list));
        return miss_list;
    }

    [[nodiscard]] inline std::size_t App::remaining_size(bool recurse) const {
    ...
    inline void App::_validate() const {
    ...
    inline void App::run_callback(bool final_mode, bool suppress_final_callback) {
        pre_callback();
    ...
    [[nodiscard]] inline bool App::_valid_subcommand(const std::string &current, bool ignore_used) const {
    ...
    [[nodiscard]] inline detail::Classifier App::_recognize(const std::string &current,
    ...
    inline bool App::_process_config_file(const std::string &config_file, bool throw_error) {
    ...
    inline void App::_process_config_file() {
        if (config_ptr_ != nullptr) {
    ...
    inline void App::_process_callbacks() {
    ...
    inline void App::_process_help_flags(bool trigger_help, bool trigger_all_help) const {
    ...
    inline void App::_process_requirements() {
        // check excludes
        bool excluded{false};
        std::string excluder;
        ...
        for (App_p &sub: subcommands_) {
            ...
            if (sub->required_ && sub->count_all() == 0) {
                throw (collie::RequiredError(sub->get_display_name()));
            }
        }
    }

    inline void App::_process() {
        try {
    ...
    inline void App::_process_extras() {
    ...
    inline void App::_process_extras(std::vector<std::string> &args) {
        if (!(allow_extras_ || prefix_command_)) {
            std::size_t num_left_over = remaining_size();
            if (num_left_over > 0) {
    ...
    inline void App::increment_parsed() {
        ++parsed_;
        for (App_p &sub: subcommands_) {
    ...
    inline void App::_parse(std::vector<std::string> &args) {
        increment_parsed();
    ...
    inline void App::_parse_stream(std::istream &input) {
    ...
    inline void App::_parse_config(const std::vector<ConfigItem> &args) {
    ...
    inline bool App::_parse_single_config(const ConfigItem &item, std::size_t level) {
    ...
    inline bool App::_parse_single(std::vector<std::string> &args, bool &positional_only) {
        bool retval = true;
    ...
    [[nodiscard]] inline std::size_t App::_count_remaining_positionals(bool required_only) const {
        std::size_t retval = 0;
        for (const Option_p &opt: options_) {
    ...
    [[nodiscard]] inline bool App::_has_remaining_positionals() const {
        for (const Option_p &opt: options_) {
    ...
    inline bool App::_parse_positional(std::vector<std::string> &args, bool haltOnSubcommand) {
        ...
        if (positionals_at_end_) {
            throw collie::ExtrasError(name_, args);
        }
    ...
    inline bool App::_parse_subcommand(std::vector<std::string> &args) {
    ...
    inline void App::_trigger_pre_parse(std::size_t remaining_args) {
    ...
    inline App *App::_get_fallthrough_parent() {
        if (parent_ == nullptr) {
    ...
    [[nodiscard]] inline const std::string &App::_compare_subcommand_names(const App &subcom,
    ...
    inline void App::_move_to_missing(detail::Classifier val_type, const std::string &val) {
    ...
    inline void App::_move_option(Option *opt, App *app) {
        if (opt == nullptr) {
