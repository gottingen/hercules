...
#include <collie/taskflow/cuda/cuda_optimizer.h>
...
The usage of collie::tf::cudaFlowCapturer is similar to collie::tf::cudaFlow, except users can
call the method collie::tf::cudaFlowCapturer::on to capture a sequence of asynchronous
...
@code{.cpp}
taskflow.emplace([](collie::tf::cudaFlowCapturer& capturer){
...
Similar to collie::tf::cudaFlow, a %cudaFlowCapturer is a task (collie::tf::Task)
created from collie::tf::Taskflow
and will be run by @em one worker thread in the executor.
That is, the callable that describes a %cudaFlowCapturer
will be executed sequentially.
Inside a %cudaFlow capturer task, different GPU tasks (collie::tf::cudaTask) may run
in parallel depending on the selected optimization algorithm.
By default, we use collie::tf::cudaFlowRoundRobinOptimizer to transform a user-level
...
class cudaFlowCapturer {
  ...
  public:
    ...
    can be run by the caller thread using collie::tf::cudaFlowCapturer::run.
    */
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlowCapturer::noop but
    ...
    */
    ...
    a user-described %cudaFlow:
      + collie::tf::cudaFlowSequentialOptimizer
      + collie::tf::cudaFlowRoundRobinOptimizer
      + collie::tf::cudaFlowLinearOptimizer

    By default, collie::tf::cudaFlowCapturer uses the round-robin optimization
