...
#include <collie/taskflow/dsl/meta_macro.h>
#include <collie/taskflow/dsl/task_analyzer.h>
...
#define TF_CONTEXT_1(name) collie::tf::dsl::EmptyContext
...
#define make_task(name, ...)                                                    \
  struct TF_GET_FIRST name : collie::tf::dsl::TaskSignature,                           \
...
#define some_tasks(...) auto (*)(collie::tf::dsl::SomeTask<__VA_ARGS__>)
// same as some_tasks
...
#define build_taskflow(...) collie::tf::dsl::taskDsl<void TF_MAP(TF_CHAIN, __VA_ARGS__)>
