...
#include <collie/taskflow/core/worker.h>
...
The collie::tf::ObserverInterface class allows users to define custom methods to monitor
...
collie::tf::ObserverInterface provides immutable wrappers,
collie::tf::WorkerView and collie::tf::TaskView, over workers and tasks.

Please refer to collie::tf::WorkerView and collie::tf::TaskView for details.

Example usage:

@code{.cpp}

struct MyObserver : public collie::tf::ObserverInterface {
  ...
  void on_entry(WorkerView w, collie::tf::TaskView tv) override final {
    std::ostringstream oss;
    oss << "worker " << w.id() << " ready to run " << tv.name() << '\n';
    std::cout << oss.str();
  }

  void on_exit(WorkerView w, collie::tf::TaskView tv) override final {
    std::ostringstream oss;
...
A collie::tf::ChromeObserver inherits collie::tf::ObserverInterface and defines methods to dump
...
// create a custom observer
std::shared_ptr<collie::tf::ChromeObserver> observer = executor.make_observer<collie::tf::ChromeObserver>();
...
A collie::tf::TFProfObserver inherits collie::tf::ObserverInterface and defines methods to dump
the observed thread activities into a format that can be visualized through
@TFProf.
...
// create a custom observer
std::shared_ptr<collie::tf::TFProfObserver> observer = executor.make_observer<collie::tf::TFProfObserver>();
...
*/
    inline const char *to_string(ObserverType type) {
        switch (type) {
            case ObserverType::TFPROF:
                return "tfprof";
            case ObserverType::CHROME:
                return "chrome";
            default:
                return "undefined";
        }
