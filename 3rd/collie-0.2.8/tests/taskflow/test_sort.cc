...
TEST_CASE("BubbleSort" * doctest::timeout(300)) {
    ...
    for (unsigned w = 1; w <= 9; w += 2) {
        ...
        for (int end = 10; end <= 1000; end += 200) {
            ...
            collie::tf::Taskflow taskflow("BubbleSort");
            ...
            auto even_phase = taskflow.emplace([&](collie::tf::Subflow &sf) {
            ...
            auto odd_phase = taskflow.emplace([&](collie::tf::Subflow &sf) {
...
TEST_CASE("SelectionSort" * doctest::timeout(300)) {
    ...
    std::function<
            void(collie::tf::Subflow &sf, std::vector<int> &, int, int, int &)
    > spawn;
    ...
    spawn = [&](
        ...
        auto SL = sf.emplace(
                [&spawn, &data, beg, m, l = minl](collie::tf::Subflow &sf2) mutable {
        ...
        auto SR = sf.emplace(
                [&spawn, &data, m, end, r = minr](collie::tf::Subflow &sf2) mutable {
    ...
    for (unsigned w = 1; w <= 9; w += 2) {
        ...
        for (int end = 16; end <= 512; end <<= 1) {
            collie::tf::Taskflow taskflow("SelectionSort");
            ...
            auto argmin = taskflow.emplace(
                    [&spawn, &data, &beg, end, &min](collie::tf::Subflow &sf) mutable {
...
TEST_CASE("MergeSort" * doctest::timeout(300)) {
    ...
    std::function<void(collie::tf::Subflow &sf, std::vector<int> &, int, int)> spawn;

    spawn = [&](collie::tf::Subflow &sf, std::vector<int> &data, int beg, int end) mutable {
        ...
        auto SL = sf.emplace([&spawn, &data, beg, m](collie::tf::Subflow &sf2) {
        ...
        auto SR = sf.emplace([&spawn, &data, m, end](collie::tf::Subflow &sf2) {
    ...
    for (unsigned w = 1; w <= 9; w += 2) {
        ...
        for (int end = 10; end <= 10000; end = end * 10) {
            collie::tf::Taskflow taskflow("MergeSort");
            ...
            taskflow.emplace([&spawn, &data, end](collie::tf::Subflow &sf) {
                spawn(sf, data, 0, end);
            }).name(std::string("[0")
                    + ":"
                    + std::to_string(end) + ")");
...
TEST_CASE("QuickSort" * doctest::timeout(300)) {
    ...
    std::function<void(collie::tf::Subflow &sf, std::vector<int> &, itr_t, itr_t)> spawn;
    ...
    spawn = [&](
        ...
        sf.emplace([&spawn, &data, beg, pvt](collie::tf::Subflow &sf2) {
        ...
        sf.emplace([&spawn, &data, pvt, end](collie::tf::Subflow &sf2) {
    ...
    for (unsigned w = 1; w <= 9; w += 2) {
        ...
        for (int end = 16; end <= 16384; end <<= 1) {
            ...
            collie::tf::Taskflow taskflow("QuickSort");
            ...
            taskflow.emplace([&spawn, &data](collie::tf::Subflow &sf) {
