...
TEST_CASE("Runtime.Schedule.ModuleTask" * doctest::timeout(300)) {
  ...
  collie::tf::Taskflow module_flow;
  ...
  auto schedule = tf.emplace([&](collie::tf::Runtime& runtime) {
    value++;
...
TEST_CASE("Runtime.ExternalGraph.Simple" * doctest::timeout(300)) {
  ...
  std::vector<collie::tf::Taskflow> graphs(N);
  ...
  for(size_t i=0; i<N; i++) {
    ...
    taskflow.emplace([&res=results[i], &graph=graphs[i]](collie::tf::Runtime& rt)mutable{
      rt.corun(graph);
    });
  }
