...
int main(int argc, char **argv) {
    ...
    std::string file;
    collie::Option *opt = app.add_option("-f,--file,file", file, "File name")->required()->group("Important");

    int count{0};
    collie::Option *copt = app.add_flag("-c,--count", count, "Counter")->required()->group("Important");
