...
template <typename ConsoleMutex>
inline wincolor_sink<ConsoleMutex>::~wincolor_sink() {
    this->flush();
}

// change the color for the given level
template <typename ConsoleMutex>
void inline wincolor_sink<ConsoleMutex>::set_color(level::level_enum level,
                                                          std::uint16_t color) {
...
template <typename ConsoleMutex>
void inline wincolor_sink<ConsoleMutex>::log(const details::log_msg &msg) {
...
template <typename ConsoleMutex>
void inline wincolor_sink<ConsoleMutex>::flush() {
...
template <typename ConsoleMutex>
void inline wincolor_sink<ConsoleMutex>::set_color_mode(color_mode mode) {
    std::lock_guard<mutex_t> lock(mutex_);
    set_color_mode_impl(mode);
}

template <typename ConsoleMutex>
void inline wincolor_sink<ConsoleMutex>::set_color_mode_impl(color_mode mode) {
...
template <typename ConsoleMutex>
void inline wincolor_sink<ConsoleMutex>::print_range_(const memory_buf_t &formatted,
                                                             size_t start,
                                                             size_t end) {
    if (end > start) {
...
template <typename ConsoleMutex>
void inline wincolor_sink<ConsoleMutex>::write_to_file_(const memory_buf_t &formatted) {
...
template <typename ConsoleMutex>
inline wincolor_stdout_sink<ConsoleMutex>::wincolor_stdout_sink(color_mode mode)
