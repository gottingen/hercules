...
#ifndef COLLIE_BASE_BIT_H_
#define COLLIE_BASE_BIT_H_
...
/// Reverses the bytes in the given integer value V.
...
#else
            uint64_t Hi = collie::byteswap<uint32_t>(UV);
            uint32_t Lo = collie::byteswap<uint32_t>(UV >> 32);
            return (Hi << 32) | Lo;
#endif
        } else {
    ...
    template <typename T> [[nodiscard]] int countr_zero(T Val) {
        static_assert(std::is_unsigned_v<T>,
                      "Only unsigned integral types are allowed.");
        return collie::detail::TrailingZerosCounter<T, sizeof(T)>::count(Val);
    }
    ...
    template <typename T> [[nodiscard]] int countl_zero(T Val) {
        static_assert(std::is_unsigned_v<T>,
                      "Only unsigned integral types are allowed.");
        return collie::detail::LeadingZerosCounter<T, sizeof(T)>::count(Val);
    }
    ...
    template <typename T> [[nodiscard]] int countl_one(T Value) {
        static_assert(std::is_unsigned_v<T>,
                      "Only unsigned integral types are allowed.");
        return collie::countl_zero<T>(~Value);
    }
    ...
    template <typename T> [[nodiscard]] int countr_one(T Value) {
        static_assert(std::is_unsigned_v<T>,
                      "Only unsigned integral types are allowed.");
        return collie::countr_zero<T>(~Value);
    }
    ...
    template <typename T> [[nodiscard]] int bit_width(T Value) {
        static_assert(std::is_unsigned_v<T>,
                      "Only unsigned integral types are allowed.");
        return std::numeric_limits<T>::digits - collie::countl_zero(Value);
    }
    ...
    template <typename T> [[nodiscard]] T bit_floor(T Value) {
        ...
        if (!Value)
            return 0;
        return T(1) << (collie::bit_width(Value) - 1);
    }
...
/// of two representable in T.
    template <typename T> [[nodiscard]] T bit_ceil(T Value) {
        ...
        if (Value < 2)
            return 1;
        return T(1) << collie::bit_width<T>(Value - 1u);
    }
