...
namespace collie {
    ...
    namespace detail {
        ...
        inline bool is_printable(const std::string &test_string) {
        ...
        generate_parents(const std::string &section, std::string &name, char parentSeparator) {
            ...
            try {
                ...
                throw collie::ParseError(iarg.what(), collie::ExitCodes::InvalidError);
            }
            return parents;
        }
...
/// @brief  checks if a string represents a multiline comment
        inline bool hasMLString(std::string const &fullString, char check) {
    ...
    inline std::vector<ConfigItem> ConfigBase::from_config(std::istream &input) const {
        ...
        while (getline(input, buffer)) {
            ...
            if (delimiter_pos != std::string::npos) {
                ...
                bool mlquote =
                        (item.compare(0, 3, multiline_literal_quote) == 0 ||
            ...
            try {
                ...
                throw collie::ParseError(ia.what(), collie::ExitCodes::InvalidError);
            }
    ...
    inline std::string &clean_name_string(std::string &name, const std::string &keyChars) {
                ...
                if (opt->get_configurable()) {
                    ...
                    if (!value.empty()) {
                        ...
                        if (!opt->get_fnames().empty()) {
                            try {
                            ...
                            } catch (const collie::ArgumentMismatch &) {
                                bool valid{false};
                                ...
                                if (!valid) {
                                    value = detail::ini_join(
                                            opt->results(), arraySeparator, arrayStart, arrayEnd, stringQuote,
                                            literalQuote);
                                }
                            }
                        }
