...
inline Option *Option::expected(int value) {
    if(value < 0) {
        expected_min_ = -value;
...
inline Option *Option::expected(int value_min, int value_max) {
    if(value_min < 0) {
        value_min = -value_min;
    }
...
inline Option *Option::check(Validator validator, const std::string &validator_name) {
...
inline Option *Option::check(std::function<std::string(const std::string &)> Validator,
...
inline Option *Option::transform(Validator Validator, const std::string &Validator_name) {
...
inline Option *Option::transform(const std::function<std::string(std::string)> &func,
...
inline Option *Option::each(const std::function<void(std::string)> &func) {
...
inline Validator *Option::get_validator(const std::string &Validator_name) {
...
inline Validator *Option::get_validator(int index) {
...
inline bool Option::remove_needs(Option *opt) {
...
inline Option *Option::excludes(Option *opt) {
    if(opt == this) {
...
inline bool Option::remove_excludes(Option *opt) {
...
inline Option *Option::multi_option_policy(MultiOptionPolicy value) {
...
[[nodiscard]] inline std::string Option::get_name(bool positional, bool all_options) const {
    if(get_group().empty())
        return {};  // Hidden
...
inline void Option::run_callback() {
...
[[nodiscard]] inline const std::string &Option::matching_name(const Option &other) const {
...
[[nodiscard]] inline bool Option::check_name(const std::string &name) const {
...
[[nodiscard]] inline std::string Option::get_flag_value(const std::string &name,
...
inline Option *Option::add_result(std::string s) {
    _add_result(std::move(s), results_);
    current_option_state_ = option_state::parsing;
    return this;
}

inline Option *Option::add_result(std::string s, int &results_added) {
    results_added = _add_result(std::move(s), results_);
    current_option_state_ = option_state::parsing;
    return this;
}

inline Option *Option::add_result(std::vector<std::string> s) {
    current_option_state_ = option_state::parsing;
    for(auto &str : s) {
...
[[nodiscard]] inline results_t Option::reduced_results() const {
...
inline Option *Option::type_size(int option_type_size) {
...
inline Option *Option::type_size(int option_type_size_min, int option_type_size_max) {
...
[[nodiscard]] inline std::string Option::get_type_name() const {
...
inline void Option::_validate_results(results_t &res) const {
    // Run the Validators (can change the string)
    if(!validators_.empty()) {
        if(type_size_max_ > 1) {  // in this context index refers to the index in the type
            int index = 0;
            if(get_items_expected_max() < static_cast<int>(res.size()) &&
               (multi_option_policy_ == collie::MultiOptionPolicy::TakeLast ||
                multi_option_policy_ == collie::MultiOptionPolicy::Reverse)) {
...
inline void Option::_reduce_results(results_t &out, const results_t &original) const {
...
inline std::string Option::_validate(std::string &result, int index) const {
    std::string err_msg;
...
inline int Option::_add_result(std::string &&result, std::vector<std::string> &res) const {
    int result_count = 0;
    if(allow_extra_args_ && !result.empty() && result.front() == '[' &&
       result.back() == ']') {  // this is now a vector string likely from the default or user entry
        result.pop_back();

        for(auto &var : collie::detail::split(result.substr(1), ',')) {
            if(!var.empty()) {
    ...
    if(delimiter_ == '\0') {
        ...
        if((result.find_first_of(delimiter_) != std::string::npos)) {
            for(const auto &var : collie::detail::split(result, delimiter_)) {
                if(!var.empty()) {
                    res.push_back(var);
                    ++result_count;
                }
