...
#include <collie/testing/pico_bench.hpp>
...
namespace collie::simd
{
                    ...
                    int output[])
    {
        ...
        // float_batch_type programIndex(&arange[0], collie::simd::aligned_mode());

        auto programIndex = float_batch_type::load(&arange[0], collie::simd::aligned_mode());
        ...
        for (int j = 0; j < height; j++)
        {
            for (int i = 0; i < width; i += N)
            {
                ...
                // collie::simd::store_aligned(result, output + base_index, active);
...
void run_arch(
    ...
    int height,
    int maxIters,
    std::vector<int, collie::aligned_allocator<int, Align>>& buffer)
{
    std::fill(buffer.begin(), buffer.end(), 0);
    auto stats = bencher([&]()
                         { collie::simd::mandelbrot<arch>(x0, y0, x1, y1, width, height, maxIters, buffer.data()); });
...
int main()
{
    ...
    std::vector<int, collie::aligned_allocator<int, collie::simd::default_arch::alignment()>> buf(width * height);
    ...
    run_archlist<collie::simd::supported_architectures>::run(bencher, x0, y0, x1, y1, width, height, maxIters, buf);
