...
#ifndef COLLIE_SIMD_ARCH_SCALAR_H_
#define COLLIE_SIMD_ARCH_SCALAR_H_
...
namespace collie::simd {
    ...
    namespace detail {
#define COLLIE_SIMD_HASSINCOS_TRAIT(func)                                                                                                     \
    template <class S>                                                                                                                  \
    struct has##func                                                                                                                    \
    {                                                                                                                                   \
        template <class T>                                                                                                              \
...
#define COLLIE_SIMD_HASSINCOS(func, T) has##func<T>::value

        COLLIE_SIMD_HASSINCOS_TRAIT(sincos);

        COLLIE_SIMD_HASSINCOS_TRAIT(sincosf);

        COLLIE_SIMD_HASSINCOS_TRAIT(__sincos);

        COLLIE_SIMD_HASSINCOS_TRAIT(__sincosf);

        struct generic_sincosf {
            template<class T>
            inline typename std::enable_if<COLLIE_SIMD_HASSINCOS(sincosf, T), void>::type
            operator()(float val, T &s, T &c) {
                sincosf(val, &s, &c);
            }

            template<class T>
            inline typename std::enable_if<!COLLIE_SIMD_HASSINCOS(sincosf, T) && COLLIE_SIMD_HASSINCOS(__sincosf, T), void>::type
            operator()(float val, T &s, T &c) {
                __sincosf(val, &s, &c);
            }
        ...
        struct generic_sincos {
            template<class T>
            inline typename std::enable_if<COLLIE_SIMD_HASSINCOS(sincos, T), void>::type
            operator()(double val, T &s, T &c) {
                sincos(val, &s, &c);
            }

            template<class T>
            inline typename std::enable_if<!COLLIE_SIMD_HASSINCOS(sincos, T) && COLLIE_SIMD_HASSINCOS(__sincos, T), void>::type
            operator()(double val, T &s, T &c) {
                __sincos(val, &s, &c);
            }
...
#undef COLLIE_SIMD_HASSINCOS_TRAIT
#undef COLLIE_SIMD_HASSINCOS
    }
...
#endif  // COLLIE_SIMD_ARCH_SCALAR_H_
