...
#ifndef COLLIE_SIMD_ARCH_SSSE3_H_
#define COLLIE_SIMD_ARCH_SSSE3_H_
    ...
    namespace kernel {
        ...
        namespace detail {
            ...
            inline batch<T, A> extract_pair(batch<T, A> const &, batch<T, A> const &other, std::size_t,
                                            ::collie::simd::detail::index_sequence<>) noexcept {
                return other;
            }
            ...
            inline batch<T, A> extract_pair(batch<T, A> const &self, batch<T, A> const &other, std::size_t i,
                ...
                if (i == I) {
                    ...
                    return extract_pair(self, other, i, ::collie::simd::detail::index_sequence<Is...>());
            }
        }
        ...
        extract_pair(batch<T, A> const &self, batch<T, A> const &other, std::size_t i, requires_arch<ssse3>) noexcept {
            ...
            return detail::extract_pair(self, other, i, ::collie::simd::detail::make_index_sequence<size>());
        }
        ...
        swizzle(batch<uint8_t, A> const &self, batch<uint8_t, A> mask, requires_arch<ssse3>) noexcept {
            return _mm_shuffle_epi8(self, mask);
        }

        template<class A>
        inline batch<int8_t, A>
        swizzle(batch<int8_t, A> const &self, batch<uint8_t, A> mask, requires_arch<ssse3>) noexcept {
            return _mm_shuffle_epi8(self, mask);
        }
...
#endif  // COLLIE_SIMD_ARCH_SSSE3_H_
