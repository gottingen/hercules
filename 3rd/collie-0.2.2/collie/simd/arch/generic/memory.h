...
#ifndef COLLIE_SIMD_ARCH_GENERIC_MEMORY_H_
#define COLLIE_SIMD_ARCH_GENERIC_MEMORY_H_
...
namespace collie::simd {
    ...
    namespace kernel {
        ...
        namespace detail {
            ...
            inline batch<IT, A> create_compress_swizzle_mask(I bitmask, ::collie::simd::detail::index_sequence<Is...>) {
        ...
        compress(batch<T, A> const &x, batch_bool<T, A> const &mask,
            ...
            auto compress_mask = detail::create_compress_swizzle_mask<IT, A>(bitmask,
                                                                             ::collie::simd::detail::make_index_sequence<size>());
        ...
        namespace detail {
            template<class IT, class A, class I, size_t... Is>
            inline batch<IT, A> create_expand_swizzle_mask(I bitmask, ::collie::simd::detail::index_sequence<Is...>) {
                batch<IT, A> swizzle_mask(IT(0));
                IT j = 0;
                (void) std::initializer_list<bool>{
                        ((swizzle_mask = insert(swizzle_mask, j, index<Is>())), (j += ((bitmask >> Is) &
                                                                                       1u)), true)...};
                return swizzle_mask;
            }
        }
        ...
        expand(batch<T, A> const &x, batch_bool<T, A> const &mask,
            ...
            constexpr std::size_t size = batch_bool<T, A>::size;
            auto bitmask = mask.mask();
            auto swizzle_mask = detail::create_expand_swizzle_mask<as_unsigned_integer_t<T>, A>(bitmask,
                                                                                                ::collie::simd::detail::make_index_sequence<size>());
        ...
        // gather
        namespace detail {
            template<size_t N, typename T, typename A, typename U, typename V, typename std::enable_if<
                    N == 0, int>::type = 0>
            inline batch<T, A> gather(U const *src, batch<V, A> const &index,
                                      ::collie::simd::index<N> I) noexcept {
            ...
            template<size_t N, typename T, typename A, typename U, typename V, typename std::enable_if<
                    N != 0, int>::type = 0>
            inline batch<T, A>
            gather(U const *src, batch<V, A> const &index, ::collie::simd::index<N> I) noexcept {
        ...
        inline T get(batch<T, A> const &self, ::collie::simd::index<I>, requires_arch<generic>) noexcept {
        ...
        template<class A, size_t I, class T>
        inline T get(batch_bool<T, A> const &self, ::collie::simd::index<I>, requires_arch<generic>) noexcept {
        ...
        template<class A, size_t I, class T>
        inline auto get(batch<std::complex<T>, A> const &self, ::collie::simd::index<I>,
                        requires_arch<generic>) noexcept -> typename batch<std::complex<T>, A>::value_type {
        ...
        template<class A, class T>
        inline auto get(batch<std::complex<T>, A> const &self, std::size_t i,
                        requires_arch<generic>) noexcept -> typename batch<std::complex<T>, A>::value_type {
        ...
        namespace detail {
            ...
            load_aligned(T_in const *mem, convert<T_out>, requires_arch<generic>, with_fast_conversion) noexcept {
            ...
            template<class A, class T_in, class T_out>
            inline batch<T_out, A>
            load_aligned(T_in const *mem, convert<T_out>, requires_arch<generic>, with_slow_conversion) noexcept {
        ...
        namespace detail {
            template<class A, class T_in, class T_out>
            inline batch<T_out, A>
            load_unaligned(T_in const *mem, convert<T_out>, requires_arch<generic>, with_fast_conversion) noexcept {
                using batch_type_in = batch<T_in, A>;
                using batch_type_out = batch<T_out, A>;
            ...
            template<class A, class T_in, class T_out>
            inline batch<T_out, A>
            load_unaligned(T_in const *mem, convert<T_out> cvt, requires_arch<generic>, with_slow_conversion) noexcept {
                static_assert(!std::is_same<T_in, T_out>::value,
                              "there should be a direct load for this type combination");
        ...
        rotate_left(batch<std::complex<T>, A> const &self, requires_arch<generic>) noexcept {
        ...
        template<size_t N, class A, class T>
        inline batch<std::complex<T>, A>
        rotate_right(batch<std::complex<T>, A> const &self, requires_arch<generic>) noexcept {
        ...
        namespace detail {
            ...
            constexpr bool is_zip_hi(size_t bsize, ITy0 index0, ITy1 index1, ITys... indices) {
                return index0 == (bsize / 2 + bsize - (sizeof...(indices) + 2)) &&
                       index1 == (bsize / 2 + 2 * bsize - (sizeof...(indices) + 2)) && is_zip_hi(bsize, indices...);
            }
        ...
        inline batch<T, A>
        shuffle(batch<T, A> const &x, batch<T, A> const &y, batch_constant<batch<ITy, A>, Indices...>,
                requires_arch<generic>) noexcept {
            constexpr size_t bsize = sizeof...(Indices);

            // Detect common patterns
            if constexpr (detail::is_swizzle_fst(bsize, Indices...)) {
                return swizzle(x, batch_constant<batch<ITy, A>, ((Indices >= bsize) ? 0 /* never happens */
                                                                                    : Indices)...>());
            }

            if constexpr (detail::is_swizzle_snd(bsize, Indices...)) {
                return swizzle(y, batch_constant<batch<ITy, A>, ((Indices >= bsize) ? (Indices - bsize)
                                                                                    : 0 /* never happens */)...>());
            }
...
//            return __builtin_shuffle(x.data, y.data, integer_vector_type{Indices...});
            ...
            // pretty well.
            batch<T, A> x_lane = swizzle(x, batch_constant<batch<ITy, A>, ((Indices >= bsize) ? (Indices - bsize)
                                                                                              : Indices)...>());
            batch<T, A> y_lane = swizzle(y, batch_constant<batch<ITy, A>, ((Indices >= bsize) ? (Indices - bsize)
                                                                                              : Indices)...>());
        ...
        inline batch<std::complex<T>, A>
        swizzle(batch<std::complex<T>, A> const &self, batch_constant<batch<ITy, A>, Vs...> mask,
                requires_arch<generic>) noexcept {
        ...
        template<class A, class T, class ITy>
        inline batch<std::complex<T>, A>
        swizzle(batch<std::complex<T>, A> const &self, batch<ITy, A> mask, requires_arch<generic>) noexcept {
            return {swizzle(self.real(), mask), swizzle(self.imag(), mask)};
        }
        ...
        namespace detail {
            template<class A, class T>
            inline batch<std::complex<T>, A>
            load_complex(batch<T, A> const & /*hi*/, batch<T, A> const & /*lo*/, requires_arch<generic>) noexcept {
            ...
            template<class A, class T>
            inline batch<T, A>
            complex_high(batch<std::complex<T>, A> const & /*src*/, requires_arch<generic>) noexcept {
                static_assert(std::is_same<T, void>::value,
        ...
        inline batch<std::complex<T_out>, A>
        load_complex_aligned(std::complex<T_in> const *mem, convert<std::complex<T_out>>,
                             requires_arch<generic>) noexcept {
        ...
        template<class A, class T_out, class T_in>
        inline batch<std::complex<T_out>, A>
        load_complex_unaligned(std::complex<T_in> const *mem, convert<std::complex<T_out>>,
                               requires_arch<generic>) noexcept {
            using real_batch = batch<T_out, A>;
            T_in const *buffer = reinterpret_cast<T_in const *>(mem);
        ...
        template<class A, class T_out, class T_in>
        inline void store_complex_aligned(std::complex<T_out> *dst, batch<std::complex<T_in>, A> const &src,
                                          requires_arch<generic>) noexcept {
        ...
        template<class A, class T_out, class T_in>
        inline void store_complex_unaligned(std::complex<T_out> *dst, batch<std::complex<T_in>, A> const &src,
                                            requires_arch<generic>) noexcept {
            using real_batch = batch<T_in, A>;
            real_batch hi = detail::complex_high(src, A{});
            real_batch lo = detail::complex_low(src, A{});
...
#endif  // COLLIE_SIMD_ARCH_GENERIC_MEMORY_H_
