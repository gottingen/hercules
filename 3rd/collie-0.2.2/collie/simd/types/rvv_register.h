...
#ifndef COLLIE_SIMD_TYPES_RVV_REGISTER_H_
#define COLLIE_SIMD_TYPES_RVV_REGISTER_H_
...
namespace collie::simd {
    namespace detail {
        ...
        struct rvv : collie::simd::generic {
            static constexpr size_t width = Width;

            static constexpr bool supported() noexcept { return Width == COLLIE_SIMD_RVV_BITS; }
...
#define COLLIE_SIMD_RVV_JOINT_(a, b, c) a##b##c
#define COLLIE_SIMD_RVV_JOINT(a, b, c) COLLIE_SIMD_RVV_JOINT_(a, b, c)
#define COLLIE_SIMD_RVV_JOINT5(a, b, c, d, e) COLLIE_SIMD_RVV_JOINT(COLLIE_SIMD_RVV_JOINT(a, b, c), d, e)

#define COLLIE_SIMD_RVV_TYPE_i(S, V) COLLIE_SIMD_RVV_JOINT5(vint, S, m, V, _t)
#define COLLIE_SIMD_RVV_TYPE_u(S, V) COLLIE_SIMD_RVV_JOINT5(vuint, S, m, V, _t)
#define COLLIE_SIMD_RVV_TYPE_f(S, V) COLLIE_SIMD_RVV_JOINT5(vfloat, S, m, V, _t)
#define COLLIE_SIMD_RVV_TYPE(T, S, V) COLLIE_SIMD_RVV_JOINT(COLLIE_SIMD_RVV_TYPE, _, T)(S, V)

    namespace types
    {
        namespace detail
        {
            static constexpr size_t rvv_width_mf8 = COLLIE_SIMD_RVV_BITS / 8;
            static constexpr size_t rvv_width_mf4 = COLLIE_SIMD_RVV_BITS / 4;
            static constexpr size_t rvv_width_mf2 = COLLIE_SIMD_RVV_BITS / 2;
            static constexpr size_t rvv_width_m1 = COLLIE_SIMD_RVV_BITS;
            static constexpr size_t rvv_width_m2 = COLLIE_SIMD_RVV_BITS * 2;
            static constexpr size_t rvv_width_m4 = COLLIE_SIMD_RVV_BITS * 4;
            static constexpr size_t rvv_width_m8 = COLLIE_SIMD_RVV_BITS * 8;
...
#define COLLIE_SIMD_RVV_MAKE_TYPE(scalar, t, s, vmul)                                           \
    template <>                                                                           \
    struct rvv_type_info<scalar, rvv_width_m1 * vmul>                                     \
    {                                                                                     \
        static constexpr size_t width = rvv_width_m1 * vmul;                              \
        using type = COLLIE_SIMD_RVV_TYPE(t, s, vmul);                                          \
        using byte_type = COLLIE_SIMD_RVV_TYPE(u, 8, vmul);                                     \
        using fixed_type = type __attribute__((riscv_rvv_vector_bits(width)));            \
        template <class U>                                                                \
        static inline type bitcast(U x) noexcept                                          \
        {                                                                                 \
            const auto words = COLLIE_SIMD_RVV_JOINT5(__riscv_vreinterpret_, u, s, m, vmul)(x); \
            return COLLIE_SIMD_RVV_JOINT5(__riscv_vreinterpret_, t, s, m, vmul)(words);         \
        }                                                                                 \
        template <>                                                                       \
        ...
        {                                                                                 \
            const auto words = COLLIE_SIMD_RVV_JOINT5(__riscv_vreinterpret_, u, s, m, vmul)(x); \
            return COLLIE_SIMD_RVV_JOINT5(__riscv_vreinterpret_, u, 8, m, vmul)(words);         \
        }                                                                                 \
    };

#define COLLIE_SIMD_RVV_MAKE_TYPES(vmul)             \
    COLLIE_SIMD_RVV_MAKE_TYPE(int8_t, i, 8, vmul)    \
    COLLIE_SIMD_RVV_MAKE_TYPE(uint8_t, u, 8, vmul)   \
    COLLIE_SIMD_RVV_MAKE_TYPE(int16_t, i, 16, vmul)  \
    COLLIE_SIMD_RVV_MAKE_TYPE(uint16_t, u, 16, vmul) \
    COLLIE_SIMD_RVV_MAKE_TYPE(int32_t, i, 32, vmul)  \
    COLLIE_SIMD_RVV_MAKE_TYPE(uint32_t, u, 32, vmul) \
    COLLIE_SIMD_RVV_MAKE_TYPE(int64_t, i, 64, vmul)  \
    COLLIE_SIMD_RVV_MAKE_TYPE(uint64_t, u, 64, vmul) \
    COLLIE_SIMD_RVV_MAKE_TYPE(float, f, 32, vmul)    \
    COLLIE_SIMD_RVV_MAKE_TYPE(double, f, 64, vmul)

            COLLIE_SIMD_RVV_MAKE_TYPES(8)
            COLLIE_SIMD_RVV_MAKE_TYPES(4)
            COLLIE_SIMD_RVV_MAKE_TYPES(2)
            COLLIE_SIMD_RVV_MAKE_TYPES(1)
#undef COLLIE_SIMD_RVV_TYPE
#undef COLLIE_SIMD_RVV_TYPE_f
#undef COLLIE_SIMD_RVV_TYPE_u
#undef COLLIE_SIMD_RVV_TYPE_i
#undef COLLIE_SIMD_RVV_MAKE_TYPES
#undef COLLIE_SIMD_RVV_MAKE_TYPE
            ...
            struct rvv_reg
            {
                ...
                explicit rvv_reg(rvv_reg<U, Width> v, rvv_bitcast_flag)
                    : rvv_reg(v.get_bytes(), COLLIE_SIMD_RVV_BITCAST)
                {
                }
...
#define COLLIE_SIMD_RVV_MAKE_BOOL_TYPE(i)                                                       \
    template <>                                                                           \
    struct rvv_bool_info<i>                                                               \
    {                                                                                     \
        using type = COLLIE_SIMD_RVV_JOINT(vbool, i, _t);                                       \
        template <class T>                                                                \
        static inline type bitcast(T value) noexcept                                      \
        {                                                                                 \
            return COLLIE_SIMD_RVV_JOINT(__riscv_vreinterpret_b, i, )(value);                   \
        }                                                                                 \
        /*template <> static inline type bitcast(type value) noexcept { return value; }*/ \
    };
            COLLIE_SIMD_RVV_MAKE_BOOL_TYPE(1);
            COLLIE_SIMD_RVV_MAKE_BOOL_TYPE(2);
            COLLIE_SIMD_RVV_MAKE_BOOL_TYPE(4);
            COLLIE_SIMD_RVV_MAKE_BOOL_TYPE(8);
            COLLIE_SIMD_RVV_MAKE_BOOL_TYPE(16);
            COLLIE_SIMD_RVV_MAKE_BOOL_TYPE(32);
            COLLIE_SIMD_RVV_MAKE_BOOL_TYPE(64);
#undef COLLIE_SIMD_RVV_MAKE_BOOL_TYPE
#undef COLLIE_SIMD_RVV_JOINT5
#undef COLLIE_SIMD_RVV_JOINT
#undef COLLIE_SIMD_RVV_JOINT_
        ...
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(bool, rvv, detail::rvv_vector_type<unsigned char>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(signed char, rvv, detail::rvv_vector_type<signed char>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(unsigned char, rvv, detail::rvv_vector_type<unsigned char>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(char, rvv, detail::rvv_vector_type<char>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(short, rvv, detail::rvv_vector_type<short>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(unsigned short, rvv, detail::rvv_vector_type<unsigned short>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(int, rvv, detail::rvv_vector_type<int>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(unsigned int, rvv, detail::rvv_vector_type<unsigned int>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(long int, rvv, detail::rvv_vector_type<long int>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(unsigned long int, rvv, detail::rvv_vector_type<unsigned long int>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(long long int, rvv, detail::rvv_vector_type<long long int>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(unsigned long long int, rvv, detail::rvv_vector_type<unsigned long long int>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(float, rvv, detail::rvv_vector_type<float>);
        COLLIE_SIMD_DECLARE_SIMD_REGISTER(double, rvv, detail::rvv_vector_type<double>);
...
#endif  // COLLIE_SIMD_TYPES_RVV_REGISTER_H_
