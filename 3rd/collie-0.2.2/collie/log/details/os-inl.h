...
#if defined(CLOG_WCHAR_TO_UTF8_SUPPORT) || defined(CLOG_WCHAR_FILENAMES)
#include <cassert>
#include <limits>
#endif
...
#ifdef _WIN32
#ifdef CLOG_WCHAR_FILENAMES
        *fp = ::_wfsopen((filename.c_str()), mode.c_str(), _SH_DENYNO);
#else
        *fp = ::_fsopen((filename.c_str()), mode.c_str(), _SH_DENYNO);
#endif
#if defined(CLOG_PREVENT_CHILD_FD)
        if (*fp != nullptr) {
...
#else  // unix
#if defined(CLOG_PREVENT_CHILD_FD)
        const int mode_flag = mode == CLOG_FILENAME_T("ab") ? O_APPEND : O_TRUNC;
        const int fd =
...
#pragma warning(disable : 4702)
    ...
    inline size_t filesize(FILE *f) {
        if (f == nullptr) {
            throw_clog_ex("Failed getting file size. fd is null");
        }
...
#endif
#endif
        throw_clog_ex("Failed getting file size from fd", errno);
...
#endif
        if (rv == TIME_ZONE_ID_INVALID) throw_clog_ex("Failed getting timezone info. ", errno);
...
// wchar support for windows file names (CLOG_WCHAR_FILENAMES must be defined)
#if defined(_WIN32) && defined(CLOG_WCHAR_FILENAMES)
    ...
    inline void wstr_to_utf8buf(wstring_view_t wstr, memory_buf_t &target) {
        if (wstr.size() > static_cast<size_t>((std::numeric_limits<int>::max)()) / 2 - 1) {
            throw_clog_ex("UTF-16 string is too big to be converted to UTF-8");
        }
    ...
    inline void utf8_to_wstrbuf(string_view_t str, wmemory_buf_t &target) {
        if (str.size() > static_cast<size_t>((std::numeric_limits<int>::max)()) - 1) {
            throw_clog_ex("UTF-8 string is too big to be converted to UTF-16");
        }
...
#endif  // (defined(CLOG_WCHAR_TO_UTF8_SUPPORT) || defined(CLOG_WCHAR_FILENAMES)) &&
    // defined(_WIN32)
