...
#include <collie/taskflow/core/observer.h>
#include <collie/taskflow/core/taskflow.h>
#include <collie/taskflow/core/async_task.h>
...
*/
    ...
    public:
        ...
        */
        ...
        @param taskflow a collie::tf::Taskflow object

        @return a collie::tf::Future that holds the result of the execution

        This member function executes the given taskflow once and returns a collie::tf::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        collie::tf::Future<void> future = executor.run(taskflow);
        // do something else
        future.wait();
        @endcode
        ...
        */
        collie::tf::Future<void> run(Taskflow &taskflow);

        /**
        @brief runs a moved taskflow once

        @param taskflow a moved collie::tf::Taskflow object

        @return a collie::tf::Future that holds the result of the execution

        This member function executes a moved taskflow once and returns a collie::tf::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved taskflow.

        @code{.cpp}
        collie::tf::Future<void> future = executor.run(std::move(taskflow));
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        This member function returns a collie::tf::Future object that
        eventually holds the result of the execution.

        @code{.cpp}
        collie::tf::Future<void> future = executor.run(taskflow, [](){ std::cout << "done"; });
        // do something else
        future.wait();
        @endcode
        ...
        */
        template<typename C>
        collie::tf::Future<void> run(Taskflow &taskflow, C &&callable);
        ...
        @code{.cpp}
        collie::tf::Future<void> future = executor.run(
        ...
        */
        ...
        This member function executes the given taskflow @c N times and returns a collie::tf::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        collie::tf::Future<void> future = executor.run_n(taskflow, 2);  // run taskflow 2 times
        // do something else
        future.wait();
        @endcode
        ...
        */
        collie::tf::Future<void> run_n(Taskflow &taskflow, size_t N);
        ...
        This member function executes a moved taskflow @c N times and returns a collie::tf::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved taskflow.

        @code{.cpp}
        collie::tf::Future<void> future = executor.run_n(
        ...
        */
        ...
        @param taskflow a collie::tf::Taskflow
        @param N number of runs
        @param callable a callable object to be invoked after this run
        ...
        */
        template<typename C>
        collie::tf::Future<void> run_n(Taskflow &taskflow, size_t N, C &&callable);

        /**
        @brief runs a moved taskflow for @c N times and then invokes a callback

        @param taskflow a moved collie::tf::Taskflow
        @param N number of runs
        @param callable a callable object to be invoked after this run
        ...
        */
        ...
        @code{.cpp}
        collie::tf::Future<void> future = executor.run_until(
        ...
        */
        template<typename P>
        collie::tf::Future<void> run_until(Taskflow &taskflow, P &&pred);
        ...
        */
        ...
        collie::tf::Future<void> run_until(Taskflow &taskflow, P &&pred, C &&callable);
        ...
        */
        ...
        @tparam T target type which has `collie::tf::Graph& T::graph()` defined
        @param target the target task graph object

        The method runs a target graph which has `collie::tf::Graph& T::graph()` defined
        and waits until the execution completes.
        Unlike the typical flow of calling `collie::tf::Executor::run` series
        ...
        collie::tf::Taskflow taskflow;
        std::array<collie::tf::Taskflow, 1000> others;
        ...
        @attention
        You must call collie::tf::Executor::corun from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename T>
        void corun(T &target);
        ...
        @attention
        You must call collie::tf::Executor::corun_until from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename P>
        ...
        */
        ...
        @code{.cpp}
        collie::tf::Executor executor(4);   // 4 workers in the executor
        ...
        */
        ...
        @tparam Observer observer type derived from collie::tf::ObserverInterface
        ...
        collie::tf::ObserverInterface::on_entry and collie::tf::ObserverInterface::on_exit
        ...
        */
        ...
        /**
        @brief similar to collie::tf::Executor::async but does not return a future object
        ...
        This member function is more efficient than collie::tf::Executor::async
        ...
        */
        ...
        @tparam F callable type
        @tparam Tasks task types convertible to collie::tf::AsyncTask

        @param func callable object
        @param tasks asynchronous tasks on which this execution depends

        @return a collie::tf::AsyncTask handle

        This member function is more efficient than collie::tf::Executor::dependent_async
        ...
        @code{.cpp}
        collie::tf::AsyncTask A = executor.silent_dependent_async([](){ printf("A\n"); });
        collie::tf::AsyncTask B = executor.silent_dependent_async([](){ printf("B\n"); });
        ...
        */
        ...
        >
        collie::tf::AsyncTask silent_dependent_async(F &&func, Tasks &&... tasks);
        ...
        @code{.cpp}
        collie::tf::AsyncTask A = executor.silent_dependent_async("A", [](){ printf("A\n"); });
        collie::tf::AsyncTask B = executor.silent_dependent_async("B", [](){ printf("B\n"); });
        ...
        */
        ...
        >
        collie::tf::AsyncTask silent_dependent_async(P &&params, F &&func, Tasks &&... tasks);
        ...
        @code{.cpp}
        std::array<collie::tf::AsyncTask, 2> array {
        ...
        */
        ...
        >
        collie::tf::AsyncTask silent_dependent_async(F &&func, I first, I last);
        ...
        */
        ...
        >
        collie::tf::AsyncTask silent_dependent_async(P &&params, F &&func, I first, I last);
        ...
        @return a pair of a collie::tf::AsyncTask handle and
                          a @std_future that holds the result of the execution

        The example below creates three asynchronous tasks, @c A, @c B, and @c C,
        in which task @c C runs after task @c A and task @c B.
        Task @c C returns a pair of its collie::tf::AsyncTask handle and a std::future<int>
        that eventually will hold the result of the execution.
        ...
        You can mixed the use of collie::tf::AsyncTask handles
    ...
    private:
        ...
        void _set_up_graph(Graph &, Node *, Topology *, int, InlinedVector<Node *> &);
        ...
        void _process_async_dependent(Node *, collie::tf::AsyncTask &, size_t &);
    ...
    inline void Executor::_process_exception(Worker &, Node *node) {
        ...
        if (auto parent = node->_parent; parent) {
            ...
            // multiple tasks may throw, so we only take the first thrown exception
                                             ...
                                             ((tpg->_state.fetch_or(flag, std::memory_order_relaxed) &
                                               Topology::EXCEPTION) == 0)
                ) {
...
// Procedure: _invoke_subflow_task
    ...
    // Function: run
    inline collie::tf::Future<void> Executor::run(Taskflow &f) {
        return run_n(f, 1, []() {});
    }
    ...
    // Function: run
    template<typename C>
    collie::tf::Future<void> Executor::run(Taskflow &f, C &&c) {
    ...
    // Function: run_n
    inline collie::tf::Future<void> Executor::run_n(Taskflow &f, size_t repeat) {
    ...
    // Function: run_n
    template<typename C>
    collie::tf::Future<void> Executor::run_n(Taskflow &f, size_t repeat, C &&c) {
        return run_until(
...
// Function: run_until
    template<typename P>
    collie::tf::Future<void> Executor::run_until(Taskflow &f, P &&pred) {
...
// Function: run_until
    template<typename P, typename C>
    collie::tf::Future<void> Executor::run_until(Taskflow &f, P &&p, C &&c) {
        ...
        if (empty || p()) {
            ...
            promise.set_value();
            _decrement_topology();
            return collie::tf::Future<void>(promise.get_future());
        }
        ...
        collie::tf::Future<void> future(t->_promise.get_future(), t);
...
// Function: run_until
    template<typename P, typename C>
    collie::tf::Future<void> Executor::run_until(Taskflow &&f, P &&pred, C &&c) {
...
// Function: _set_up_topology
    ...
    inline void Executor::_set_up_graph(
            Graph &g, Node *parent, Topology *tpg, int state, InlinedVector<Node *> &src
    ) {
