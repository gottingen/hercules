...
#include <collie/taskflow/utility/os.h>
#include <collie/taskflow/utility/math.h>
#include <collie/container/inlined_vector.h>
#include <collie/taskflow/utility/serializer.h>
#include <collie/taskflow/core/error.h>
#include <collie/taskflow/core/declarations.h>
#include <collie/taskflow/core/semaphore.h>
#include <collie/taskflow/core/environment.h>
#include <collie/taskflow/core/topology.h>
#include <collie/taskflow/core/tsq.h>
...
*/
    ...
    public:
        ...
        Issues a collie::tf::Runtime::corun_all to finish all spawned asynchronous tasks
        ...
        */
        ...
        taskflow.emplace([&](collie::tf::Runtime& rt){
        ...
        */
        ...
        The difference to collie::tf::Executor::async is that the created asynchronous task
        pertains to the runtime object.
        Applications can explicitly issue collie::tf::Runtime::corun_all
        ...
        */
        ...
        This member function is more efficient than collie::tf::Runtime::async
        ...
        */
        ...
        /**
        @brief similar to collie::tf::Runtime::silent_async but the caller must be the worker of the runtime
        ...
        */
        ...
        A target can be one of the following forms:
          ...
          + a composable graph object with `collie::tf::Graph& T::graph()` defined

        @code{.cpp}
        // co-run a subflow and wait until all tasks complete
        taskflow.emplace([](collie::tf::Runtime& rt){
          rt.corun([](collie::tf::Subflow& sf){
            collie::tf::Task A = sf.emplace([](){});
            collie::tf::Task B = sf.emplace([](){});
          });
        });

        // co-run a taskflow and wait until all tasks complete
        collie::tf::Taskflow taskflow1, taskflow2;
        taskflow1.emplace([](){ std::cout << "running taskflow1\n"; });
        taskflow2.emplace([&](collie::tf::Runtime& rt){
        ...
        Although collie::tf::Runtime::corun blocks until the operation completes,
        ...
        @attention
        Only the worker of this collie::tf::Runtime can issue corun.
        */
        template<typename T>
        void corun(T &&target);
        ...
        */
        ...
        Coruns all asynchronous tasks (collie::tf::Runtime::async,
        collie::tf::Runtime::silent_async) with other workers until all those
        ...
        @attention
        Only the worker of this collie::tf::Runtime can issue collie::tf::Runtime::corun_all.
        */
...
*/
    class Node {
        ...
        struct Subflow {
            ...
            std::function<void(collie::tf::Subflow &)> work;
            Graph subgraph;
        };
        ...
        struct MultiCondition {
            ...
            std::variant<
                    std::function<InlinedVector<int>()>, std::function<InlinedVector<int>(Runtime & )>
            > work;
        };
    ...
    private:
        ...
        InlinedVector<Node *> _successors;
        InlinedVector<Node *> _dependents;
