...
#include <collie/taskflow/cuda/cuda_task.h>
#include <collie/taskflow/cuda/cuda_capturer.h>
...
taskflow.emplace([&](collie::tf::cudaFlow& cf){
  // create two kernel tasks
  collie::tf::cudaTask task1 = cf.kernel(grid1, block1, shm_size1, kernel1, args1);
  collie::tf::cudaTask task2 = cf.kernel(grid2, block2, shm_size2, kernel2, args2);
...
A %cudaFlow is a task (collie::tf::Task) created from collie::tf::Taskflow
and will be run by @em one worker thread in the executor.
That is, the callable that describes a %cudaFlow
will be executed sequentially.
Inside a %cudaFlow task, different GPU tasks (collie::tf::cudaTask) may run
...
class cudaFlow {
  ...
  public:
    ...
    */
    ...
    @return a collie::tf::cudaTask handle
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlow::host but operates on a task
    of type collie::tf::cudaTaskType::HOST.
    */
    template <typename C>
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlow::kernel but operates on a task
    of type collie::tf::cudaTaskType::KERNEL.
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlow::memset but operates on a task
    of type collie::tf::cudaTaskType::MEMSET.
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlow::memcpy but operates on a task
    of type collie::tf::cudaTaskType::MEMCPY.
    The source/destination memory may have different address values but
    must be allocated from the same contexts as the original
    source/destination memory.
    */
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlow::zero but operates on
    a task of type collie::tf::cudaTaskType::MEMSET.
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlow::fill but operates on a task
    of type collie::tf::cudaTaskType::MEMSET.
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlow::copy but operates on a task
    of type collie::tf::cudaTaskType::MEMCPY.
    The source/destination memory may have different address values but
    must be allocated from the same contexts as the original
    source/destination memory.
    ...
    */
    ...
    @brief updates parameters of a kernel task created from
           collie::tf::cudaFlow::for_each

    The type of the iterators and the callable must be the same as
    the task created from collie::tf::cudaFlow::for_each.
    */
    template <typename I, typename C>
    ...
    */
    ...
    /**
    @brief updates parameters of a kernel task created from
           collie::tf::cudaFlow::for_each_index

    The type of the iterators and the callable must be the same as
    the task created from collie::tf::cudaFlow::for_each_index.
    */
    template <typename I, typename C>
    void for_each_index(
    ...
    */
    ...
    /**
    @brief constructs a subflow graph through collie::tf::cudaFlowCapturer

    @tparam C callable type constructible from
              @c std::function<void(collie::tf::cudaFlowCapturer&)>
    ...
    taskflow.emplace([&](collie::tf::cudaFlow& cf){
      ...
      collie::tf::cudaTask my_kernel = cf.kernel(my_arguments);

      // create a flow capturer to capture custom kernels
      collie::tf::cudaTask my_subflow = cf.capture([&](collie::tf::cudaFlowCapturer& capturer){
    ...
    */
    ...
    The method is similar to collie::tf::cudaFlow::capture but operates on a task
    of type collie::tf::cudaTaskType::SUBFLOW.
