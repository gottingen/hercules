...
TEST_CASE("ScalablePipeline.Defer.Basics" * doctest::timeout(300)) {
  ...
  std::vector< collie::tf::Pipe<std::function<void(collie::tf::Pipeflow&)>> > pipes;

  for (size_t i = 0; i < N; i++) {
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [&](collie::tf::Pipeflow&) {});
  }

  using iterator_type = decltype(pipes)::iterator;

  collie::tf::ScalablePipeline<iterator_type> rhs;
  ...
  collie::tf::ScalablePipeline<iterator_type> lhs(std::move(rhs));
...
void scalable_pipeline_1P_S_DeferPreviousToken(size_t L, size_t W) {
  ...
  for (size_t N = 0; N <= maxN; N++) {
    ...
    // first pipe
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [L, N, &collection1, &deferrals1](collie::tf::Pipeflow& pf){
      if (pf.token() == N) {
        pf.stop();
        return;
      }
    ...
    collie::tf::ScalablePipeline<decltype(pipes)::iterator> spl(L);
    
    spl.reset(pipes.begin(), pipes.end());
 
    collie::tf::Task pipeline = taskflow.composed_of(spl); 

    collie::tf::Task test = taskflow.emplace([&](){
...
void scalable_pipeline_2P_SS_DeferPreviousToken(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    // first pipe
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [&mybuffer, N, &collection1, L, &deferrals1](auto& pf) mutable {
      if(pf.token() == N) {
        pf.stop();
        return;
      }
    ...
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [&mybuffer, &mutex, &collection2, L, &deferrals2](auto& pf) mutable {
      REQUIRE(pf.token() % L == pf.line());
      {
    ...
    collie::tf::ScalablePipeline<typename decltype(pipes)::iterator> spl(L);
...
void scalable_pipeline_2P_SP_DeferPreviousToken(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    pipes.emplace_back(collie::tf::PipeType::PARALLEL, [&mybuffer, &mutex, &collection2, L, &deferrals2](auto& pf) mutable {
      REQUIRE(pf.token() % L == pf.line());
      {
        std::scoped_lock<std::mutex> lock(mutex);
        collection2.push_back(mybuffer[pf.line()][pf.pipe() - 1]);
...
void scalable_pipeline_1P_S_DeferNextToken(size_t L, unsigned w, collie::tf::PipeType) {
  ...
  for(size_t N = 1; N <= maxN; N++) {
    ...
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [N, &collection1, &deferrals](auto& pf) mutable {
      if(pf.token() == N) {
        pf.stop();
        return;
      }
    ...
    collie::tf::ScalablePipeline<typename decltype(pipes)::iterator> spl(L, pipes.begin(), pipes.end());
...
void scalable_pipeline_2P_SS_DeferNextToken(size_t L, unsigned w, collie::tf::PipeType second_type) {
  ...
  for(size_t N = 1; N <= maxN; N++) {
    ...
    // first serial pipe
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [N, &mybuffer, &collection1, &deferrals1](auto& pf) mutable {
      if(pf.token() == N) {
        pf.stop();
        return;
      }
...
void scalable_pipeline_2P_SP_DeferNextToken(size_t L, unsigned w) {
  ...
  for(size_t N = 1; N <= maxN; N++) {
    ...
    // second parallel pipe
    pipes.emplace_back(collie::tf::PipeType::PARALLEL, [&mybuffer, &collection2, &mtx, &deferrals2](auto& pf) mutable {
      {
    ...
    collie::tf::ScalablePipeline<decltype(pipes)::iterator> spl(L, pipes.begin(), pipes.end());
...
void scalable_pipeline_1P_S_264VideoFormat(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    // first serial pipe
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [N, &collection1, &video, &deferrals1](auto& pf) mutable {
...
void scalable_pipeline_2P_SS_264VideoFormat(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    // first serial pipe
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [N, &collection1, &mybuffer, &video, &deferrals1](auto& pf) mutable {
      if(pf.token() == N) {
        //printf("Token %zu stops on line %zu\n", pf.token() ,pf.line());
        pf.stop();
        return;
    ...
    // second serial pipe
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [&mybuffer, &mutex, &collection2, &deferrals2](auto& pf) mutable {
      {
        std::scoped_lock<std::mutex> lock(mutex);
        collection2.push_back(mybuffer[pf.line()][pf.pipe() - 1]);
        deferrals2[pf.token()] = pf.num_deferrals();
...
void scalable_pipeline_2P_SP_264VideoFormat(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    //second parallel pipe
    pipes.emplace_back(collie::tf::PipeType::PARALLEL, [&mybuffer, &mutex, &collection2, &deferrals2](auto& pf) mutable {
      {
        std::scoped_lock<std::mutex> lock(mutex);
        collection2.push_back(mybuffer[pf.line()][pf.pipe() - 1]);
        deferrals2[pf.token()] = pf.num_deferrals();
...
void scalable_pipeline_3P_SPP_264VideoFormat(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    // third parallel pipe
    pipes.emplace_back(collie::tf::PipeType::PARALLEL, [&mybuffer, &mutex, &collection3, &deferrals3](auto& pf) mutable {
      {
        std::scoped_lock<std::mutex> lock(mutex);
