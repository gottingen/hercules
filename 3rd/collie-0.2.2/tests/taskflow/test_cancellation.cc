...
TEST_CASE("MultipleCancellations" * doctest::timeout(300)) {
  ...
  collie::tf::Taskflow taskflow1, taskflow2, taskflow3, taskflow4;
  collie::tf::Executor executor(4);
...
TEST_CASE("CancelSubflow" * doctest::timeout(300)) {
  ...
  // artificially long (possible larger than 300 seconds)
  for(int i=0; i<100; i++) {
    taskflow.emplace([&, i](collie::tf::Subflow& sf){
      for(int j=0; j<100; j++) {
        sf.emplace([&](){
          std::this_thread::sleep_for(std::chrono::milliseconds(100));
          counter.fetch_add(1, std::memory_order_relaxed);
...
TEST_CASE("CancelFromAnother" * doctest::timeout(300)) {
  ...
  collie::tf::Taskflow taskflow, another;
  collie::tf::Executor executor(4);
...
TEST_CASE("CancelComposition") {
  ...
  for(int r=0; r<100; r++) {
    ...
    std::vector<collie::tf::Future<void>> futures;
