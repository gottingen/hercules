...
void joined_subflow(unsigned W) {
  ...
  SUBCASE("Trivial") {
    ...
    collie::tf::Task subflow3, subflow3_;
    //std::future<int> fu3, fu3_;
    std::atomic<int> fu3v{0}, fu3v_{0};

    // empty flow
    auto subflow1 = tf.emplace([&] (collie::tf::Subflow& fb) {
      fu3v++;
      fb.join();
    }).name("subflow1");

    // nested empty flow
    auto subflow2 = tf.emplace([&] (collie::tf::Subflow& fb) {
      fu3v++;
      fb.emplace([&] (collie::tf::Subflow& fb2) {
        fu3v++;
        fb2.emplace( [&] (collie::tf::Subflow& fb3) {
          fu3v++;
          fb3.join();
        }).name("subflow2_1_1");
      }).name("subflow2_1");
    ...
    subflow3 = tf.emplace([&] (collie::tf::Subflow& fb) {
      ...
      subflow3_ = fb.emplace([&] (collie::tf::Subflow& fb2) {
        REQUIRE(fu3v_ == 3);
        fu3v++;
        fu3v_++;
        //return 200;
  ...
  SUBCASE("Complex") {
    ...
    auto B = tf.emplace([&count, &data, &sum](collie::tf::Subflow& fb){
      ...
      auto n = fb.emplace([&count](collie::tf::Subflow& fb2){
...
// --------------------------------------------------------
void detach_spawn(const int max_depth, std::atomic<int>& counter, int depth, collie::tf::Subflow& subflow)  {
  if(depth < max_depth) {
    counter.fetch_add(1, std::memory_order_relaxed);
    subflow.emplace([&, max_depth, depth=depth+1](collie::tf::Subflow& sfl){
      detach_spawn(max_depth, counter, depth, sfl); }
    );
    subflow.emplace([&, max_depth, depth=depth+1](collie::tf::Subflow& sfr){
...
void join_spawn(const int max_depth, std::atomic<int>& counter, int depth, collie::tf::Subflow& subflow)  {
  if(depth < max_depth) {
    counter.fetch_add(1, std::memory_order_relaxed);
    subflow.emplace([&, max_depth, depth=depth+1](collie::tf::Subflow& sfl){
...
void mix_spawn(
  const int max_depth, std::atomic<int>& counter, int depth, collie::tf::Subflow& subflow
) {
...
TEST_CASE("TreeSubflow" * doctest::timeout(300)) {
  ...
  SUBCASE("AllDetach") {
    ...
    for(int W=1; W<=4; W++) {
      ...
      collie::tf::Taskflow tf;
      tf.emplace([&](collie::tf::Subflow& subflow){
...
// Testcase: FibSubflow
// --------------------------------------------------------
int fibonacci_spawn(int n, collie::tf::Subflow& sbf) {
  if (n < 2) return n;
  int res1, res2;
  sbf.emplace([&res1, n] (collie::tf::Subflow& sbfl) { res1 = fibonacci_spawn(n - 1, sbfl); } );
  sbf.emplace([&res2, n] (collie::tf::Subflow& sbfr) { res2 = fibonacci_spawn(n - 2, sbfr); } );
