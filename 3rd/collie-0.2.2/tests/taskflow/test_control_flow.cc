...
TEST_CASE("Cond.Types") {
  ...
  static_assert(collie::tf::is_condition_task_v<decltype(explicit_c)>);
  static_assert(collie::tf::is_condition_task_v<decltype(implicit_c)>);

  REQUIRE(explicit_task.type() == collie::tf::TaskType::CONDITION);
  REQUIRE(implicit_task.type() == collie::tf::TaskType::CONDITION);
}
...
void conditional_spawn(
  ...
  const int max_depth,
  int depth,
  collie::tf::Subflow& subflow
)  {
  if(depth < max_depth) {
    for(int i=0; i<2; i++) {
      auto A = subflow.emplace([&](){ counter++; });
      auto B = subflow.emplace(
        [&, max_depth, depth=depth+1](collie::tf::Subflow& sf2){
...
TEST_CASE("BTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    for(int l=1; l<12; l++) {
      collie::tf::Taskflow flow;
      std::vector<collie::tf::Task> prev_tasks;
...
TEST_CASE("DynamicBTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    ...
    collie::tf::Taskflow flow;
    flow.emplace([&](collie::tf::Subflow& subflow) {
      counter++;
...
void nested_cond(unsigned w) {
  ...
  collie::tf::Taskflow flow;
  auto S = flow.emplace([](){});
  auto A = flow.emplace([&] (collie::tf::Subflow& subflow) mutable {
    //           ___________
    //          |           |
    //          v           |
    //   S1 -> A1 -> B1 -> cond
    ...
    auto B1 = subflow.emplace([&](collie::tf::Subflow& sf){
...
void hierarchical_condition(unsigned w) {
  ...
  collie::tf::Executor executor(w);
  collie::tf::Taskflow tf0("c0");
  collie::tf::Taskflow tf1("c1");
  collie::tf::Taskflow tf2("c2");
  collie::tf::Taskflow tf3("top");
  ...
  auto sbf1 = tf3.emplace([&](collie::tf::Subflow& sbf){
...
TEST_CASE("MultiCond.Types") {
  ...
  auto explicit_mc = [](){
    collie::InlinedVector<int> v;
    return v;
  };

  auto implicit_mc = []() -> collie::InlinedVector<int> {
    return {1, 2, 3, 9};
  };
  ...
  static_assert(collie::tf::is_multi_condition_task_v<decltype(explicit_mc)>);
  static_assert(collie::tf::is_multi_condition_task_v<decltype(implicit_mc)>);
...
void multiple_branches(unsigned W) {
  ...
  int ans = 0;
  collie::InlinedVector<int> conds;
...
void multiple_loops(unsigned W) {
  ...
  auto B = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> collie::InlinedVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto C = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> collie::InlinedVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto D = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> collie::InlinedVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto E = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> collie::InlinedVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
...
void binary_tree(unsigned w) {
  ...
  for(int i=1; i<(1<<N); i++) {
    tasks.emplace_back(taskflow.emplace([&counter]() -> collie::InlinedVector<int>{
      counter.fetch_add(1, std::memory_order_relaxed);
      return {0, 1};
    }));
  }
