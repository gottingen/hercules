...
TEST_CASE("Type" * doctest::timeout(300)) {
  ...
  collie::tf::Taskflow taskflow, taskflow2;

  auto t1 = taskflow.emplace([](){});
  auto t2 = taskflow.emplace([](){ return 1; });
  auto t3 = taskflow.emplace([](collie::tf::Subflow&){ });
  auto t4 = taskflow.composed_of(taskflow2);
  auto t5 = taskflow.emplace([](){ return collie::InlinedVector{1, 2}; });
  auto t6 = taskflow.emplace([](collie::tf::Runtime&){});
  auto t7 = taskflow.emplace([](collie::tf::Runtime&){ return 1; });
  auto t8 = taskflow.emplace([](collie::tf::Runtime&){ return collie::InlinedVector{1, 2}; });

  REQUIRE(t1.type() == collie::tf::TaskType::STATIC);
  REQUIRE(t2.type() == collie::tf::TaskType::CONDITION);
  REQUIRE(t3.type() == collie::tf::TaskType::SUBFLOW);
  REQUIRE(t4.type() == collie::tf::TaskType::MODULE);
  REQUIRE(t5.type() == collie::tf::TaskType::CONDITION);
  REQUIRE(t6.type() == collie::tf::TaskType::STATIC);
  REQUIRE(t7.type() == collie::tf::TaskType::CONDITION);
  REQUIRE(t8.type() == collie::tf::TaskType::CONDITION);
}
...
TEST_CASE("Builder" * doctest::timeout(300)) {
  ...
  std::vector<collie::tf::Task> silent_tasks;
...
TEST_CASE("Iterators" * doctest::timeout(300)) {
  ...
  SUBCASE("Order") {
    ...
    A.for_each_successor([&, i=0] (collie::tf::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == B);
        break;
    ...
    E.for_each_dependent([&, i=0](collie::tf::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == A);
        break;
  ...
  SUBCASE("Generic") {
    ...
    taskflow.for_each_task([&tasks](collie::tf::Task s){
      tasks.push_back(s);
    });
    ...
    A.for_each_successor([B](collie::tf::Task s){ REQUIRE(s==B); });
    B.for_each_dependent([A](collie::tf::Task s){ REQUIRE(s==A); });
    ...
    A.for_each_successor([&](collie::tf::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
    ...
    counter = a = b = c = d = e = 0;
    B.for_each_dependent([&](collie::tf::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
...
TEST_CASE("Hash" * doctest::timeout(300)) {
  ...
  std::hash<collie::tf::Task> hash;

  // empty hash
  collie::tf::Task t1, t2;
...
void sequential_runs(unsigned W) {
  ...
  SUBCASE("RunWithFuture") {
    ...
    collie::tf::Taskflow f;
    auto A = f.emplace([&](){ count ++; });
    auto B = f.emplace([&](collie::tf::Subflow& subflow){
      count ++;
    ...
    std::list<collie::tf::Future<void>> fu_list;
  ...
  SUBCASE("MultipleRuns") {
    ...
    collie::tf::Taskflow tf1, tf2, tf3, tf4;
...
TEST_CASE("RunAndWait.Simple") {
  ...
  collie::tf::Taskflow taskflow("Demo");
  ...
  collie::tf::Taskflow others;
  collie::tf::Task A = others.emplace([&](){ counter++; });
  collie::tf::Task B = others.emplace([&](){ counter++; });
  A.precede(B);

  // main taskflow
  collie::tf::Task C = taskflow.emplace([&](){
    executor.corun(others);
    REQUIRE(counter == 2);
  });
  collie::tf::Task D = taskflow.emplace([&](){
    executor.corun(others);
    REQUIRE(counter == 4);
  });
  C.precede(D);
...
void worker_id(unsigned w) {
  ...
  for(int i=0; i<1000; i++) {
    ...
    auto B = taskflow.emplace([&](collie::tf::Subflow& sf){
      auto id = executor.this_worker_id();
      REQUIRE(id>=0);
      REQUIRE(id< w);
      sf.emplace([&](){
...
void parallel_runs(unsigned w) {
  ...
  auto make_taskflow = [&] (collie::tf::Taskflow& tf) {
  ...
  SUBCASE("RunAndWaitForAll") {
    collie::tf::Executor executor(w);
    counter = 0;
    std::vector<std::unique_ptr<collie::tf::Taskflow>> taskflows(32);
    std::atomic<size_t> barrier(0);
    for(int t=0; t<32; t++) {
      threads.emplace_back([&, t=t] () {
        taskflows[t] = std::make_unique<collie::tf::Taskflow>();
...
void observer(unsigned w) {
  ...
  auto observer = executor.make_observer<collie::tf::ChromeObserver>();

  collie::tf::Taskflow taskflowA;
  std::vector<collie::tf::Task> tasks;
  // Static tasking
  for(auto i=0; i < 64; i ++) {
