...
namespace detail
{
    ...
    using uses_fast_cast = std::is_same<collie::simd::kernel::detail::conversion_type<Arch, From, To>,
                                        collie::simd::kernel::detail::with_fast_conversion>;
}
...
struct batch_cast_test
{
    ...
    using int8_batch = collie::simd::batch<int8_t>;
    using uint8_batch = collie::simd::batch<uint8_t>;
    using int16_batch = collie::simd::batch<int16_t>;
...
#if 0 && COLLIE_SIMD_X86_INSTR_SET >= COLLIE_SIMD_X86_AVX_VERSION
...
#if 0 && COLLIE_SIMD_X86_INSTR_SET >= COLLIE_SIMD_X86_AVX512_VERSION
    template <size_t Align = A>
...
private:
    ...
    void test_cast_impl(T test_value, const std::string& name) const
    {
        ...
        using B_common_in = collie::simd::batch<T_in>;
        using B_common_out = collie::simd::batch<T_out>;

        T_in in_test_value = static_cast<T_in>(test_value);
        if (detail::is_convertible<T_out>(in_test_value))
        {
            B_common_out res = collie::simd::batch_cast<T_out>(B_common_in(in_test_value));
            INFO(name);
    ...
    void test_bool_cast_impl(const std::string& name) const
    {
        using T_in = typename B_in::value_type;
        using T_out = typename B_out::value_type;
        using B_common_in = collie::simd::batch_bool<T_in>;
        using B_common_out = collie::simd::batch_bool<T_out>;

        B_common_in all_true_in(true);
        B_common_out all_true_res = collie::simd::batch_bool_cast<T_out>(all_true_in);
        INFO(name);
        CHECK_SCALAR_EQ(all_true_res.get(0), true);

        B_common_in all_false_in(false);
        B_common_out all_false_res = collie::simd::batch_bool_cast<T_out>(all_false_in);
        INFO(name);
...
TEST_CASE_TEMPLATE("[simd cast tests]", B, CONVERSION_TYPES)
{
    ...
    {
        using A = collie::simd::default_arch;
