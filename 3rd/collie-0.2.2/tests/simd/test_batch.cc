...
struct batch_test
{
    ...
    void test_load_store() const
    {
        ...
        alignas(collie::simd::default_arch::alignment()) array_type arhs(this->rhs);
        alignas(collie::simd::default_arch::alignment()) array_type ares;
    ...
    void test_incr_decr() const
    {
        ...
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(), collie::simd::incr<value_type>);
            batch_type res = collie::simd::incr(batch_lhs());
            INFO("incr(batch)");
            CHECK_BATCH_EQ(res, expected);
        }
        ...
        // incr_if
        {
            ...
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           [](value_type v)
                           { return v > 1 ? v + 1 : v; });
            batch_type res = collie::simd::incr_if(batch_lhs(), batch_lhs() > value_type(1));
            INFO("incr_if(batch)");
            CHECK_BATCH_EQ(res, expected);
        }
        ...
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(), collie::simd::decr<value_type>);
            batch_type res = collie::simd::decr(batch_lhs());
            INFO("decr(batch)");
            CHECK_BATCH_EQ(res, expected);
        }
        ...
        // decr_if
        {
            ...
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           [](value_type v)
                           { return v > 1 ? v - 1 : v; });
            batch_type res = collie::simd::decr_if(batch_lhs(), batch_lhs() > value_type(1));
            INFO("decr_if(batch)");
            CHECK_BATCH_EQ(res, expected);
        }
    }
    ...
    void test_saturated_arithmetic() const
    {
        ...
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), rhs.cbegin(), expected.begin(), collie::simd::sadd<value_type>);
            batch_type res = collie::simd::sadd(batch_lhs(), batch_rhs());
        ...
        // batch + scalar
        {
            ...
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(), [this](value_type x)
                           { return collie::simd::sadd(x, scalar); });
            batch_type lres = collie::simd::sadd(batch_lhs(), scalar);
            INFO("sadd(batch, scalar)");
            CHECK_BATCH_EQ(lres, expected);
            batch_type rres = collie::simd::sadd(scalar, batch_lhs());
        ...
        // batch - batch
        {
            ...
            std::transform(lhs.cbegin(), lhs.cend(), rhs.cbegin(), expected.begin(), [](value_type x, value_type y)
                           { return collie::simd::ssub(x, y); });
            batch_type res = collie::simd::ssub(batch_lhs(), batch_rhs());
        ...
        // batch - scalar
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(), [this](value_type x)
                           { return collie::simd::ssub(x, scalar); });
            batch_type lres = collie::simd::ssub(batch_lhs(), scalar);
            INFO("ssub(batch, scalar)");
            CHECK_BATCH_EQ(lres, expected);
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(), [this](value_type x)
                           { return collie::simd::ssub(scalar, x); });
            batch_type rres = collie::simd::ssub(scalar, batch_lhs());
    ...
    void test_min_max() const
    {
        ...
        // min limit case
        {
            ...
            batch_type res = collie::simd::min(batch_type(std::numeric_limits<value_type>::min()), batch_rhs());
            INFO("min limit");
            CHECK_BATCH_EQ(res, expected);
        }
        // fmin
        ...
        // max limit case
        {
            ...
            batch_type res = collie::simd::max(batch_type(std::numeric_limits<value_type>::max()), batch_rhs());
            INFO("max limit");
            CHECK_BATCH_EQ(res, expected);
        }
        // fmax
    ...
    void test_fused_operations() const
    {
        // fma
        {
            ...
            batch_type res = collie::simd::fma(batch_lhs(), batch_rhs(), batch_rhs());
            INFO("fma");
            CHECK_BATCH_EQ(res, expected);
        }
        // fms
    ...
    typename std::enable_if<4 <= N, void>::type test_generic_horizontal_operations(std::integral_constant<size_t, N>) const
    {
        // reduce generic
        {
            ...
            value_type res = reduce(collie::simd::mul<typename B::value_type, typename B::arch_type>, batch_lhs());
            INFO("generic reduce");
            CHECK_SCALAR_EQ(res, expected);
        }
    }
...
private:
    ...
    collie::simd::enable_integral_t<T, void> init_operands()
    {
        for (size_t i = 0; i < size; ++i)
        {
    ...
    template <class T = value_type>
    collie::simd::enable_floating_point_t<T, void> init_operands()
    {
        for (size_t i = 0; i < size; ++i)
        {
