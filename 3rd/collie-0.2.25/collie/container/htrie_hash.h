...
#include <collie/container/internal/array_map.h>
#include <collie/container/internal/array_set.h>
...
#define COLLIE_HT_HAS_STRING_VIEW
#endif
#endif

#ifdef COLLIE_HT_HAS_STRING_VIEW
...
#ifdef TSL_DEBUG
#define collie_ht_assert(expr) assert(expr)
#else
#define collie_ht_assert(expr) (static_cast<void>(0))
#endif
        ...
        private:
            using array_hash_type = typename std::conditional<
                    ...
                    collie::array_map<CharT, T, Hash, collie::ah::str_equal<CharT>, false, KeySizeT,
                            std::uint16_t, collie::ah::power_of_two_growth_policy<4>>,
        ...
        private:
            ...
            public:
                ...
                CharT child_of_char() const noexcept {
                    collie_ht_assert(parent() != nullptr);
            ...
            public:
                ...
                const anode *next_child(const anode &current_child) const noexcept {
                    collie_ht_assert(current_child.parent() == this);
        ...
        public:
            ...
            private:
                ...
                htrie_hash_iterator(hash_node_type &start_hash_node,
                    ...
                    collie_ht_assert(!m_current_hash_node->array_hash().empty());
                }
                ...
                htrie_hash_iterator(trie_node_type &start_trie_node) noexcept
                    ...
                    collie_ht_assert(m_current_trie_node->val_node() != nullptr);
                }
            ...
            public:
                ...
                void key(std::basic_string<CharT> &key_buffer_out) const {
                    ...
                    if (!m_read_trie_node_value) {
                        collie_ht_assert(m_current_hash_node != nullptr);
                ...
                reference value() const {
                    if (this->m_read_trie_node_value) {
                        collie_ht_assert(this->m_current_trie_node != nullptr);
                        collie_ht_assert(this->m_current_trie_node->val_node() != nullptr);
                ...
                htrie_hash_iterator &operator++() {
                    if (m_read_trie_node_value) {
                        collie_ht_assert(m_current_trie_node != nullptr);
            ...
            private:
                void hash_node_prefix(std::basic_string<CharT> &key_buffer_out) const {
                    collie_ht_assert(!m_read_trie_node_value);
                    key_buffer_out.clear();
                ...
                template<bool TIsPrefixIterator = IsPrefixIterator,
                        typename std::enable_if<TIsPrefixIterator>::type * = nullptr>
                void filter_prefix() {
                    collie_ht_assert(m_array_hash_iterator != m_array_hash_end_iterator);
                    collie_ht_assert(!m_read_trie_node_value && m_current_hash_node != nullptr);
                ...
                void set_next_node_ascending(anode_type &current_trie_node_child) {
                    collie_ht_assert(m_current_trie_node != nullptr);
                    collie_ht_assert(current_trie_node_child.parent() == m_current_trie_node);
                ...
                void set_most_left_descendant_as_next_node(anode_type &search_start) {
                    if (search_start.is_hash_node()) {
                        ...
                        if (m_current_trie_node->val_node() != nullptr) {
                            ...
                            collie_ht_assert(first_child != nullptr);
                ...
                void set_current_hash_node(hash_node_type &hnode) {
                    collie_ht_assert(!hnode.array_hash().empty());
            ...
            void shrink_to_fit() {
                ...
                while (first != last) {
                    if (first.m_read_trie_node_value) {
                        ...
                        collie_ht_assert(hnode != nullptr);
        ...
        private:
            ...
            std::pair<iterator, bool> insert_in_hash_node(hash_node &hnode,
                ...
                if (need_burst(hnode)) {
                    ...
                    if (hnode.parent() == nullptr) {
                        collie_ht_assert(m_root.get() == &hnode);
            ...
            iterator erase(iterator pos) {
                ...
                if (pos.m_read_trie_node_value) {
                    collie_ht_assert(pos.m_current_trie_node != nullptr &&
                    ...
                    collie_ht_assert(pos.m_current_hash_node != nullptr);
            ...
            void clear_empty_nodes(anode &empty_node) noexcept {
                collie_ht_assert(!empty_node.is_trie_node() ||
                              (empty_node.as_trie_node().empty() &&
                               empty_node.as_trie_node().val_node() == nullptr));
                collie_ht_assert(!empty_node.is_hash_node() ||
                              empty_node.as_hash_node().array_hash().empty());

                trie_node *parent = empty_node.parent();
                if (parent == nullptr) {
                    collie_ht_assert(m_root.get() == &empty_node);
                    collie_ht_assert(m_nb_elements == 0);
                    m_root.reset(nullptr);
                    ...
                    collie_ht_assert(m_root.get() == empty_node.parent());
                    collie_ht_assert(m_nb_elements == 0);
                    m_root.reset(nullptr);
                } else {
                    /**
            ...
            std::unique_ptr<trie_node> burst(hash_node &node) {
                ...
                collie_ht_assert(new_node->val_node() != nullptr || !new_node->empty());
                return new_node;
            }
            ...
            void deserialize_impl(Deserializer &deserializer, bool hash_compatible) {
                ...
                while (m_nb_elements < nb_elements) {
                    ...
                    if (node_type == slz_node_type::TRIE_NODE) {
                        ...
                        if (str_size == 0) {
                            collie_ht_assert(m_nb_elements == 0 && !m_root);
                            ...
                            collie_ht_assert(m_nb_elements == nb_elements);
                        } else {
                            str_buffer.resize(str_size);
                            deserializer(str_buffer.data(), str_size);
            ...
            void deserialize_value_node(Deserializer & /*deserializer*/,
                                        trie_node *current_node) {
                collie_ht_assert(!current_node->val_node());
