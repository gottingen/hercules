...
#include <collie/container/internal/array_growth_policy.h>
...
#define COLLIE_AH_HAS_STRING_VIEW
#endif
#endif
...
#ifdef TSL_DEBUG
#define collie_ah_assert(expr) assert(expr)
#else
#define collie_ah_assert(expr) (static_cast<void>(0))
#endif
    ...
    namespace detail_array_hash {
        ...
        public:
            ...
            const_iterator append(const_iterator end_of_bucket, const CharT *key,
                ...
                if (end_of_bucket == cend()) {
                    collie_ah_assert(m_buffer == nullptr);
                    ...
                    collie_ah_assert(is_end_of_bucket(end_of_bucket.m_position));
            ...
            const_iterator erase(const_iterator position) noexcept {
                collie_ah_assert(position.m_position != nullptr &&
            ...
            void serialize(Serializer &serializer) const {
                ...
                collie_ah_assert(m_buffer != nullptr || bucket_size == 0);
            ...
            static array_bucket deserialize(Deserializer &deserializer) {
                ...
                collie_ah_assert(bucket.size() == bucket_size);
                return bucket;
            }
        ...
        public:
            ...
            private:
                array_hash_iterator(iterator_buckets buckets_iterator,
                    ...
                    collie_ah_assert(m_array_hash != nullptr);
                }
            ...
            public:
                ...
                array_hash_iterator &operator++() {
                    collie_ah_assert(m_buckets_iterator != m_array_hash->m_buckets_data.end());
                    collie_ah_assert(m_array_bucket_iterator != m_buckets_iterator->cend());
        ...
        private:
            ...
            void clear_old_erased_values() {
                ...
                collie_ah_assert(m_nb_elements == this->m_values.size());
            }
            ...
            void deserialize_impl(Deserializer &deserializer, bool hash_compatible) {
                collie_ah_assert(m_buckets_data.empty());  // Current hash table must be empty
            ...
            void deserialize_bucket_values(Deserializer &deserializer,
                ...
                for (auto it = bucket.begin(); it != bucket.end(); ++it) {
                    ...
                    collie_ah_assert(this->m_values.size() - 1 <=
                                  std::numeric_limits<IndexSizeT>::max());
