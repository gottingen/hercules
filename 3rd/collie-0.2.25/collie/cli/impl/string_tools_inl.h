...
namespace collie {
    ...
    namespace detail {
        ...
        inline std::string &remove_outer(std::string &str, char key) {
        ...
        inline std::uint32_t hexConvert(char hc) {
            int hcode{0};
            if (hc >= '0' && hc <= '9') {
                hcode = (hc - '0');
            } else if (hc >= 'A' && hc <= 'F') {
        ...
        inline char make_char(std::uint32_t code) { return static_cast<char>(static_cast<unsigned char>(code)); }

        inline void append_codepoint(std::string &str, std::uint32_t code) {
                ...
                if (*loc == '\\') {
                    ...
                    if (ecloc != std::string::npos) {
                        ...
                        throw std::invalid_argument(
                                std::string("unrecognized escape sequence \\") + *(loc + 1) + " in " + str);
                    }
                } else {
                    out.push_back(*loc);
                }
        ...
        inline std::size_t close_string_quote(const std::string &str, std::size_t start, char closure_char) {
            std::size_t loc{0};
        ...
        inline std::size_t close_literal_quote(const std::string &str, std::size_t start, char closure_char) {
        ...
        inline std::vector<std::string> split_up(std::string str, char delimiter) {
