...
public:
    qt_sink(QObject *qt_object, std::string meta_method)
        ...
        if (!qt_object_) {
            throw_clog_ex("qt_sink: qt_object is null");
        }
    }
...
// Color location is determined by the sink log pattern like in the rest of clog sinks.
...
public:
    qt_color_sink(QTextEdit *qt_text_edit,
        ...
        : qt_text_edit_(qt_text_edit),
          ...
          is_utf8_(is_utf8) {
        if (!qt_text_edit_) {
            throw_clog_ex("qt_color_text_sink: text_edit is null");
        }
