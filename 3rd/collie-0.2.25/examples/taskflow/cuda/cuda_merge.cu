...
int main(int argc, char* argv[]) {
  ...
  // gpu data
  auto da = collie::tf::cuda_malloc_shared<int>(N);
  auto db = collie::tf::cuda_malloc_shared<int>(N);
  auto dc = collie::tf::cuda_malloc_shared<int>(N + N);
  ...
  collie::tf::cuda_merge(policy,
    da, da+N, db, db+N, dc, collie::tf::cuda_less<int>{}, buf
  );
  stream.synchronize();
