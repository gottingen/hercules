...
#include <collie/taskflow/sycl/syclflow.h>
...
int main() {
  ...
  collie::tf::Taskflow taskflow("saxpy example");
  ...
  taskflow.emplace_on([&](collie::tf::syclFlow& sf){

    collie::tf::syclTask fillX = sf.fill(X, 1.0f, N).name("fillX");
    collie::tf::syclTask fillY = sf.fill(Y, 2.0f, N).name("fillY");

    collie::tf::syclTask saxpy = sf.parallel_for(sycl::range<1>(N),
      [=] (sycl::id<1> id) {
        X[id] = 3.0f * X[id] + Y[id];
      }
    ).name("saxpy");
