...
void pipeline_1P_S_DeferPreviousToken(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &collection1, L, &deferrals](auto& pf) mutable {
        if(pf.token() == N) {
          pf.stop();
          return;
        }
...
void pipeline_2P_SS_DeferPreviousToken(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &collection1, &mybuffer, L, &deferrals1](auto& pf) mutable {
        if(pf.token() == N) {
          pf.stop();
          return;
        }
      ...
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [&mybuffer, &mutex, &collection2, L, &deferrals2](auto& pf) mutable {
        REQUIRE(pf.token() % L == pf.line());
        {
...
void pipeline_2P_SP_DeferPreviousToken(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [&mybuffer, &mutex, &collection2, L, &deferrals2](auto& pf) mutable {
        REQUIRE(pf.token() % L == pf.line());
        {
          std::scoped_lock<std::mutex> lock(mutex);
          collection2.push_back(mybuffer[pf.line()][pf.pipe() - 1]);
...
void pipeline_1P_S_DeferNextToken(size_t L, unsigned w, collie::tf::PipeType) {
  ...
  for(size_t N = 1; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &collection1, &deferrals](auto& pf) mutable {
        if(pf.token() == N) {
          pf.stop();
          return;
        }
...
void pipeline_2P_SS_DeferNextToken(size_t L, unsigned w, collie::tf::PipeType second_type) {
  ...
  for(size_t N = 1; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &mybuffer, &collection1, &deferrals1](auto& pf) mutable {
        if(pf.token() == N) {
          pf.stop();
          return;
        }
      ...
      collie::tf::Pipe{second_type, [&mybuffer, &collection2, &deferrals2](auto& pf) mutable {
...
void pipeline_2P_SP_DeferNextToken(size_t L, unsigned w) {
  ...
  for(size_t N = 1; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [&mybuffer, &collection2, &mtx, &deferrals2](auto& pf) mutable {
        {
...
void pipeline_1P_S_264VideoFormat(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &collection1, &video, &deferrals1](auto& pf) mutable {
...
void pipeline_2P_SS_264VideoFormat(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &collection1, &mybuffer, &video, &deferrals1](auto& pf) mutable {
        if(pf.token() == N) {
          //printf("Token %zu stops on line %zu\n", pf.token() ,pf.line());
          pf.stop();
          return;
      ...
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [&mybuffer, &mutex, &collection2, &deferrals2](auto& pf) mutable {
        {
          std::scoped_lock<std::mutex> lock(mutex);
          collection2.push_back(mybuffer[pf.line()][pf.pipe() - 1]);
          deferrals2[pf.token()] = pf.num_deferrals();
...
void pipeline_2P_SP_264VideoFormat(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [&mybuffer, &mutex, &collection2, &deferrals2](auto& pf) mutable {
        {
          std::scoped_lock<std::mutex> lock(mutex);
          collection2.push_back(mybuffer[pf.line()][pf.pipe() - 1]);
          deferrals2[pf.token()] = pf.num_deferrals();
...
void pipeline_3P_SPP_264VideoFormat(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [&mybuffer, &mutex, &collection3, &deferrals3](auto& pf) mutable {
        {
          std::scoped_lock<std::mutex> lock(mutex);
