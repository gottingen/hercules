...
void scalable_pipeline(size_t num_lines, size_t num_pipes) {
  ...
  for(size_t i=0; i<num_pipes; i++) {
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [&](collie::tf::Pipeflow& pf) mutable {
  ...
  collie::tf::ScalablePipeline spl(num_lines, pipes.begin(), pipes.end());
...
void scalable_pipeline_reset(size_t num_lines, size_t num_pipes) {
  ...
  collie::tf::ScalablePipeline<typename decltype(pipes)::iterator> spl(num_lines);
...
void scalable_pipeline_lines_reset(size_t num_lines, size_t num_pipes) {
  ...
  std::vector<collie::tf::Pipe<>> pipes;
  collie::tf::ScalablePipeline<typename decltype(pipes)::iterator> spl;
...
void ifelse_spipeline(size_t L, unsigned w) {
  ...
  std::vector<collie::tf::Pipe<>> pipes;
  collie::tf::ScalablePipeline<typename decltype(pipes)::iterator> pl;
  ...
  for(size_t N = 1; N < maxN; ++N) {
    ...
    // pipe 1
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [&, N](auto& pf){
        if(pf.token() == N) {
          pf.stop();
          return;
        }
      ...
      // pipe 2
    pipes.emplace_back(collie::tf::PipeType::PARALLEL, [&](auto& pf){
    ...
    // pipe 3
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [&](auto& pf){
...
void spipeline_in_spipeline(size_t L, unsigned w, unsigned subL) {
  ...
  for (size_t N = 1; N < maxN; ++N) {
    for(size_t subN = 1; subN < maxsubN; ++subN) {
      ...
      // begin of pipe 1 -----------------------------
      pipes.emplace_back(collie::tf::PipeType::SERIAL, [&, N, subN, subL](auto& pf) mutable {
        if(j1 == N) {
          pf.stop();
          return;
        }
        ...
        std::vector<collie::tf::Pipe<>> subpipes;
        collie::tf::ScalablePipeline<typename decltype(subpipes)::iterator> subpl;

        // subpipe 1
        subpipes.emplace_back(collie::tf::PipeType::SERIAL, [&, subN](auto& subpf) mutable {
            if(subj1 == subN) {
              subpf.stop();
              return;
            }
        ...
        // subpipe 2
        subpipes.emplace_back(collie::tf::PipeType::PARALLEL, [&, subN](auto& subpf) mutable {
            REQUIRE(subj2++ < subN);
            REQUIRE(subpf.token() % subL == subpf.line());
            REQUIRE(
      ...
      //begin of pipe 2 ---------------------------
      pipes.emplace_back(collie::tf::PipeType::PARALLEL, [&, subN, subL](auto& pf) mutable {
      ...
      // begin of pipe 4 ---------------------------
      pipes.emplace_back(collie::tf::PipeType::SERIAL, [&, subN](auto& pf) mutable {
        int res = std::accumulate(
          source[j4].begin(),
...
void snig_spipeline(size_t L, unsigned w) {
  ...
  for(auto&& NUM_DEVICE: NUM_DEVICES) {
    ...
    std::vector<collie::tf::Task> dev_ends(NUM_DEVICE);
    for(auto&& dev_end: dev_ends) {
      dev_end = taskflow.emplace([](){}).name("dev_end");
    }

    std::vector<collie::tf::Task> first_fetches(NUM_DEVICE);
    std::vector<collie::tf::Task> fetches(NUM_DEVICE);
    std::vector<std::vector<collie::tf::Pipe<>>> pipes(NUM_DEVICE);

    // for type
    using pipeline_it = std::vector<collie::tf::Pipe<>>::iterator;

    std::vector<collie::tf::Task> module_of_pipelines(NUM_DEVICE);

    std::vector<collie::tf::ScalablePipeline<pipeline_it>> pipelines(NUM_DEVICE);
    ...
    for(size_t dev = 0; dev < NUM_DEVICE; ++dev) {
      ...
      // pipe 2
      pipes[dev].emplace_back(
        collie::tf::PipeType::PARALLEL, [&, dev, BATCH_SIZE](auto& pf) mutable {
      ...
      // pipe 3
      pipes[dev].emplace_back(
        collie::tf::PipeType::SERIAL, [&, dev, BATCH_SIZE](auto& pf) mutable {
...
void spawn(
  ...
  std::vector<std::vector<collie::tf::Pipe<>>>& pipes,
  std::vector<collie::tf::ScalablePipeline<typename std::vector<collie::tf::Pipe<>>::iterator>>& spls,
  size_t& counter
) {

  // construct pipes
  for(size_t p = 0; p < NUM_PIPES; ++p) {
    pipes[r].emplace_back(collie::tf::PipeType::SERIAL, [&, maxN, r](collie::tf::Pipeflow& pf) mutable {
  ...
  if(r + 1 < NUM_RECURS) {
    auto spawn_t = sf.emplace([&, L, NUM_PIPES, NUM_RECURS, maxN, r](collie::tf::Subflow& sf2) mutable {
...
void subflow_spipeline(unsigned NUM_RECURS, unsigned w, size_t L) {
  ...
  collie::tf::Executor executor(w);
  collie::tf::Taskflow taskflow;
  std::vector<collie::tf::ScalablePipeline<typename std::vector<collie::tf::Pipe<>>::iterator>> spls(NUM_RECURS);
  std::vector<std::vector<collie::tf::Pipe<>>> pipes(NUM_RECURS);
  ...
  auto subflows = taskflow.emplace([&, L, NUM_PIPES, NUM_RECURS, maxN](collie::tf::Subflow& sf){
...
TEST_CASE("ScalablePipeline.move" * doctest::timeout(300)) {
  ...
  for(size_t i=0; i<N; i++) {
    pipes.emplace_back(collie::tf::PipeType::SERIAL, [&](collie::tf::Pipeflow& pf) {
      if (pf.token() == 5) {
        pf.stop();
      }
      else {
