...
namespace turbo::fmt_detail::dragonbox {
...
// Various subroutines using pow10 cache
    ...
    struct cache_accessor<float> {
        ...
        static uint64_t get_cached_power(int k) noexcept {
            TURBO_ASSERT(
                    (k >= float_info<float>::min_k && k <= float_info<float>::max_k) && "k is out of range");
    ...
    struct cache_accessor<double> {
        ...
        static uint128_fallback get_cached_power(int k) noexcept {
            TURBO_ASSERT(
                    (k >= float_info<double>::min_k && k <= float_info<double>::max_k) && "k is out of range");
...
namespace turbo {
    template<>
    struct formatter<fmt_detail::bigint> {
