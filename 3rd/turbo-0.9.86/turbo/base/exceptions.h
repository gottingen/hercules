...
#ifndef TURBO_BASE_EXCEPTION_H_
#define TURBO_BASE_EXCEPTION_H_
...
namespace turbo {
    ...
    // clang format off
    TURBO_DISABLE_GCC_WARNING(-Wnoexcept-type) /* NOLINT */
    // clang format on
    /// throw_exception
    ///
    ...
    namespace detail {
        ...
        struct throw_exception_arg_trivial_ {
            template<typename R>
            using apply = turbo::remove_cvref_t<R>;
        };
        ...
        using throw_exception_arg_ = //
                std::conditional_t<
                        ...
                        std::conditional_t<
                                std::is_trivially_copyable<turbo::remove_cvref_t<R>>::value,
    ...
    template<
            ...
            typename R = std::common_type_t<
                    decltype(TURBO_DECLVAL(Try &&)()),
                    decltype(TURBO_DECLVAL(Catch &&)(
                            TURBO_DECLVAL(E &), TURBO_DECLVAL(CatchA &&)...))>>
    TURBO_INLINE_VISIBILITY R catch_exception(Try &&t, Catch &&c, CatchA &&... a) {
    ...
    template<
            ...
            typename R = std::common_type_t<
                    decltype(TURBO_DECLVAL(Try &&)()),
                    decltype(TURBO_DECLVAL(Catch &&)(TURBO_DECLVAL(CatchA &&)...))>>
    TURBO_INLINE_VISIBILITY R
    ...
    namespace detail {
        ...
        constexpr bool exception_ptr_access_ct = true;

// 0 unknown, 1 true, -1 false
        extern std::atomic<int> exception_ptr_access_rt_cache_;

        TURBO_COLD bool exception_ptr_access_rt_v_() noexcept;

        TURBO_COLD bool exception_ptr_access_rt_() noexcept;
        ...
        inline bool exception_ptr_access_rt() noexcept {
            ...
            return TURBO_LIKELY(value) ? value > 0 : exception_ptr_access_rt_();
        }
    ...
    T *exception_ptr_get_object(std::exception_ptr const &ptr) noexcept {
        ...
        auto target = turbo::type_info_of<T>();
    ...
    public:
        ...
        template<
                ...
                typename = decltype(test_params_(
                        TURBO_DECLVAL(String const&).data(),
                        TURBO_DECLVAL(String const&).size()))>
