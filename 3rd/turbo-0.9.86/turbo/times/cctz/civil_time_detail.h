...
namespace turbo::time_internal::cctz {
    ...
    namespace detail {
        ...
        struct fields {
            constexpr fields(year_t year, month_t month, day_t day, hour_t hour,
        ...
        namespace impl {
            ...
            constexpr bool is_leap_year(year_t y) noexcept {
                return y % 4 == 0 && (y % 100 != 0 || y % 400 == 0);
            }

            constexpr int year_index(year_t y, month_t m) noexcept {
                const int yi = static_cast<int>((y + (m > 2)) % 400);
                return yi < 0 ? yi + 400 : yi;
            }

            constexpr int days_per_century(int yi) noexcept {
                return 36524 + (yi == 0 || yi > 300);
            }

            constexpr int days_per_4years(int yi) noexcept {
                return 1460 + (yi == 0 || yi > 300 || (yi - 1) % 100 < 96);
            }

            constexpr int days_per_year(year_t y, month_t m) noexcept {
                return is_leap_year(y + (m > 2)) ? 366 : 365;
            }

            constexpr int days_per_month(year_t y, month_t m) noexcept {
                constexpr int k_days_per_month[1 + 12] = {
                        -1, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31  // non leap year
                };
                return k_days_per_month[m] + (m == 2 && is_leap_year(y));
            }

            constexpr fields n_day(year_t y, month_t m, diff_t d, diff_t cd, hour_t hh,
            ...
            constexpr fields n_mon(year_t y, diff_t m, diff_t d, diff_t cd, hour_t hh,
                                   minute_t mm, second_t ss) noexcept {
                if (m != 12) {
                    y += m / 12;
                    m %= 12;
            ...
            constexpr fields n_hour(year_t y, diff_t m, diff_t d, diff_t cd, diff_t hh,
                                    minute_t mm, second_t ss) noexcept {
                cd += hh / 24;
                hh %= 24;
                if (hh < 0) {
            ...
            constexpr fields n_min(year_t y, diff_t m, diff_t d, diff_t hh, diff_t ch,
            ...
            constexpr fields n_sec(year_t y, diff_t m, diff_t d, diff_t hh, diff_t mm,
                                   diff_t ss) noexcept {
        ...
        constexpr fields step(minute_tag, fields f, diff_t n) noexcept {
            return impl::n_min(f.y, f.m, f.d, f.hh + n / 60, 0, f.mm + n % 60, f.ss);
        }

        constexpr fields step(hour_tag, fields f, diff_t n) noexcept {
            return impl::n_hour(f.y, f.m, f.d + n / 24, 0, f.hh + n % 24, f.mm, f.ss);
        }

        constexpr fields step(day_tag, fields f, diff_t n) noexcept {
            return impl::n_day(f.y, f.m, f.d, n, f.hh, f.mm, f.ss);
        }

        constexpr fields step(month_tag, fields f, diff_t n) noexcept {
            return impl::n_mon(f.y + n / 12, f.m + n % 12, f.d, 0, f.hh, f.mm, f.ss);
        }

        constexpr fields step(year_tag, fields f, diff_t n) noexcept {
        ...
        constexpr diff_t difference(month_tag, fields f1, fields f2) noexcept {
            return impl::scale_add(difference(year_tag{}, f1, f2), 12, (f1.m - f2.m));
        }

        constexpr diff_t difference(day_tag, fields f1, fields f2) noexcept {
            return impl::day_difference(f1.y, f1.m, f1.d, f2.y, f2.m, f2.d);
        }

        constexpr diff_t difference(hour_tag, fields f1, fields f2) noexcept {
            return impl::scale_add(difference(day_tag{}, f1, f2), 24, (f1.hh - f2.hh));
        }

        constexpr diff_t difference(minute_tag, fields f1, fields f2) noexcept {
            return impl::scale_add(difference(hour_tag{}, f1, f2), 60, (f1.mm - f2.mm));
        }

        constexpr diff_t difference(second_tag, fields f1, fields f2) noexcept {
        ...
        constexpr fields align(hour_tag, fields f) noexcept {
            return fields{f.y, f.m, f.d, f.hh, 0, 0};
        }

        constexpr fields align(day_tag, fields f) noexcept {
            return fields{f.y, f.m, f.d, 0, 0, 0};
        }

        constexpr fields align(month_tag, fields f) noexcept {
            return fields{f.y, f.m, 1, 0, 0, 0};
        }

        constexpr fields align(year_tag, fields f) noexcept {
...
////////////////////////////////////////////////////////////////////////
        ...
        class civil_time {
        public:
            explicit constexpr civil_time(year_t y, diff_t m = 1, diff_t d = 1,
                                          diff_t hh = 0, diff_t mm = 0,
                                          diff_t ss = 0) noexcept
                    : civil_time(impl::n_sec(y, m, d, hh, mm, ss)) {}

            constexpr civil_time() noexcept: f_{1970, 1, 1, 0, 0, 0} {}
            ...
            static constexpr civil_time(max)() {
                const auto max_year = (std::numeric_limits<std::int_least64_t>::max)();
                return civil_time(max_year, 12, 31, 23, 59, 59);
            }

            static constexpr civil_time(min)() {
                const auto min_year = (std::numeric_limits<std::int_least64_t>::min)();
                return civil_time(min_year, 1, 1, 0, 0, 0);
            }

            // Field accessors.  Note: All but year() return an int.
            constexpr year_t year() const noexcept { return f_.y; }

            constexpr int month() const noexcept { return f_.m; }

            constexpr int day() const noexcept { return f_.d; }

            constexpr int hour() const noexcept { return f_.hh; }

            constexpr int minute() const noexcept { return f_.mm; }

            constexpr int second() const noexcept { return f_.ss; }

            // Assigning arithmetic.
            constexpr civil_time &operator+=(diff_t n) noexcept {
                return *this = *this + n;
            }
            ...
            constexpr civil_time &operator++() noexcept { return *this += 1; }

            constexpr civil_time operator++(int) noexcept {
            ...
            friend constexpr civil_time operator+(civil_time a, diff_t n) noexcept {
                return civil_time(step(T{}, a.f_, n));
            }

            friend constexpr civil_time operator+(diff_t n, civil_time a) noexcept {
                return a + n;
            }
            ...
            friend constexpr diff_t operator-(civil_time lhs, civil_time rhs) noexcept {
        ...
        private:
            ...
            explicit constexpr civil_time(fields f) noexcept: f_(align(T{}, f)) {}
...
////////////////////////////////////////////////////////////////////////
        ...
        constexpr weekday get_weekday(const civil_second &cs) noexcept {
...
////////////////////////////////////////////////////////////////////////
        ...
        constexpr civil_day next_weekday(civil_day cd, weekday wd) noexcept {
            constexpr weekday k_weekdays_forw[14] = {
                    weekday::monday, weekday::tuesday, weekday::wednesday,
                    weekday::thursday, weekday::friday, weekday::saturday,
                    weekday::sunday, weekday::monday, weekday::tuesday,
                    weekday::wednesday, weekday::thursday, weekday::friday,
        ...
        constexpr civil_day prev_weekday(civil_day cd, weekday wd) noexcept {
            constexpr weekday k_weekdays_back[14] = {
        ...
        constexpr int get_yearday(const civil_second &cs) noexcept {
...
}  // namespace turbo::time_internal::cctz::detail
