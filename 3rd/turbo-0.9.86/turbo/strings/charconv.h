...
namespace turbo {
     ...
     * @brief chars_format is an bitfield enumerator which can be passed to turbo::from_chars to
    ...
    /**
     * @ingroup turbo_strings_convert
     * @brief from_chars_result is the result of a string-to-number conversion.
    ...
    /**
     * @ingroup turbo_strings_convert
     * @brief from_chars convert string to number.
     *        This function is a workalike compatibilty version of std::from_chars from C++17.
     *        Currently this only supports the `double` and `float` types.
     *
     *        This interface incorporates the proposed resolutions for library issues
     *        DR 3080 and DR 3081.  If these are adopted with different wording,
     *        Turbo's behavior will change to match the standard.  (The behavior most
     ...
     *        matching pattern "0".)
     *
     *        If a matching pattern is found, the function returns {last, std::errc()}.
     *        Otherwise, it returns {first, std::errc::invalid_argument}.
     *
     *        If a matching pattern is found, but the value is out of range for the
     *        requested type, the function returns {last, std::errc::result_out_of_range}.
     *        In this case, `value` is set to the appropriate signed or unsigned
     *        infinity, or to the largest finite value representable by the requested
     *        type, depending on the sign of the value.
     * @param first [input] the first character of the string to convert.
     * @param last [input] one past the last character of the string to convert.
     * @param value [output] the converted value.
     * @param fmt [input] the format of the string to convert.
