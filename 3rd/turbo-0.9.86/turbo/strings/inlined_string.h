...
#define INLINED_STRING_DISABLE_SSO true
#else
#define INLINED_STRING_DISABLE_SSO false
#endif
    ...
    namespace strings_internal {
        ...
        copy_n(InIt b, typename std::iterator_traits<InIt>::difference_type n,
               OutIt d) {
            for (; n != 0; --n, ++b, ++d) {
                *d = *b;
            }
    ...
    /**
     * Defines a special acquisition method for constructing inlined_string
     * objects. AcquireMallocatedString means that the user passes a
     * pointer to a malloc-allocated string that the inlined_string object will
     * take into custody.
     */
     ...
     * implement the primitives. Then plug the core into basic_inlined_string as
     * a template argument.

    template <class Char>
    class inlined_string_core_model {
     public:
    ...
    */
    ...
    public:
        ...
        inlined_string_core(const inlined_string_core &rhs) {
            assert(&rhs != this);
            switch (rhs.category()) {
        ...
        inlined_string_core &operator=(const inlined_string_core &rhs) = delete;

        inlined_string_core(inlined_string_core &&goner) noexcept {
            // Take goner's guts
            ml_ = goner.ml_;
        ...
        // worth.
        void swap(inlined_string_core &rhs) {
            auto const t = ml_;
            ml_ = rhs.ml_;
            rhs.ml_ = t;
        }
        ...
        TURBO_NO_INLINE void destroyMediumLarge() noexcept {
        ...
        struct RefCounted {
            ...
            static RefCounted *create(size_t *size) {
                ...
                if (!turbo::checked_add(&capacityBytes, *size, size_t(1))) {
                    turbo::base_internal::ThrowStdLengthError("");
                }
                if (!turbo::checked_muladd(&capacityBytes, capacityBytes, sizeof(Char),
                                           getDataOffset())) {
                    turbo::base_internal::ThrowStdLengthError("");
                }
            ...
            static RefCounted *create(const Char *data, size_t *size) {
                ...
                if (TURBO_LIKELY(effectiveSize > 0)) {
                    strings_internal::podCopy(data, data + effectiveSize, result->data_);
                }
                return result;
            }

            static RefCounted *reallocate(Char *const data, const size_t currentSize,
                                          const size_t currentCapacity,
                                          size_t *newCapacity) {
                assert(*newCapacity > 0 && *newCapacity > currentSize);
                size_t capacityBytes;
                if (!turbo::checked_add(&capacityBytes, *newCapacity, size_t(1))) {
                    turbo::base_internal::ThrowStdLengthError("");
                }
                if (!turbo::checked_muladd(&capacityBytes, capacityBytes, sizeof(Char),
                                           getDataOffset())) {
    ...
    public:
        ...
        static_assert(!(sizeof(MediumLarge) % sizeof(Char)),
                      "Corrupt memory layout for inlined_string.");
        ...
        void copySmall(const inlined_string_core &);

        void copyMedium(const inlined_string_core &);

        void copyLarge(const inlined_string_core &);
...
// Small strings are bitblitted
    ...
    inline void inlined_string_core<Char>::initSmall(const Char *const data,
        ...
        static_assert(sizeof(Char *) == sizeof(size_t),
                      "inlined_string size assumption violation");
...
// compiling with ASan.
#ifndef TURBO_HAVE_ADDRESS_SANITIZER
        ...
        {
            if (size != 0) {
                strings_internal::podCopy(data, data + size, small_);
            }
        }
        setSmallSize(size);
    }
    ...
    inlined_string_core<Char>::initMedium(const Char *const data,
        ...
        ml_.data_ = static_cast<Char *>(checkedMalloc(allocSize));
        if (TURBO_LIKELY(size > 0)) {
            strings_internal::podCopy(data, data + size, ml_.data_);
        }
        ml_.size_ = size;
        ml_.setCapacity(allocSize / sizeof(Char) - 1, Category::isMedium);
        ml_.data_[size] = '\0';
    ...
    template<class Char>
    TURBO_NO_INLINE void inlined_string_core<Char>::unshare(size_t minCapacity) {
        ...
        // Also copies terminator.
        strings_internal::podCopy(ml_.data_, ml_.data_ + ml_.size_ + 1, newRC->data_);
        RefCounted::decrementRefs(ml_.data_);
        ml_.data_ = newRC->data_;
        ml_.setCapacity(effectiveCapacity, Category::isLarge);
    ...
    template<class Char>
    TURBO_NO_INLINE void
    inlined_string_core<Char>::reserveMedium(const size_t minCapacity) {
        assert(category() == Category::isMedium);
    ...
    inlined_string_core<Char>::reserveSmall(size_t minCapacity,
        ...
        if (!disableSSO && minCapacity <= maxSmallSize) {
            ...
            // Also copies terminator.
            strings_internal::podCopy(small_, small_ + size + 1, pData);
            ml_.data_ = pData;
            ml_.size_ = size;
            ...
            auto const size = smallSize();
            // Also copies terminator.
            strings_internal::podCopy(small_, small_ + size + 1, newRC->data_);
            ml_.data_ = newRC->data_;
            ml_.size_ = size;
            ml_.setCapacity(minCapacity, Category::isLarge);
            assert(capacity() >= minCapacity);
    ...
    template<class Char>
    inline Char *inlined_string_core<Char>::expandNoinit(
            const size_t delta, bool expGrowth, /* = false */
            bool disableSSO /* = INLINED_STRING_DISABLE_SSO */) {
        ...
        if (category() == Category::isSmall) {
            sz = smallSize();
            newSz = sz + delta;
            if (!disableSSO && TURBO_LIKELY(newSz <= maxSmallSize)) {
                setSmallSize(newSz);
                return small_ + sz;
            }
            ...
            sz = ml_.size_;
            newSz = sz + delta;
            if (TURBO_UNLIKELY(newSz > capacity())) {
                // ensures not shared
    ...
    template<class Char>
    inline void inlined_string_core<Char>::shrinkSmall(const size_t delta) {
        // Check for underflow
    ...
    template<class Char>
    inline void inlined_string_core<Char>::shrinkMedium(const size_t delta) {
    ...
    template<class Char>
    inline void inlined_string_core<Char>::shrinkLarge(const size_t delta) {
        assert(ml_.size_ >= delta);
        // Shared large string, must make unique. This is because of the
        // durn terminator must be written, which may trample the shared
        // data.
        if (delta) {
            inlined_string_core(ml_.data_, ml_.size_ - delta).swap(*this);
        }
        // No need to write the terminator.
    }

/**
 * Dummy inlined_string core that uses an actual std::string. This doesn't
 * make any sense - it's just for testing purposes.
 */
    template<class Char>
    class dummy_inlined_string_core {
    public:
        dummy_inlined_string_core() {}

        dummy_inlined_string_core(const dummy_inlined_string_core &another)
                : backend_(another.backend_) {}

        dummy_inlined_string_core(const Char *s, size_t n) : backend_(s, n) {}

        void swap(dummy_inlined_string_core &rhs) { backend_.swap(rhs.backend_); }
 ...
 * basic_inlined_string takes the same template parameters, plus the last
 * one which is the core.
 */
    template<typename E, class T = std::char_traits<E>,
            class A = std::allocator<E>, class Storage = inlined_string_core<E>>
    class basic_inlined_string {
        static_assert(std::is_same<A, std::allocator<E>>::value,
                      "inlined_string ignores custom allocators");
        ...
        struct Invariant {
            ...
            explicit Invariant(const basic_inlined_string &s) noexcept: s_(s) {
                assert(s_.isSane());
            }
    ...
    public:
        ...
        //
        //   explicit basic_inlined_string(const A& a = A()) noexcept { }
        //
        ...
        basic_inlined_string() noexcept: basic_inlined_string(A()) {}

        explicit basic_inlined_string(const A &) noexcept {}

        basic_inlined_string(const basic_inlined_string &str) : store_(str.store_) {}

        // Move constructor
        basic_inlined_string(basic_inlined_string &&goner) noexcept
        ...
        TURBO_NO_INLINE
        basic_inlined_string(const value_type *s, size_type n, const A & /*a*/ = A())
                : store_(s, n) {}

        TURBO_NO_INLINE
        basic_inlined_string(size_type n, value_type c, const A & /*a*/ = A()) {
            auto const pData = store_.expandNoinit(n);
            strings_internal::podFill(pData, pData + n, c);
        }

        template<class InIt>
        TURBO_NO_INLINE basic_inlined_string(
                InIt begin, InIt end,
        ...
        TURBO_NO_INLINE
        basic_inlined_string(std::initializer_list<value_type> il) {
            assign(il.begin(), il.end());
        }

        ~basic_inlined_string() noexcept {}

        basic_inlined_string &operator=(const basic_inlined_string &lhs);

        // Move assignment
        basic_inlined_string &operator=(basic_inlined_string &&goner) noexcept;

        // Compatibility with std::string
        template<typename A2>
        basic_inlined_string &operator=(const std::basic_string<E, T, A2> &rhs) {
        ...
        basic_inlined_string &operator=(const value_type *s) { return assign(s); }

        basic_inlined_string &operator=(value_type c);
        ...
        basic_inlined_string &operator=(std::initializer_list<value_type> il) {
        ...
        void shrink_to_fit() {
            ...
            basic_inlined_string(cbegin(), cend()).swap(*this);
        }
        ...
        basic_inlined_string &operator+=(const value_type c) {
            push_back(c);
            return *this;
        }
        ...
        basic_inlined_string &append(const value_type *s) {
            return append(s, traitsLength(s));
        }

        basic_inlined_string &append(size_type n, value_type c);

        template<class InputIterator>
        basic_inlined_string &append(InputIterator first, InputIterator last) {
        ...
        basic_inlined_string &assign(basic_inlined_string &&str) {
        ...
        basic_inlined_string &assign(const value_type *s) {
            return assign(s, traitsLength(s));
        }

        basic_inlined_string &assign(std::initializer_list<value_type> il) {
            return assign(il.begin(), il.end());
        }

        template<class ItOrLength, class ItOrChar>
        basic_inlined_string &assign(ItOrLength first_or_n, ItOrChar last_or_c) {
        ...
        basic_inlined_string &insert(size_type pos, size_type n, value_type c) {
            enforce<std::out_of_range>(pos <= length(), "");
    ...
    private:
        iterator insertImplDiscr(const_iterator i, size_type n, value_type c,
                                 std::true_type);

        template<class InputIter>
        iterator insertImplDiscr(const_iterator i, InputIter b, InputIter e,
                                 std::false_type);

        template<class FwdIterator>
        iterator insertImpl(const_iterator i, FwdIterator s1, FwdIterator s2,
                            std::forward_iterator_tag);

        template<class InputIterator>
        iterator insertImpl(const_iterator i, InputIterator b, InputIterator e,
    ...
    public:
        ...
        basic_inlined_string &erase(size_type pos = 0, size_type n = npos) {
        ...
        basic_inlined_string &replace(iterator i1, iterator i2, const value_type *s) {
    ...
    public:
        ...
        size_type copy(value_type *s, size_type n, size_type pos = 0) const {
            ...
            if (n != 0) {
                strings_internal::podCopy(data() + pos, data() + pos + n, s);
            }
            return n;
        }

        void swap(basic_inlined_string &rhs) { store_.swap(rhs.store_); }
        ...
        size_type find(const basic_inlined_string &str, size_type pos = 0) const {
            return find(str.data(), pos, str.length());
        }

        size_type find(const value_type *needle, size_type pos,
        ...
        size_type rfind(const basic_inlined_string &str, size_type pos = npos) const {
        ...
        size_type find_last_of(const basic_inlined_string &str,
        ...
        size_type find_first_not_of(const basic_inlined_string &str,
                                    size_type pos = 0) const {
        ...
        size_type find_last_not_of(const basic_inlined_string &str,
                                   size_type pos = npos) const {
        ...
        basic_inlined_string substr(size_type pos = 0, size_type n = npos) const &{
            enforce<std::out_of_range>(pos <= size(), "");
            return basic_inlined_string(data() + pos, std::min(n, size() - pos));
        }

        basic_inlined_string substr(size_type pos = 0, size_type n = npos) &&{
            enforce<std::out_of_range>(pos <= size(), "");
            erase(0, pos);
            if (n < size()) {
                resize(n);
        ...
        int compare(const basic_inlined_string &str) const {
        ...
        int compare(size_type pos1, size_type n1, const basic_inlined_string &str,
    ...
    TURBO_NO_INLINE typename basic_inlined_string<E, T, A, S>::size_type
    basic_inlined_string<E, T, A, S>::traitsLength(const value_type *s) {
        return s ? traits_type::length(s)
                 : (turbo::base_internal::ThrowStdLogicError(
                        "basic_InlinedString: null pointer initializer not valid"),
                        0);
    }

    template<typename E, class T, class A, class S>
    inline basic_inlined_string<E, T, A, S> &
    basic_inlined_string<E, T, A, S>::operator=(const basic_inlined_string &lhs) {
        Invariant checker(*this);
...
// Move assignment
    ...
    template<typename E, class T, class A, class S>
    inline basic_inlined_string<E, T, A, S> &
    basic_inlined_string<E, T, A, S>::operator=(value_type c) {
        Invariant checker(*this);

        if (empty()) {
            store_.expandNoinit(1);
        } else if (store_.isShared()) {
            basic_inlined_string(1, c).swap(*this);
            return *this;
        } else {
    ...
    basic_inlined_string<E, T, A, S>::append(const value_type *s, size_type n) {
        Invariant checker(*this);
    ...
    template<typename E, class T, class A, class S>
    inline basic_inlined_string<E, T, A, S> &
    basic_inlined_string<E, T, A, S>::append(size_type n, value_type c) {
        Invariant checker(*this);
        auto pData = store_.expandNoinit(n, /* expGrowth = */ true);
        strings_internal::podFill(pData, pData + n, c);
        return *this;
    ...
    basic_inlined_string<E, T, A, S>::assign(const value_type *s,
        ...
        if (n == 0) {
            ...
            strings_internal::podMove(s, s + n, store_.mutableData());
            store_.shrink(size() - n);
            assert(size() == n);
        } else {
            ...
            strings_internal::podCopy(s, s + n, store_.expandNoinit(n));
        }
    ...
    basic_inlined_string<E, T, A, S>::getlineImpl(istream_type &is,
        ...
        while (true) {
            ...
            // inlined_string has 1 byte extra capacity for the null terminator,
    ...
    basic_inlined_string<E, T, A, S>::replaceImpl(iterator i1, iterator i2,
        ...
        if (n1 > n2) {
            ...
            // grows
            s1 = strings_internal::copy_n(s1, n1, i1).first;
            insert(i2, s1, s2);
        }
        assert(isSane());
    }
    ...
    operator+(const basic_inlined_string<E, T, A, S> &lhs,
              const basic_inlined_string<E, T, A, S> &rhs) {
        basic_inlined_string <E, T, A, S> result;
...
// C++11 21.4.8.1/9
    template<typename E, class T, class A, class S>
    inline basic_inlined_string<E, T, A, S>
    operator+(const basic_inlined_string<E, T, A, S> &lhs, const E *rhs) {
        typedef typename basic_inlined_string<E, T, A, S>::size_type size_type;
        typedef typename basic_inlined_string<E, T, A, S>::traits_type traits_type;
    ...
    template<typename E, class T, class A, class S>
    inline bool operator==(const basic_inlined_string<E, T, A, S> &lhs,
                           const basic_inlined_string<E, T, A, S> &rhs) {
    ...
    template<typename E, class T, class A, class S>
    inline bool
    operator==(const typename basic_inlined_string<E, T, A, S>::value_type *lhs,
               const basic_inlined_string<E, T, A, S> &rhs) {
        return rhs == lhs;
    }
    ...
    inline bool
    operator==(const basic_inlined_string<E, T, A, S> &lhs,
               const typename basic_inlined_string<E, T, A, S>::value_type *rhs) {
...
// TODO: make this faster.
    ...
    constexpr typename basic_inlined_string<E1, T, A, S>::size_type
            basic_inlined_string<E1, T, A, S>::npos;
