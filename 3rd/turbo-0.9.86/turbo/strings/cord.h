...
namespace turbo {
    ...
    public:
        ...
        //   * The `std::string_view` returned by dereferencing a valid, non-`end()`
        //     iterator is guaranteed to be non-empty.
        //   * Two `ChunkIterator` objects can be compared equal if and only if they
        //     remain valid and iterate over the same Cord.
        //   * The iterator in this case is a proxy iterator; the `std::string_view`
        ...
        private:
            ...
            turbo::cord_internal::CordRep *current_leaf_ = nullptr;
        ...
        static Cord advance_and_read(CharIterator *it, size_t n_bytes);
    ...
    private:
        ...
        public:
            ...
            turbo::cord_internal::CordRep *tree() const;

            turbo::cord_internal::CordRep *as_tree() const;

            const char *as_chars() const;

            // Returns non-null iff was holding a pointer
            turbo::cord_internal::CordRep *clear();
            ...
            void CopyTo(std::string *dst) const {
                ...
                turbo::strings_internal::STLStringResizeUninitialized(dst, kMaxInline);
            ...
            turbo::cord_internal::CordzInfo *cordz_info() const {
        ...
        turbo::cord_internal::CordRep *TakeRep() const &;

        turbo::cord_internal::CordRep *TakeRep() &&;
...
namespace turbo {
    ...
    inline turbo::cord_internal::CordRep *Cord::InlineRep::as_tree() const {
        assert(data_.is_tree());
        return data_.as_tree();
    }

    inline turbo::cord_internal::CordRep *Cord::InlineRep::tree() const {
        if (is_tree()) {
            return as_tree();
        } else {
            return nullptr;
    ...
    inline turbo::cord_internal::CordRep *Cord::InlineRep::clear() {
        if (is_tree()) {
    ...
    inline Cord Cord::advance_and_read(CharIterator *it, size_t n_bytes) {
        assert(it != nullptr);
