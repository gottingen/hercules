...
// Comparators
    ...
    namespace {
        ...
        int CompareChunks(std::string_view *lhs, std::string_view *rhs,
...
// initializing the iterator, and returns a std::string_view referencing the data.
    ...
    inline int Cord::CompareSlowPath(std::string_view rhs, size_t compared_size,
        ...
        auto advance = [](Cord::ChunkIterator *it, std::string_view *chunk) {
...
/* static */ bool Cord::GetFlatAux(CordRep *rep, std::string_view *fragment) {
        assert(rep != nullptr);
        if (rep->length == 0) {
...
/* static */ void Cord::ForEachChunkAux(
    ...
    static void DumpNode(CordRep *rep, bool include_data, std::ostream *os,
        ...
        turbo::InlinedVector<CordRep *, kInlinedVectorSize> stack;
        turbo::InlinedVector<int, kInlinedVectorSize> indents;
        for (;;) {
    ...
    static bool VerifyNode(CordRep *root, CordRep *start_node,
                           bool /* full_validation */) {
        turbo::InlinedVector<CordRep *, 2> worklist;
