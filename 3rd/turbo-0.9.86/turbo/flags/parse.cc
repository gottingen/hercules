...
.on_update([]() {
    ...
    if (turbo::flags_internal::flagfile_needs_processing) {
        TURBO_RAW_LOG(WARNING, "flagfile set twice before it is handled");
    }
...
.on_update([]() {
    ...
    if (turbo::flags_internal::fromenv_needs_processing) {
        TURBO_RAW_LOG(WARNING, "fromenv set twice before it is handled.");
    }
            ...
            bool ArgsList::ReadFromFlagfile(const std::string &flag_file_name) {
                ...
                if (!flag_file) {
                    flags_internal::ReportUsageError(
                            turbo::format("Can't open flagfile {}", flag_file_name), true);
                ...
                while (std::getline(flag_file, line)) {
                    ...
                    flags_internal::ReportUsageError(
                            turbo::format("Unexpected line in the flagfile {}: {}", flag_file_name, line),
                            true);
...
#undef TURBO_FLAGS_INTERNAL_IGNORE_TYPE
            ...
            bool ReadFlagsFromEnv(const std::vector<std::string> &flag_names,
                ...
                for (const auto &flag_name: flag_names) {
                    ...
                    if (flag_name == "fromenv" || flag_name == "tryfromenv") {
                        flags_internal::ReportUsageError(
                                turbo::format("Infinite recursion on flag {}", flag_name), true);

                        success = false;
                        continue;
                    }

                    const std::string envname = turbo::format("FLAGS_{}", flag_name);
                    std::string envval;
                    if (!GetEnvVar(envname.c_str(), envval)) {
                        if (fail_on_absent_in_env) {
                            flags_internal::ReportUsageError(
                                    turbo::format("{} not found in environment", envname), true);
                    ...
                    args.push_back(turbo::format("--{}={}", flag_name, envval));
                }
            ...
            bool HandleGeneratorFlags(std::vector<ArgsList> &input_args,
                ...
                std::unique_lock l(flags_internal::processing_checks_guard);
            ...
            std::tuple<bool, std::string_view> DeduceFlagValue(const CommandLineFlag &flag,
                ...
                if (flag.is_of_type<bool>()) {
                    if (value.empty()) {
                        ...
                        // "--nobool_flag=Y" case
                        flags_internal::ReportUsageError(
                                turbo::format("Negative form with assignment is not valid for the "
                                              "boolean flag '{}'",
                                              flag.name()),
                                true);
                        return std::make_tuple(false, "");
                    ...
                    // "--noint_flag=1" case
                    flags_internal::ReportUsageError(
                            turbo::format("Negative form is not valid for the flag '{}'", flag.name()),
                            true);
                    return std::make_tuple(false, "");
                    ...
                    if (curr_list->Size() == 1) {
                        // "--int_flag" case
                        flags_internal::ReportUsageError(
                                turbo::format("Missing the value for the flag '{}'", flag.name()),
                                true);
                        return std::make_tuple(false, "");
                    }
                    ...
                    if (!value.empty() && value[0] == '-' && flag.is_of_type<std::string>()) {
                        ...
                        if (maybe_flag_name.empty() ||
                            ...
                            TURBO_RAW_LOG(
                                    WARNING, "%s",
                                    turbo::format("Did you really mean to set flag '{}' to the value '{}'?",
            ...
            void report_unrecognized_flags(
                ...
                for (const auto &unrecognized: unrecognized_flags) {
                    ...
                    if (misspelling_hints.empty()) {
                        flags_internal::ReportUsageError(
                                turbo::format("Unknown command line flag '{}'", unrecognized.flag_name),
                                report_as_fatal_error);
                    } else {
                        flags_internal::ReportUsageError(
                                turbo::format("Unknown command line flag '{}'. Did you mean: {}?",
                                              unrecognized.flag_name,
                                              turbo::format_range("{}", misspelling_hints,", ")),
                                report_as_fatal_error);
                    }
                }
            }
...
// --------------------------------------------------------------------
        ...
        bool WasPresentOnCommandLine(std::string_view flag_name) {
            std::unique_lock l(specified_flags_guard);
            TURBO_RAW_CHECK(specified_flags != nullptr,
...
// --------------------------------------------------------------------
        ...
        std::vector<std::string> GetMisspellingHints(const std::string_view flag) {
            ...
            flags_internal::ForEachFlag([&](const CommandLineFlag &f) {
                ...
                if (f.is_of_type<bool>()) {
                    const std::string negated_flag = turbo::format("no{}", f.name());
            ...
            turbo::c_for_each(undefok, [&](const std::string_view f) {
                ...
                best_hints.AddHint(turbo::format("{} (undefok)", f), distance);
            });
        ...
        std::vector<char *> ParseCommandLineImpl(int argc, char *argv[],
            ...
            auto help_mode = flags_internal::ParseTurbFlagsOnlyImpl(
        ...
        HelpMode ParseTurbFlagsOnlyImpl(
                int argc, char *argv[], std::vector<char *> &positional_args,
                std::vector<UnrecognizedFlag> &unrecognized_flags,
                UsageFlagsAction usage_flag_action) {
            TURBO_RAW_CHECK(argc > 0, "Missing argv[0]");
            ...
            if (flags_internal::program_invocation_name() == "UNKNOWN") {
                flags_internal::set_program_invocation_name(argv[0]);
            }
            positional_args.push_back(argv[0]);

            std::unique_lock l(flags_internal::specified_flags_guard);
