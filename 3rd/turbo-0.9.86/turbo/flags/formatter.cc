...
namespace turbo {
    ...
    std::string Formatter::make_usage(const App *app, std::string name) const {
        ...
        if (!app->get_subcommands(
                        [](const turbo::App *subc) { return ((!subc->get_disabled()) && (!subc->get_name().empty())); })
                .empty()) {
    ...
    std::string Formatter::make_option_desc(const Option *opt) const { return opt->get_description(); }
