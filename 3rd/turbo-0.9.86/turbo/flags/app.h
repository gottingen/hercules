...
#ifndef TURBO_FLAGS_PARSE
#define TURBO_FLAGS_PARSE(app, argc, argv)                                                                                   \
    try {                                                                                                              \
        (app).parse((argc), (argv));                                                                                   \
    ...
    public:
        ...
        Option *add_option(std::string option_name,
            ...
            auto fun = [&variable](const turbo::results_t &res) {  // comment for spacing
                return detail::lexical_conversion<AssignTo, ConvertTo>(res, variable);
            };

            Option *opt = add_option(option_name, fun, option_description, false, [&variable]() {
                return turbo::detail::checked_to_string<AssignTo, ConvertTo>(variable);
            });
        ...
        Option *add_option_function(std::string option_name,
            ...
            auto fun = [func](const turbo::results_t &res) {
                ArgType variable;
        ...
        Option *add_option(std::string option_name) {
            return add_option(option_name, turbo::callback_t{}, std::string{}, false);
        }
    ...
    private:
        ...
        Option *_add_flag_internal(std::string flag_name, turbo::callback_t fun, std::string flag_description);

    public:
        /// Add a flag with no description or variable assignment
        Option *add_flag(std::string flag_name) {
            return _add_flag_internal(flag_name, turbo::callback_t(), std::string{});
        }
        ...
        Option *add_flag(std::string flag_name,
            ...
            turbo::callback_t fun = [&flag_result](const turbo::results_t &res) {
        ...
        Option *add_flag(std::string flag_name,
                         ...
                         std::string flag_description = "") {
            turbo::callback_t fun = [&flag_results](const turbo::results_t &res) {
                bool retval = true;
        ...
        template<typename T = OptionGroup>
        ...
        App *add_subcommand(turbo::App_p subcom);
        ...
        turbo::App_p get_subcommand_ptr(App *subcom) const;

        /// Check to see if a subcommand is part of this command (text version)
        [[nodiscard]] turbo::App_p get_subcommand_ptr(std::string subcom) const;

        /// Get an owning pointer to subcommand by index
        [[nodiscard]] turbo::App_p get_subcommand_ptr(int index = 0) const;
    ...
    public:
        ...
        /// Add an existing option to the OptionGroup
