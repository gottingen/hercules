...
namespace turbo {
    ...
    App::set_version_flag(std::string flag_name, const std::string &versionString, const std::string &version_help) {
        ...
        if (!flag_name.empty()) {
            version_ptr_ = add_flag_callback(
                    flag_name, [versionString]() { throw (turbo::CallForVersion(versionString, 0)); }, version_help);
    ...
    App::set_version_flag(std::string flag_name, std::function<std::string()> vfunc, const std::string &version_help) {
        ...
        if (!flag_name.empty()) {
            version_ptr_ = add_flag_callback(
                    flag_name, [vfunc]() { throw (turbo::CallForVersion(vfunc(), 0)); }, version_help);
            version_ptr_->configurable(false);
        }
    ...
    Option *App::_add_flag_internal(std::string flag_name, turbo::callback_t fun, std::string flag_description) {
        Option *opt = nullptr;
    ...
    Option *App::add_flag_callback(std::string flag_name,
        ...
        turbo::callback_t fun = [function](const turbo::results_t &res) {
    ...
    App *App::add_subcommand(std::string subcommand_name, std::string subcommand_description) {
        ...
        turbo::App_p subcom = std::shared_ptr<App>(new App(std::move(subcommand_description), subcommand_name, this));
        return add_subcommand(std::move(subcom));
    }

    App *App::add_subcommand(turbo::App_p subcom) {
        if (!subcom)
    ...
    turbo::App_p App::get_subcommand_ptr(App *subcom) const {
        if (subcom == nullptr)
            throw OptionNotFound("nullptr passed");
        for (const App_p &subcomptr: subcommands_)
            if (subcomptr.get() == subcom)
    ...
    [[nodiscard]] turbo::App_p App::get_subcommand_ptr(std::string subcom) const {
        for (const App_p &subcomptr: subcommands_)
    ...
    [[nodiscard]] turbo::App_p App::get_subcommand_ptr(int index) const {
        if (index >= 0) {
            auto uindex = static_cast<unsigned>(index);
            if (uindex < subcommands_.size())
    ...
    [[nodiscard]] turbo::App *App::get_option_group(std::string group_name) const {
    ...
    int App::exit(const Error &e, std::ostream &out, std::ostream &err) const {
        ...
        /// Avoid printing anything if this is a turbo::RuntimeError
    ...
    [[nodiscard]] std::string App::version() const {
        ...
        if (version_ptr_ != nullptr) {
            ...
            } catch (const turbo::CallForVersion &cfv) {
                val = cfv.what();
            }
            version_ptr_->clear();
    ...
    Option *App::get_option_no_throw(std::string option_name) noexcept {
    ...
    void App::_process_requirements() {
        ...
        for (App_p &sub: subcommands_) {
            ...
            if (sub->required_ && sub->count_all() == 0) {
                throw (turbo::RequiredError(sub->get_display_name()));
            }
        }
    }
    ...
    bool App::_parse_positional(std::vector<std::string> &args, bool haltOnSubcommand) {
        ...
        if (positionals_at_end_) {
            throw turbo::ExtrasError(name_, args);
        }
