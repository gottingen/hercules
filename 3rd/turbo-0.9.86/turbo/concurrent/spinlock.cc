...
// Uncommon constructors.
    SpinLock::SpinLock(base_internal::SchedulingMode mode)
            : lockword_(is_cooperative(mode) ? kSpinLockCooperative : 0) {
    ...
    void SpinLock::slow_lock() {
        ...
        // obtains the lock.
        int64_t wait_start_time = base_internal::CycleClock::time_now();
        ...
        while ((lock_value & kSpinLockHeld) != 0) {
            ...
            spin_lock_delay(&lockword_, lock_value, ++lock_wait_call_count,
                                         scheduling_mode);
            TURBO_TSAN_MUTEX_POST_DIVERT(this, 0);
            // Spin again after returning from the wait routine to give this thread
            // some chance of obtaining the lock.
            lock_value = spin_loop();
            wait_cycles = encode_wait_cycles(wait_start_time, base_internal::CycleClock::time_now());
            lock_value = try_lock_internal(lock_value, wait_cycles);
        }
    }

    void SpinLock::slow_unlock(uint32_t lock_value) {
        spin_lock_wake(&lockword_, false);  // wake waiter if necessary

        // If our acquisition was contended, collect contentionz profile info.  We
        // reserve a unitary wait time to represent that a waiter exists without our
        // own acquisition having been contended.
        if ((lock_value & kWaitTimeMask) != kSpinLockSleeper) {
            const int64_t wait_cycles = decode_wait_cycles(lock_value);
            TURBO_TSAN_MUTEX_PRE_DIVERT(this, 0);
    ...
    uint32_t SpinLock::encode_wait_cycles(int64_t wait_start_time,
    ...
    int64_t SpinLock::decode_wait_cycles(uint32_t lock_value) {
