...
#ifndef TURBO_CONCURRENT_SPINLOCK_H_
#define TURBO_CONCURRENT_SPINLOCK_H_
...
namespace turbo {
    ...
    public:
        ...
        inline bool try_lock() TURBO_EXCLUSIVE_TRYLOCK_FUNCTION(true) {
        ...
        inline void assert_held() const TURBO_ASSERT_EXCLUSIVE_LOCK() {
            if (!is_held()) {
    ...
    private:
        ...
        static constexpr bool is_cooperative(
        ...
        void slow_lock() TURBO_COLD;

        void slow_unlock(uint32_t lock_value) TURBO_COLD;

        uint32_t spin_loop();

        inline bool try_lock_impl() {
            uint32_t lock_value = lockword_.load(std::memory_order_relaxed);
    ...
    inline uint32_t SpinLock::try_lock_internal(uint32_t lock_value,
...
#endif  // TURBO_CONCURRENT_SPINLOCK_H_
