...
// logger->info("Some buffer {}", turbo::tlog::to_hex(v));
// char buf[128];
// logger->info("Some buffer {:X}", turbo::tlog::to_hex(std::begin(buf), std::end(buf)));
// logger->info("Some buffer {:X}", turbo::tlog::to_hex(std::begin(buf), std::end(buf), 16));
...
namespace turbo {
    ...
    struct formatter<turbo::tlog::details::dump_info<T>, char> {
        ...
        auto format(const turbo::tlog::details::dump_info<Container> &the_range, FormatContext &ctx) -> decltype(ctx.out()) {
