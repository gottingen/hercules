...
#ifndef TLOG_DISABLE_DEFAULT_LOGGER
...
#    endif
#endif // TLOG_DISABLE_DEFAULT_LOGGER
...
#    endif
        ...
        default_logger_ = std::make_shared<turbo::tlog::logger>(default_logger_name, std::move(color_sink));
    ...
    // To be used directly by the spdlog default api (e.g. turbo::tlog::info)
...
// Set global formatter. Each sink in each logger will get a clone of this object
    ...
    void registry::enable_backtrace(size_t n_messages) {
        std::lock_guard<std::mutex> lock(logger_map_mutex_);
    ...
    void registry::disable_backtrace() {
        std::lock_guard<std::mutex> lock(logger_map_mutex_);
    ...
    void registry::set_levels(log_levels levels, level::level_enum *global_level) {
        std::lock_guard<std::mutex> lock(logger_map_mutex_);
    ...
    void registry::throw_if_exists_(const std::string &logger_name) {
        if (loggers_.find(logger_name) != loggers_.end()) {
            throw_tlog_ex("logger with name '" + logger_name + "' already exists");
        }
    }
