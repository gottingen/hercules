...
//
#ifndef TURBO_MEMORY_ATOMIC_OBJECT_POOL_H_
#define TURBO_MEMORY_ATOMIC_OBJECT_POOL_H_
...
#define TURBO_ENABLE_POOLABLE_ON_THIS                          \
  template <typename T, size_t S> friend class AtomicObjectPool;  \
  void* _object_pool_block
    ...
    public:
        ...
        */
        explicit AtomicObjectPool(unsigned = std::thread::hardware_concurrency());
...
// ----------------------------------------------------------------------------
// AtomicObjectPool definition
// ----------------------------------------------------------------------------

// Constructor
    template<typename T, size_t S>
    AtomicObjectPool<T, S>::AtomicObjectPool(unsigned t) :
...
// Destructor
    template<typename T, size_t S>
    AtomicObjectPool<T, S>::~AtomicObjectPool() {
...
// Function: num_bins_per_local_heap
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::num_bins_per_local_heap() const {
        return B;
    }

// Function: num_objects_per_bin
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::num_objects_per_bin() const {
        return W;
    }

// Function: num_objects_per_block
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::num_objects_per_block() const {
        return M;
    }

// Function: emptiness_threshold
    template<typename T, size_t S>
    float AtomicObjectPool<T, S>::emptiness_threshold() const {
        return 1.0f / F;
    }

// Function: num_global_heaps
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::num_global_heaps() const {
        return 1;
    }

// Function: num_lheaps
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::num_local_heaps() const {
        return _lheaps.size();
    }

// Function: num_heaps
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::num_heaps() const {
        return _lheaps.size() + 1;
    }

// Function: capacity
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::capacity() const {
...
// Function: num_available_objects
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::num_available_objects() const {
...
// Function: num_allocated_objects
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::num_allocated_objects() const {
...
// Function: _bin
    template<typename T, size_t S>
    size_t AtomicObjectPool<T, S>::_bin(size_t u) const {
        return u == M ? F : u / W;
    }
...
// Function: _offset_in_class
    template<typename T, size_t S>
    template<class P, class Q>
    constexpr size_t AtomicObjectPool<T, S>::_offset_in_class(
...
// C++: parent_class_of(list_pointer,  &Block::list)
    template<typename T, size_t S>
    template<class P, class Q>
    constexpr P *AtomicObjectPool<T, S>::_parent_class_of(
...
// Function: _block_of
    template<typename T, size_t S>
    constexpr typename AtomicObjectPool<T, S>::Block *
    AtomicObjectPool<T, S>::_block_of(Blocklist *list) {
...
// Function: _block_of
    template<typename T, size_t S>
    constexpr typename AtomicObjectPool<T, S>::Block *
    AtomicObjectPool<T, S>::_block_of(const Blocklist *list) const {
        return _parent_class_of(list, &Block::list_node);
    }

// Procedure: initialize a list head
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_init_head(Blocklist *list) {
        list->next = list;
        list->prev = list;
    }
...
// the prev/next entries already!
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_add_impl(
...
//
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_push_front(
...
//
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_push_back(
            Blocklist *curr, Blocklist *head
    ) {
...
//
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_del_impl(
...
// in an undefined state.
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_del(Blocklist *entry) {
...
// If @old was empty, it will be overwritten.
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_replace(
...
// @head: the head that will precede our entry
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_move_front(
...
// @head: the head that will follow our entry
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_blocklist_move_back(
            Blocklist *list, Blocklist *head
    ) {
        _blocklist_del_impl(list->prev, list->next);
...
// @head: the head of the list
    template<typename T, size_t S>
    bool AtomicObjectPool<T, S>::_blocklist_is_first(
...
// @head: the head of the list
    template<typename T, size_t S>
    bool AtomicObjectPool<T, S>::_blocklist_is_last(
            const Blocklist *list, const Blocklist *head
    ) {
...
// @head: the list to test.
    template<typename T, size_t S>
    bool AtomicObjectPool<T, S>::_blocklist_is_empty(const Blocklist *head) {
...
// @head: the list to test.
    template<typename T, size_t S>
    bool AtomicObjectPool<T, S>::_blocklist_is_singular(
...
// Procedure: _for_each_block
    template<typename T, size_t S>
    template<typename C>
    void AtomicObjectPool<T, S>::_for_each_block(Blocklist *head, C &&c) {
        Blocklist *p;
...
// Iterate each item of a list - safe to free
    template<typename T, size_t S>
    template<typename C>
    void AtomicObjectPool<T, S>::_for_each_block_safe(Blocklist *head, C &&c) {
        Blocklist *p;
        Blocklist *t;
...
// allocate a spot from the block
    template<typename T, size_t S>
    T *AtomicObjectPool<T, S>::_allocate(Block *s) {
        if (s->top == nullptr) {
...
// Procedure: _deallocate
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::_deallocate(Block *s, T *ptr) {
    ...
    template<typename T, size_t S>
    template<typename... ArgsT>
    T *AtomicObjectPool<T, S>::animate(ArgsT &&... args) {
...
// Function: destruct
    template<typename T, size_t S>
    void AtomicObjectPool<T, S>::recycle(T *mem) {
...
// Function: _this_heap
    template<typename T, size_t S>
    typename AtomicObjectPool<T, S>::LocalHeap &
    AtomicObjectPool<T, S>::_this_heap() {
    ...
    // Function: _next_pow2
    template<typename T, size_t S>
    constexpr unsigned AtomicObjectPool<T, S>::_next_pow2(unsigned n) const {
        if (n == 0) return 1;
        n--;
        n |= n >> 1;
        n |= n >> 2;
...
#endif  // TURBO_MEMORY_ATOMIC_OBJECT_POOL_H_
