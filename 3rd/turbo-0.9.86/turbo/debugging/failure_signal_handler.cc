...
#undef FSD_PREVIOUS_INIT
    ...
    static bool SetupAlternateStackOnce() {
        const size_t page_mask = static_cast<size_t>(turbo::get_page_size()) - 1;
        size_t stack_size =
...
// if the system supports using an alternate stack.
    ...
    static void WriteToStderr(const char *data) {
        turbo::raw_log_internal::AsyncSignalSafeWriteToStderr(data, strlen(data));
    }
    ...
    // turbo::sleep_for() can't be used here since turbo_internal_sleep_for()
