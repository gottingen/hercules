...
#ifndef TURBO_DEBUGGING_INTERNAL_STACKTRACE_POWERPC_INL_H_
#define TURBO_DEBUGGING_INTERNAL_STACKTRACE_POWERPC_INL_H_
...
#include "turbo/debugging/internal/vdso_support.h" // a no-op on non-elf or non-glibc systems
...
TURBO_NO_SANITIZE_ADDRESS  // May read random elements from stack.
TURBO_NO_SANITIZE_MEMORY   // May read random elements from stack.
static void **NextStackFrame(void **old_sp, const void *uc) {
    ...
    if (IS_WITH_CONTEXT && uc != nullptr) {
        ...
        if (kernel_symbol_status == kNotInitialized) {
            turbo::debugging_internal::VDSOSupport vdso;
            if (vdso.IsPresent()) {
                turbo::debugging_internal::VDSOSupport::SymbolInfo
                        sigtramp_rt64_symbol_info;
                if (!vdso.LookupSymbol(
                        "__kernel_sigtramp_rt64", "LINUX_2.6.15",
                        turbo::debugging_internal::VDSOSupport::kVDSOSymbolType,
                        &sigtramp_rt64_symbol_info) ||
        ...
        if (new_sp != nullptr &&
            ...
            if (sp_before_signal != nullptr &&
                ...
                // a stack corruption.
                if (turbo::debugging_internal::AddressIsReadable(sp_before_signal)) {
...
TURBO_NO_INLINE static void TurboStacktracePowerPCDummyFunction() {
...
static int UnwindImpl(void **result, int *sizes, int max_depth, int skip_count,
    ...
    TurboStacktracePowerPCDummyFunction();
...
#endif  // TURBO_DEBUGGING_INTERNAL_STACKTRACE_POWERPC_INL_H_
