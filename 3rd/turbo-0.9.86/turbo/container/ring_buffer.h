...
#ifndef TURBO_CONTAINER_RING_BUFFER_H_
#define TURBO_CONTAINER_RING_BUFFER_H_
...
namespace turbo {
    ...
    public:
        ...
        typedef ring_buffer_iterator<T, const T *, const T &, Container>
                const_iterator;
    ...
    /// of these containers, such as fixed_string. Since ring buffer works via
    /// copying elements instead of allocating and freeing nodes, inserting in the
    /// middle of a ring buffer based on list (instead of vector) is no more
    /// efficient.
    ///
    ...
    /// node pool and custom allocator. There are various tradeoffs that result from
    /// this.
    ///
    /// Example usage:
    ...
    public: // We declare public so that global comparison operators can be
        // implemented without adding an inline level and without tripping up
        // GCC 2.x friend declaration failures. GCC (through at least v4.0) is
        // poor at inlining and performance wins over correctness.
        Container c; // We follow the naming convention established for stack, queue,
        // priority_queue and name this 'c'. This variable must always
        // have a size of at least 1, as even an empty ring_buffer has an
        // unused terminating element.

    protected:
        container_iterator mBegin; // We keep track of where our begin and end are by
        // using Container iterators.
        container_iterator mEnd;
        size_type mSize;

    public:
        // There currently isn't a ring_buffer constructor that specifies an initial
        // size, unlike other containers.
        ...
        ring_buffer(
                ...
                allocator_type()); // This function sets the capacity to be equal to
        ...
        void
        set_capacity(size_type n); // Sets the capacity to the given value, including
        // values less than the current capacity. Adjusts
        // the size downward if n < size, by throwing out
        // the oldest elements in the buffer.
        void reserve(size_type n); // Reserve a given capacity. Doesn't decrease the
        // capacity; it only increases it (for
        ...
        // size_type read(iterator** pPosition1, iterator** pPosition2, size_type&
        // nCount1, size_type& nCount2);
...
///////////////////////////////////////////////////////////////////////
    ...
    ring_buffer_iterator<T, Pointer, Reference, Container>::operator+=(
            difference_type n) {
        typedef
        typename std::iterator_traits<container_iterator>::iterator_category IC;
        increment(n, IC());
        return *this;
    }
    ...
    void ring_buffer_iterator<T, Pointer, Reference, Container>::increment(
            difference_type n, std::input_iterator_tag) {
    ...
    void ring_buffer_iterator<T, Pointer, Reference, Container>::increment(
        ...
        if (n >= 0) {
            ...
            // Recall that n and d here will be negative and so the logic here works as
            // intended.
...
// You can only add an integer to an iterator, and you cannot add two iterators.
    ...
    operator+(ptrdiff_t n,
              const ring_buffer_iterator<T, Pointer, Reference, Container> &x) {
        return x + n; // Implement (n + x) in terms of (x + n).
    }

// You can only add an integer to an iterator, but you can subtract two
// iterators.
    template<typename T, typename PointerA, typename ReferenceA, typename PointerB,
            typename ReferenceB, typename Container>
    inline typename ring_buffer_iterator<T, PointerA, ReferenceA,
    ...
    operator-(const ring_buffer_iterator<T, PointerA, ReferenceA, Container> &a,
        ...
        typedef
        typename ring_buffer_iterator<T, PointerA, ReferenceA,
                Container>::difference_type difference_type;

        // To do: If container_iterator is a random access iterator, then do a simple
        // calculation. Otherwise, we have little choice but to iterate from a to b
        // and count as we go. See the ring_buffer::size function for an
        // implementation of this.

        // Iteration implementation:
        difference_type d = 0;

        for (ring_buffer_iterator<T, PointerA, ReferenceA, Container> temp(b);
             temp != a; ++temp)
            ++d;
...
// The C++ defect report #179 requires that we support comparisons between const
// and non-const iterators. Thus we provide additional template paremeters here
// to support this. The defect report does not require us to support comparisons
// between reverse_iterators and const_reverse_iterators.
    template<typename T, typename PointerA, typename ReferenceA,
            typename ContainerA, typename PointerB, typename ReferenceB,
            typename ContainerB>
    inline bool
    operator==(const ring_buffer_iterator<T, PointerA, ReferenceA, ContainerA> &a,
...
// We provide a version of operator!= for the case where the iterators are of
// the same type. This helps prevent ambiguity errors in the presence of
// rel_ops.
    template<typename T, typename Pointer, typename Reference, typename Container>
    inline bool
    operator!=(const ring_buffer_iterator<T, Pointer, Reference, Container> &a,
...
///////////////////////////////////////////////////////////////////////
            ...
            : c() // Default construction with default allocator for the container.
    {
        ...
        c.resize(cap +
                 1); // Possibly we could construct 'c' with size, but c may not have
        // such a ctor, though we rely on it having a resize function.
        mBegin = c.begin();
        mEnd = mBegin;
        mSize = 0;
    }
    ...
    template<typename T, typename Container, typename Allocator>
    ring_buffer<T, Container, Allocator>::ring_buffer(
            size_type cap, const allocator_type &allocator)
            : c(allocator) {
        // To do: This code needs to be amended to deal with possible exceptions
        // that could occur during the resize call below.
    ...
    ring_buffer<T, Container, Allocator>::ring_buffer(const Container &x)
            : c(x) // This copies elements from x, but unless the user is doing some
    // tricks, the only thing that matters is that c.size() == x.size().
    {
        // To do: This code needs to be amended to deal with possible exceptions
        // that could occur during the resize call below.
        if (c.empty())
    ...
    ring_buffer<T, Container, Allocator>::ring_buffer(
        ...
        // We add one because the element at mEnd is necessarily unused.
        c.resize(1); // Possibly we could construct 'c' with size, but c may not have
        // such a ctor, though we rely on it having a resize function.
        mBegin = c.begin();
        mEnd = mBegin;
        mSize = 0;
    ...
    template<typename T, typename Container, typename Allocator>
    ring_buffer<T, Container, Allocator>::ring_buffer(const this_type &x) : c(x.c) {
        mBegin = c.begin();
        mEnd = mBegin;
        mSize = x.mSize;
        ...
        std::advance(
                mBegin, std::distance(const_cast<this_type &>(x).c.begin(),
                                      x.mBegin)); // We can do a simple distance algorithm
        // here, as there will be no wraparound.
        std::advance(mEnd,
                     std::distance(const_cast<this_type &>(x).c.begin(), x.mEnd));
    }
            ...
            : c() // Default construction with default allocator for the container.
    {
        ...
        swap(x); // We are leaving x in an unusual state by swapping
        // default-initialized members with it, as it won't be usable and can
        // be only destructible.
    }
    ...
    ring_buffer<T, Container, Allocator>::ring_buffer(
        ...
        if (c.get_allocator() == x.c.get_allocator())
            swap(x); // We are leaving x in an unusual state by swapping
            // default-initialized members with it, as it won't be usable and
            // can be only destructible.
        else
            operator=(x);
    }
    ...
    template<typename T, typename Container, typename Allocator>
    ring_buffer<T, Container, Allocator>::ring_buffer(
            std::initializer_list<value_type> ilist, const allocator_type &allocator)
            : c(allocator) {
        ...
        if (&x != this) {
            ...
            std::advance(
                    ...
                    std::distance(const_cast<this_type &>(x).c.begin(),
                                  x.mBegin)); // We can do a simple distance algorithm here,
            // as there will be no wraparound.
            std::advance(mEnd,
                         std::distance(const_cast<this_type &>(x).c.begin(), x.mEnd));
        }
    ...
    template<typename T, typename Container, typename Allocator>
    typename ring_buffer<T, Container, Allocator>::this_type &
    ring_buffer<T, Container, Allocator>::operator=(
            std::initializer_list<value_type> ilist) {
        assign(ilist.begin(), ilist.end());
        return *this;
    }
    ...
    template<typename T, typename Container, typename Allocator>
    template<typename InputIterator>
    void ring_buffer<T, Container, Allocator>::assign(InputIterator first,
                                                      InputIterator last) {
    ...
    void ring_buffer<T, Container, Allocator>::swap(this_type &x) {
        if (&x != this) {
            const difference_type dBegin = std::distance(
                    c.begin(), mBegin); // We can do a simple distance algorithm here, as
            // there will be no wraparound.
            ...
            mBegin = c.begin();
            std::advance(mBegin, dxBegin); // We can do a simple advance algorithm here,
            // as there will be no wraparound.
    ...
    ring_buffer<T, Container, Allocator>::begin() noexcept {
    ...
    template<typename T, typename Container, typename Allocator>
    typename ring_buffer<T, Container, Allocator>::iterator
    ring_buffer<T, Container, Allocator>::end() noexcept {
        return iterator(&c, mEnd);
    }

    template<typename T, typename Container, typename Allocator>
    typename ring_buffer<T, Container, Allocator>::const_iterator
    ring_buffer<T, Container, Allocator>::end() const noexcept {
        return const_iterator(
                const_cast<Container *>(&c),
    ...
    ring_buffer<T, Container, Allocator>::rbegin() noexcept {
    ...
    ring_buffer<T, Container, Allocator>::rbegin() const noexcept {
        return const_reverse_iterator(
                const_iterator(const_cast<Container *>(&c), mEnd));
    }
    ...
    template<typename T, typename Container, typename Allocator>
    typename ring_buffer<T, Container, Allocator>::reverse_iterator
    ring_buffer<T, Container, Allocator>::rend() noexcept {
    ...
    ring_buffer<T, Container, Allocator>::rend() const noexcept {
        return const_reverse_iterator(
                const_iterator(const_cast<Container *>(&c), mBegin));
    }
    ...
    bool ring_buffer<T, Container, Allocator>::full() const noexcept {
        ...
        // return mSize == (c.size() - 1); // (c.size() - 1) == capacity(); we are
        // attempting to reduce function calls.
    ...
    ring_buffer<T, Container, Allocator>::size() const noexcept {
        return mSize;
        ...
        // typedef typename
        // std::iterator_traits<container_iterator>::iterator_category IC; return
        // DoGetSize(IC()); // This is more direct than using iterator math.
    }
    ...
    void ring_buffer<T, Container, Allocator>::resize(size_type n) {
        ...
        if (n > cap) // If we need to grow in capacity...
        {
            ...
            mBegin = c.begin();
            mEnd = mBegin;
            std::advance(mEnd, n); // We can do a simple advance algorithm on this
            // because we know that mEnd will not wrap around.
        } else // We could do a check here for n != size(), but that would be costly
            // and people don't usually resize things to their same size.
        {
            mEnd = mBegin;

            // std::advance(mEnd, n); // We *cannot* use this because there may be
            // wraparound involved.

            // To consider: Possibly we should implement some more detailed logic to
            // optimize the code here. We'd need to do different behaviour dending on
            // whether the container iterator type is a random access iterator or
            // otherwise.
    ...
    template<typename T, typename Container, typename Allocator>
    typename ring_buffer<T, Container, Allocator>::size_type
    ring_buffer<T, Container, Allocator>::capacity() const noexcept {
        TURBO_ASSERT(
                c.size() >=
    ...
    void ring_buffer<T, Container, Allocator>::set_capacity(size_type n) {
        ...
        if (n != capacity) // If we need to change capacity...
        {
            ...
            std::copy(itCopyBegin, end(),
                      cTemp.begin()); // The begin-end range may in fact be larger than
            // n, in which case values will be overwritten.
            std::swap(c, cTemp);

            mBegin = c.begin();
            mEnd = mBegin;
            std::advance(mEnd,
                         mSize); // We can do a simple advance algorithm on this because
            // we know that mEnd will not wrap around.
        }
    }
    ...
    void ring_buffer<T, Container, Allocator>::reserve(size_type n) {
        ...
        if (n >
            (c.size() - 1)) // If we need to grow in capacity... // (c.size() - 1) ==
            // capacity(); we are attempting to reduce function calls.
        {
            Container cTemp(c);
            cTemp.resize(n + 1);
            std::copy(begin(), end(), cTemp.begin());
    ...
    ring_buffer<T, Container, Allocator>::back() {
        // return *(end() - 1); // Can't use this because not all iterators support
        // operator-.

        iterator temp(end()); // To do: Find a way to construct this temporary in the
        // return statement.
        return *(--temp);     // We can do it by making all our containers' iterators
        // support operator-.
    }
    ...
    ring_buffer<T, Container, Allocator>::back() const {
        ...
        const_iterator temp(end()); // To do: Find a way to construct this temporary
...
/// being overwritten and the begin position being moved foward one position.
    ...
    ring_buffer<T, Container, Allocator>::push_back() {
        // We don't do the following assignment, as the value at mEnd is already
        // constructed; it is merely possibly not default-constructed. However, the
        // spirit of push_back is that the user intends to do an assignment or data
        // modification after the push_back call. The user can always execute *back()
        // = value_type() if he wants.
        //*mEnd = value_type();
    ...
    void ring_buffer<T, Container, Allocator>::pop_back() {
        TURBO_ASSERT(mEnd != mBegin); // We assume that size() > 0 and thus that there
        // is something to pop.
    ...
    void ring_buffer<T, Container, Allocator>::pop_front() {
        TURBO_ASSERT(mBegin != mEnd); // We assume that mEnd > mBegin and thus that
    ...
    ring_buffer<T, Container, Allocator>::operator[](size_type n) {
        // return *(begin() + n); // Can't use this because not all iterators support
        // operator+.

        // This should compile to code that is nearly as efficient as that above.
        // The primary difference is the possible generation of a temporary in this
        // case.
        iterator temp(begin());
        std::advance(temp, n);
    ...
    template<typename T, typename Container, typename Allocator>
    typename ring_buffer<T, Container, Allocator>::iterator
    ring_buffer<T, Container, Allocator>::insert(const_iterator position,
                                                 const value_type &value) {
        // To consider: It would be faster if we could tell that position was in the
        // first half of the container and instead of moving things after the position
        // back, we could move things before the position forward.
        ...
        if (afterEnd.mContainerIterator == mBegin) {
            --beforeEnd;
        } else {
            push_back();
        }
        ...
        iterator itPosition(
                position.mpContainer,
                position.mContainerIterator); // We merely copy from const_iterator to
        // iterator.
    ...
    template<typename T, typename Container, typename Allocator>
    void ring_buffer<T, Container, Allocator>::insert(const_iterator position,
                                                      size_type n,
                                                      const value_type &value) {
    ...
    template<typename T, typename Container, typename Allocator>
    void ring_buffer<T, Container, Allocator>::insert(
            const_iterator position, std::initializer_list<value_type> ilist) {
    ...
    template<typename T, typename Container, typename Allocator>
    typename ring_buffer<T, Container, Allocator>::iterator
    ring_buffer<T, Container, Allocator>::erase(const_iterator first,
                                                const_iterator last) {
        iterator itFirst(first.mpContainer,
                         first.mContainerIterator); // We merely copy from
    ...
    template<typename T, typename Container, typename Allocator>
    typename ring_buffer<T, Container, Allocator>::reverse_iterator
    ring_buffer<T, Container, Allocator>::erase(const_reverse_iterator first,
                                                const_reverse_iterator last) {
        ...
        // Version which erases in order from last to first, but is slightly more
        // efficient:
    ...
    inline bool ring_buffer<T, Container, Allocator>::validate() const {
        if (c.empty()) // c must always have a size of at least 1, as even an empty
            // ring_buffer has an unused terminating element.
            return false;
...
///////////////////////////////////////////////////////////////////////
    ...
    template<typename T, typename Container, typename Allocator>
    inline bool operator==(const ring_buffer<T, Container, Allocator> &a,
                           const ring_buffer<T, Container, Allocator> &b) {
    ...
    inline bool operator<(const ring_buffer<T, Container, Allocator> &a,
                          const ring_buffer<T, Container, Allocator> &b) {
        const typename ring_buffer<T, Container, Allocator>::size_type sizeA =
                a.size();
        const typename ring_buffer<T, Container, Allocator>::size_type sizeB =
                b.size();
    ...
    template<typename T, typename Container, typename Allocator>
    inline void swap(ring_buffer<T, Container, Allocator> &a,
                     ring_buffer<T, Container, Allocator> &b) {
        a.swap(b);
    }

} // namespace turbo

#endif // TURBO_CONTAINER_RING_BUFFER_H_
