...
#  undef  REFLECT_TYPE_SUPPORTED
#  define REFLECT_TYPE_SUPPORTED 1
#endif

// Checks nameof_type_rtti compiler compatibility.
#if defined(__clang__)
#  if __has_feature(cxx_rtti)
#    undef  REFLECT_TYPE_RTTI_SUPPORTED
#    define REFLECT_TYPE_RTTI_SUPPORTED 1
#  endif
#elif defined(__GNUC__)
...
#  undef  REFLECT_MEMBER_SUPPORTED
#  define REFLECT_MEMBER_SUPPORTED 1
#endif

// Checks nameof_pointer compiler compatibility.
#if defined(__clang__) && __clang_major__ >= 5 || defined(__GNUC__) && __GNUC__ >= 7 || defined(_MSC_VER) && defined(_MSVC_LANG) && _MSVC_LANG >= 202002L
#  undef  REFLECT_POINTER_SUPPORTED
#  define REFLECT_POINTER_SUPPORTED 1
#endif

// Checks nameof_enum compiler compatibility.
#if defined(__clang__) && __clang_major__ >= 5 || defined(__GNUC__) && __GNUC__ >= 9 || defined(_MSC_VER) && _MSC_VER >= 1910
#  undef  REFLECT_ENUM_SUPPORTED
#  define REFLECT_ENUM_SUPPORTED 1
#endif

// Checks nameof_enum compiler aliases compatibility.
#if defined(__clang__) && __clang_major__ >= 5 || defined(__GNUC__) && __GNUC__ >= 9 || defined(_MSC_VER) && _MSC_VER >= 1920
#  undef  REFLECT_ENUM_SUPPORTED_ALIASES
#  define REFLECT_ENUM_SUPPORTED_ALIASES 1
#endif
...
// If need another string_view type, define the macro REFLECT_USING_ALIAS_STRING_VIEW.
#if defined(REFLECT_USING_ALIAS_STRING_VIEW)
    REFLECT_USING_ALIAS_STRING_VIEW
#else
    using std::string_view;
#endif

// If need another string type, define the macro REFLECT_USING_ALIAS_STRING.
#if defined(REFLECT_USING_ALIAS_STRING)
    REFLECT_USING_ALIAS_STRING
#else
    using std::string;
#endif
    ...
    namespace customize {
        ...
        struct enum_range {
            ...
            inline static constexpr int min = REFLECT_ENUM_RANGE_MIN;
            inline static constexpr int max = REFLECT_ENUM_RANGE_MAX;
            static_assert(max > min, "turbo::customize::enum_range requires max > min.");
        };

        static_assert(REFLECT_ENUM_RANGE_MIN <= 0, "REFLECT_ENUM_RANGE_MIN must be less or equals than 0.");
        static_assert(REFLECT_ENUM_RANGE_MIN > (std::numeric_limits<std::int16_t>::min)(),
                      "REFLECT_ENUM_RANGE_MIN must be greater than INT16_MIN.");

        static_assert(REFLECT_ENUM_RANGE_MAX > 0, "REFLECT_ENUM_RANGE_MAX must be greater than 0.");
        static_assert(REFLECT_ENUM_RANGE_MAX < (std::numeric_limits<std::int16_t>::max)(),
                      "REFLECT_ENUM_RANGE_MAX must be less than INT16_MAX.");

        static_assert(REFLECT_ENUM_RANGE_MAX > REFLECT_ENUM_RANGE_MIN,
                      "REFLECT_ENUM_RANGE_MAX must be greater than REFLECT_ENUM_RANGE_MIN.");
        ...
        constexpr string_view enum_name(E) noexcept {
            static_assert(std::is_enum_v<E>, "turbo::customize::enum_name requires enum type.");
    ...
    public:
        ...
        constexpr explicit cstring(string_view str) noexcept: cstring{str,
    ...
    private:
        ...
        constexpr
        cstring(string_view str, std::integer_sequence<std::uint16_t, I...>) noexcept : chars_{str[I]..., '\0'} {}
    ...
    namespace detail {
        ...
        constexpr bool cmp_less(L lhs, R rhs) noexcept {
            static_assert(std::is_integral_v<L> && std::is_integral_v<R>,
                          "turbo::detail::cmp_less requires integral type.");
        ...
        constexpr I log2(I value) noexcept {
            static_assert(std::is_integral_v<I>, "turbo::detail::log2 requires integral type.");
...
#if defined(REFLECT_ENUM_SUPPORTED) && REFLECT_ENUM_SUPPORTED || defined(REFLECT_ENUM_NO_CHECK_SUPPORT)
                : std::true_type {
        };
#else
        : std::false_type {};
        ...
        constexpr auto n() noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::n requires enum type.");
        ...
        constexpr bool is_valid() noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::is_valid requires enum type.");
        ...
        constexpr U ualue(std::size_t i) noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::ualue requires enum type.");

            if constexpr (std::is_same_v<U, bool>) { // bool special case
                static_assert(O == 0, "turbo::detail::ualue requires valid offset.");
        ...
        constexpr E value(std::size_t i) noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::value requires enum type.");
        ...
        constexpr int reflected_min() noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::reflected_min requires enum type.");
        ...
        constexpr int reflected_max() noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::reflected_max requires enum type.");
        ...
        constexpr auto values(std::index_sequence<I...>) noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::values requires enum type.");
        ...
        constexpr auto values() noexcept {
            ...
            static_assert(range_size > 0, "turbo::enum_range requires valid size.");
            static_assert(range_size < (std::numeric_limits<std::uint16_t>::max)(),
                          "turbo::enum_range requires valid size.");
        ...
        constexpr std::size_t range_size() noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::range_size requires enum type.");
        ...
        constexpr auto indexes(std::index_sequence<I...>) noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::indexes requires enum type.");
            constexpr auto min = IsFlags ? log2(min_v<E, IsFlags>) : min_v<E, IsFlags>;
        ...
        constexpr bool is_sparse() noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::is_sparse requires enum type.");
        ...
        [[nodiscard]] constexpr E get_value(std::size_t i) noexcept {
            static_assert(is_enum_v<E>, "turbo::detail::strings requires enum type.");
...
#if defined(REFLECT_TYPE_SUPPORTED) && REFLECT_TYPE_SUPPORTED || defined(REFLECT_TYPE_NO_CHECK_SUPPORT)
                : std::true_type {
        };
#else
        : std::false_type {};
...
#if defined(REFLECT_TYPE_RTTI_SUPPORTED) && REFLECT_TYPE_RTTI_SUPPORTED || defined(REFLECT_TYPE_NO_CHECK_SUPPORT)
                : std::true_type {
        };
#else
        : std::false_type {};
...
#if defined(REFLECT_MEMBER_SUPPORTED) && REFLECT_MEMBER_SUPPORTED || defined(REFLECT_TYPE_NO_CHECK_SUPPORT)
                : std::true_type {
        };
#else
        : std::false_type {};
...
#if defined(REFLECT_POINTER_SUPPORTED) && REFLECT_POINTER_SUPPORTED || defined(REFLECT_TYPE_NO_CHECK_SUPPORT)
                : std::true_type {
        };
#else
        : std::false_type {};
...
#  endif
...
#if defined(__clang__)
                constexpr string_view name{__PRETTY_FUNCTION__ + 30, sizeof(__PRETTY_FUNCTION__) - 33};
#elif defined(__GNUC__)
                constexpr string_view name{__PRETTY_FUNCTION__ + 45, sizeof(__PRETTY_FUNCTION__) - 48};
#elif defined(_MSC_VER)
                constexpr string_view name{__FUNCSIG__ + 62, sizeof(__FUNCSIG__) - 82 - (__FUNCSIG__[sizeof(__FUNCSIG__) - 19] == ' ' ? 1 : 0)};
#else
                constexpr auto name = string_view{};
#endif
                return cstring<name.size()>{name};
        ...
        string nameof_type_rtti(const char *tn) {
            static_assert(nameof_type_rtti_supported<T>::value,
                          "turbo::nameof_type_rtti unsupported compiler (https://github.com/Neargye/nameof#compiler-compatibility).");
        ...
        string nameof_type_rtti(const char* tn) noexcept {
          static_assert(nameof_type_rtti_supported<T>::value, "turbo::nameof_type_rtti unsupported compiler (https://github.com/Neargye/nameof#compiler-compatibility).");
    ...
    [[nodiscard]] constexpr auto nameof_enum(E value) noexcept -> detail::enable_if_enum_t<E, string_view> {
        ...
        static_assert(detail::count_v<D> > 0,
                      "turbo::nameof_enum requires enum implementation and valid max and min.");
    ...
    [[nodiscard]] auto nameof_enum_flag(E value) -> detail::enable_if_enum_t<E, string> {
        ...
        using U = std::underlying_type_t<D>;
        static_assert(detail::nameof_enum_supported<D>::value,
                      "turbo::nameof_enum_flag unsupported compiler (https://github.com/Neargye/nameof#compiler-compatibility).");
        static_assert(detail::count_v<D, true> > 0, "turbo::nameof_enum_flag requires enum-flags implementation.");
    ...
    nameof_member() noexcept -> std::enable_if_t<std::is_member_pointer_v<decltype(V)>, string_view> {
        static_assert(detail::nameof_member_supported<decltype(V)>::value,
                      "turbo::nameof_member unsupported compiler (https://github.com/Neargye/nameof#compiler-compatibility).");
    ...
    nameof_pointer() noexcept -> std::enable_if_t<std::is_pointer_v<decltype(V)>, string_view> {
        static_assert(detail::nameof_pointer_supported<decltype(V)>::value,
                      "turbo::nameof_pointer unsupported compiler (https://github.com/Neargye/nameof#compiler-compatibility).");
...
#define REFLECT(...) []() constexpr noexcept {                         \
  ...
  constexpr auto _nameof = ::turbo::cstring<_size>{_name};           \
  return _nameof; }()

// Obtains full name of variable, function, macro.
#define REFLECT_FULL(...) []() constexpr noexcept {                           \
  ::std::void_t<decltype(__VA_ARGS__)>();                                    \
  constexpr auto _name = ::turbo::detail::pretty_name(#__VA_ARGS__, false); \
  static_assert(!_name.empty(), "Expression does not have a name.");         \
  constexpr auto _size = _name.size();                                       \
  constexpr auto _nameof_full = ::turbo::cstring<_size>{_name};             \
  return _nameof_full; }()

// Obtains raw name of variable, function, macro.
#define REFLECT_RAW(...) []() constexpr noexcept {                    \
  ::std::void_t<decltype(__VA_ARGS__)>();                            \
  constexpr auto _name = ::turbo::string_view{#__VA_ARGS__};        \
  static_assert(!_name.empty(), "Expression does not have a name."); \
  constexpr auto _size = _name.size();                               \
  constexpr auto _nameof_raw = ::turbo::cstring<_size>{_name};      \
  return _nameof_raw; }()

// Obtains name of enum variable.
#define REFLECT_ENUM(...) ::turbo::nameof_enum<::std::decay_t<decltype(__VA_ARGS__)>>(__VA_ARGS__)

// Obtains name of enum variable or default value if enum variable out of range.
#define REFLECT_ENUM_OR(...) ::turbo::nameof_enum_or(__VA_ARGS__)

// Obtains name of static storage enum variable.
// This version is much lighter on the compile times and is not restricted to the enum_range limitation.
#define REFLECT_ENUM_CONST(...) ::turbo::nameof_enum<__VA_ARGS__>()

// Obtains name of enum-flags variable.
#define REFLECT_ENUM_FLAG(...) ::turbo::nameof_enum_flag<::std::decay_t<decltype(__VA_ARGS__)>>(__VA_ARGS__)

// Obtains type name, reference and cv-qualifiers are ignored.
#define REFLECT_TYPE(...) ::turbo::nameof_type<__VA_ARGS__>()

// Obtains full type name, with reference and cv-qualifiers.
#define REFLECT_FULL_TYPE(...) ::turbo::nameof_full_type<__VA_ARGS__>()

// Obtains short type name.
#define REFLECT_SHORT_TYPE(...) ::turbo::nameof_short_type<__VA_ARGS__>()

// Obtains type name of expression, reference and cv-qualifiers are ignored.
#define REFLECT_TYPE_EXPR(...) ::turbo::nameof_type<decltype(__VA_ARGS__)>()

// Obtains full type name of expression, with reference and cv-qualifiers.
#define REFLECT_FULL_TYPE_EXPR(...) ::turbo::nameof_full_type<decltype(__VA_ARGS__)>()

// Obtains short type name of expression.
#define REFLECT_SHORT_TYPE_EXPR(...) ::turbo::nameof_short_type<decltype(__VA_ARGS__)>()

// Obtains type name, with reference and cv-qualifiers, using RTTI.
#define REFLECT_TYPE_RTTI(...) ::turbo::detail::nameof_type_rtti<::std::void_t<decltype(__VA_ARGS__)>>(typeid(__VA_ARGS__).name())

// Obtains full type name, using RTTI.
#define REFLECT_FULL_TYPE_RTTI(...) ::turbo::detail::nameof_full_type_rtti<decltype(__VA_ARGS__)>(typeid(__VA_ARGS__).name())

// Obtains short type name, using RTTI.
#define REFLECT_SHORT_TYPE_RTTI(...) ::turbo::detail::nameof_short_type_rtti<decltype(__VA_ARGS__)>(typeid(__VA_ARGS__).name())

// Obtains name of member.
#define REFLECT_MEMBER(...) ::turbo::nameof_member<__VA_ARGS__>()

// Obtains name of a function, a global or class static variable.
#define REFLECT_POINTER(...) ::turbo::nameof_pointer<__VA_ARGS__>()
