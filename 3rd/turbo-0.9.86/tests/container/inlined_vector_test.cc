...
// reference counts == 1.
    ...
    TEST(InlinedVectorTest, Noexcept) {
        ...
        EXPECT_TRUE((std::is_nothrow_move_constructible<
                turbo::InlinedVector<MoveOnly, 2>>::value));
        ...
        EXPECT_EQ(turbo::default_allocator_is_nothrow::value,
                  (std::is_nothrow_move_constructible<
                          turbo::InlinedVector<MoveCanThrow, 2>>::value));
    }
...
// SourceContainer as the source container type.
    ...
    TEST(AllocatorSupportTest, Constructors) {
        ...
        { AllocVec TURBO_MAYBE_UNUSED v; }
        { AllocVec TURBO_MAYBE_UNUSED v(alloc); }
        { AllocVec TURBO_MAYBE_UNUSED v(ia, ia + TURBO_ARRAY_SIZE(ia), alloc); }
        { AllocVec TURBO_MAYBE_UNUSED v({1, 2, 3}, alloc); }

        AllocVec v2;
        { AllocVec TURBO_MAYBE_UNUSED v(v2, alloc); }
        { AllocVec TURBO_MAYBE_UNUSED v(std::move(v2), alloc); }
    }
    ...
    TEST(AllocatorSupportTest, CountAllocations) {
        ...
        MyAlloc alloc(&allocated);
        {
            AllocVec TURBO_MAYBE_UNUSED v(ia, ia + 4, alloc);
    ...
    TEST(InlinedVectorTest, MinimumAllocatorCompilesUsingTraits) {
        ...
        using ATraits = turbo::allocator_traits<A>;
