...
// on performance.
            TEST(Btree, MovesComparisonsCopiesSwapsTracking) {
                ...
                if (sizeof(void *) == 8) {
                    EXPECT_EQ(
                            BtreeNodePeer::GetNumSlotsPerNode<turbo::btree_set<int32_t>>(),
                    // When we have generations, there is one fewer slot.
                            BtreeNodePeer::UsesGenerations<turbo::btree_set<int32_t>>() ? 60 : 61);
                }
...
// See https://en.cppreference.com/w/cpp/container/map/value_compare.
            ...
            TEST(Btree, ExtractAndGetNextMap) {
                turbo::btree_map<int, int> src = {{1, 2},
                                                  {3, 4},
                                                  {5, 6}};
                auto it = src.find(3);
                auto extracted_and_next = src.extract_and_get_next(it);
            ...
            TEST(Btree, ExtractAndGetNextMultiMap) {
                turbo::btree_multimap<int, int> src = {{1, 2},
                                                       {3, 4},
                                                       {5, 6}};
                auto it = src.find(3);
                auto extracted_and_next = src.extract_and_get_next(it);
...
// reliably check the libstdc++ version prior to that release.
    ...
    (defined(_GLIBCXX_RELEASE) && _GLIBCXX_RELEASE >= 7)
            TEST(Btree, MapRangeConstructorAndInsertSupportExplicitConversionComparable) {
                const std::pair<std::string_view, int> names[] = {{"n1", 1},
                                                                  {"n2", 2}};
...
// This test crashes on MSVC.
            TEST(Btree, InvalidIteratorUse) {
                if (!BtreeNodePeer::UsesGenerations<turbo::btree_set<int>>())
            ...
            private:
                template<typename T>
                friend
                class OnlyConstructibleAllocator;
