...
#define C(x) 0x##x##ULL
        ...
        void TestUnchanging(const uint64_t *expected, int offset, int len) {
            CHECK_EQ(expected[0], CityHash64(data + offset, len));
            CHECK_EQ(expected[3], CityHash32(data + offset, len));
            CHECK_EQ(expected[1], CityHash64WithSeed(data + offset, len, kSeed0));
            CHECK_EQ(expected[2],
                     CityHash64WithSeeds(data + offset, len, kSeed0, kSeed1));
        }

        TEST_CASE("CityHashTest - Unchanging") {
            setup();
            int i = 0;
            for (; i < kTestSize - 1; i++) {
