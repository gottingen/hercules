...
namespace {
    ...
    void serialize_check(const std::pair<double, size_t>& para) {
        const double d = para.first;
    ...
    void accuracy_check(const std::pair<double, size_t>& pdata) {
        ...
        const std::pair<double, size_t> para = pdata;
        ...
        turbo::Println("bernoulli_test: p = {} {} trials= {} yes = {}", p, std::abs(static_cast<double>(yes) - expected) / stddev, trials, yes);
        REQUIRE_LE(std::abs(static_cast<double>(yes) - expected), 5 * stddev);
        turbo::Println("bernoulli_test: p = {} {} trials= {} yes = {}", p, std::abs(static_cast<double>(yes) - expected) / stddev, trials, yes);
    }
    TEST_CASE("bernoulli_test") {
        ...
        SUBCASE("StabilityTest") {
            ...
            const double kP = 0.0331289862362;
            {
                ...
                REQUIRE_EQ(35, urbg.invocations());
                REQUIRE_EQ(v, "00000000000010000000000010000000000");
                //INFO("", dist);
            }
            {
                ...
                auto v = generate(dist);
                REQUIRE_EQ(35, urbg.invocations());
                REQUIRE_EQ(v, "00000100010010010010000011000011010");// << dist;
            }
            {
                turbo::bernoulli_distribution dist(kP * 20.0);
                auto v = generate(dist);
                REQUIRE_EQ(35, urbg.invocations());
                REQUIRE_EQ(v, "00011110010110110011011111110111011");// << dist;
            }
            {
                turbo::bernoulli_distribution dist(1.0 - kP);
                auto v = generate(dist);
                REQUIRE_EQ(35, urbg.invocations());
                REQUIRE_EQ(v, "11111111111111111111011111111111111");// << dist;
            }
        }

        SUBCASE("StabilityTest2") {
            turbo::random_internal::sequence_urbg urbg(
            ...
            constexpr double b3 = (5.0 / 13.0 / 0.2) - ((1 - b0) + (1 - b1) + (1 - b1));
            {
                ...
                REQUIRE_EQ(v, "000011100101");// << dist;
            }
            {
                turbo::bernoulli_distribution dist(b1);
                auto v = generate(dist);
                REQUIRE_EQ(12, urbg.invocations());
                REQUIRE_EQ(v, "001111101101");// << dist;
            }
            {
                turbo::bernoulli_distribution dist(b3);
                auto v = generate(dist);
                REQUIRE_EQ(12, urbg.invocations());
                REQUIRE_EQ(v, "001111101111");// << dist;
            }
        }
    }
}  // namespace
