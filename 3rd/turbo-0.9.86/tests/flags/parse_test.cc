...
namespace {
    ...
    const std::string &GetTestTempDir() {
        static std::string *temp_dir_name = []() -> std::string * {
            ...
            if (res->empty()) {
                TLOG_CRITICAL("Failed to make temporary directory for data files");
            }
    ...
    const char *GetFlagfileFlag(const std::vector<FlagfileData> &ffd,
        ...
        for (const auto &flagfile_data: ffd) {
            std::string flagfile_name =
                    turbo::format("{}{}", GetTestTempDir(), flagfile_data.file_name);

            std::ofstream flagfile_out(flagfile_name);
            for (auto line: flagfile_data.file_lines) {
                auto nline = turbo::str_replace_all(line, {{std::string("$0"), GetTestTempDir()}});
                flagfile_out<< nline << "\n";
            }

            turbo::format_append(&flagfile_flag, "{}{}", separator, flagfile_name);
            separator = ",";
        }
...
// --------------------------------------------------------------------
    ...
    flags::HelpMode InvokeParseTurboOnlyImpl(const char *(&in_argv)[N]) {
        ...
        return flags::ParseTurbFlagsOnlyImpl(N, const_cast<char **>(in_argv),
...
// --------------------------------------------------------------------
    ...
    TEST_F(ParseTest, ParsetoFlagsOnlySuccess) {
        const char *in_args[] = {
                "testbin",
                "arg1",
                "--bool_flag",
...
// --------------------------------------------------------------------
    ...
    TEST_F(ParseDeathTest, ParseTubFlagsOnlyFailure) {
        const char *in_args[] = {
                "testbin",
                "--int_flag=21.1",
        };
