...
namespace {
    ...
    static const int seed = turbo::get_current_time_seconds();
...
void clause11_21_4_2_k(String &test) {
    ...
    FOR_EACH_RANGE (i, 0, s.size()) { s[i] = random('a', 'z'); }
    test = std::move(s);
    if (std::is_same<String, inlined_string>::value) {
...
TEST(InlinedString, testAllClauses) {
    EXPECT_TRUE(1) << "Starting with seed: " << seed;
    std::string r;
    turbo::inlined_string c;
#if defined(TURBO_HAVE_WCHAR_SUPPORT)
    std::wstring wr;
    turbo::basic_inlined_string<wchar_t> wc;
#endif
    int count = 0;

    auto l = [&](const char *const clause,
                 void (*f_string)(std::string &),
                 void (*f_fbstring)(turbo::inlined_string &),
                 void (*f_wfbstring)(turbo::basic_inlined_string<wchar_t> &)) {
        do {
            if (true) {
            } else {
            ...
            wc = turbo::basic_inlined_string<wchar_t>(wr.c_str());
#endif
...
#define TEST_CLAUSE(x)             \
  l(#x,                            \
    ...
    clause11_##x<turbo::inlined_string>, \
    clause11_##x<turbo::basic_inlined_string<wchar_t>>);
...
TEST(InlinedString, testMoveCtor) {
    ...
    inlined_string s(size, 'a');
    inlined_string test = std::move(s);
    EXPECT_TRUE(s.empty());
    EXPECT_EQ(size, test.size());
}

TEST(InlinedString, testMoveAssign) {
    // Move constructor. Make sure we allocate a large string, so the
    // small string optimization doesn't kick in.
    auto size = random(100, 2000);
    inlined_string s(size, 'a');
...
TEST(InlinedString, testMoveOperatorPlusRhs) {
    // Make sure we allocate a large string, so the
    // small string optimization doesn't kick in.
    auto size1 = random(100, 2000);
    auto size2 = random(100, 2000);
...
TEST(InlinedString, testConstructionFromLiteralZero) {
    EXPECT_THROW(inlined_string s(nullptr), std::logic_error);
}

TEST(InlinedString, testFixedBugs_D479397) {
    inlined_string str(1337, 'f');
    inlined_string cp = str;
    cp.clear();
    cp.c_str();
    EXPECT_EQ(str.front(), 'f');
}

TEST(InlinedString, testFixedBugs_D481173) {
    inlined_string str(1337, 'f');
    for (int i = 0; i < 2; ++i) {
        inlined_string cp = str;
        cp[1] = 'b';
...
TEST(InlinedString, testFixedBugs_D580267_push_back) {
    inlined_string str(1337, 'f');
    inlined_string cp = str;
    cp.push_back('f');
}

TEST(InlinedString, testFixedBugs_D580267_operator_add_assign) {
    inlined_string str(1337, 'f');
    inlined_string cp = str;
    cp += "bb";
}

TEST(InlinedString, testFixedBugs_D661622) {
    turbo::basic_inlined_string<wchar_t> s;
    EXPECT_EQ(0, s.size());
}

TEST(InlinedString, testFixedBugs_D785057) {
    inlined_string str(1337, 'f');
    std::swap(str, str);
    EXPECT_EQ(1337, str.size());
}

TEST(InlinedString, testFixedBugs_D1012196_allocator_malloc) {
    inlined_string str(128, 'f');
    str.clear(); // Empty medium string.
    inlined_string copy(str); // Medium string of 0 capacity.
    copy.push_back('b');
    EXPECT_GE(copy.capacity(), 1);
}

TEST(InlinedString, testFixedBugs_D2813713) {
    inlined_string s1("a");
    s1.reserve(8); // Trigger the optimized code path.
    auto test1 = '\0' + std::move(s1);
    EXPECT_EQ(2, test1.size());

    inlined_string s2(1, '\0');
    s2.reserve(8);
    auto test2 = "a" + std::move(s2);
    EXPECT_EQ(2, test2.size());
}

TEST(InlinedString, testFixedBugs_D3698862) {
    EXPECT_EQ(inlined_string().find(inlined_string(), 4), inlined_string::npos);
}

/*
TEST(InlinedString, testFixedBugs_D4355440) {
  inlined_string str(1337, 'f');
  str.reserve(3840);
...
*/
TEST(InlinedString, findWithNpos) {
    inlined_string fbstr("localhost:80");
    EXPECT_EQ(inlined_string::npos, fbstr.find(":", inlined_string::npos));
}

TEST(InlinedString, testHash) {
    inlined_string a;
    inlined_string b;
    a.push_back(0);
    a.push_back(1);
    b.push_back(0);
    b.push_back(2);
    turbo::Hash<inlined_string> hashfunc;
    EXPECT_NE(hashfunc(a), hashfunc(b));
}

#if defined(TURBO_HAVE_WCHAR_SUPPORT)
TEST(InlinedString, testHashChar16) {
  using u16fbstring = turbo::basic_inlined_string<char16_t>;
  u16fbstring a;
  u16fbstring b;
  a.push_back(0);
  a.push_back(1);
...
TEST(InlinedString, testFrontBack) {
    inlined_string str("hello");
...
TEST(InlinedString, noexcept) {
    EXPECT_TRUE(noexcept(inlined_string()));
    inlined_string x;
    EXPECT_FALSE(noexcept(inlined_string(x)));
    EXPECT_TRUE(noexcept(inlined_string(std::move(x))));
    inlined_string y;
    EXPECT_FALSE(noexcept(y = x));
    EXPECT_TRUE(noexcept(y = std::move(x)));
}

TEST(InlinedString, iomanip) {
    stringstream ss;
    inlined_string fbstr("Hello");
...
TEST(InlinedString, rvalueIterators) {
    // you cannot take &* of a move-iterator, so use that for testing
    inlined_string s = "base";
    inlined_string r = "hello";
    r.replace(
            r.begin(),
            r.end(),
...
TEST(InlinedString, moveTerminator) {
    // The source of a move must remain in a valid state
    inlined_string s(100, 'x'); // too big to be in-situ
    inlined_string k;
    k = std::move(s);
 ...
 *   explicit basic_inlined_string(const A& a) noexcept;
 */

    struct TestStructDefaultAllocator {
        turbo::basic_inlined_string<char> stringMember;
    };
...
TEST(InlinedString, compareToStdString) {
    using turbo::inlined_string;
    using namespace std::string_literals;
    auto stdA = "a"s;
    auto stdB = "b"s;
    inlined_string fbA("a");
    inlined_string fbB("b");
...
TEST(U16FBString, compareToStdU16String) {
    using turbo::basic_inlined_string;
    using namespace std::string_literals;
    auto stdA = u"a"s;
    auto stdB = u"b"s;
    basic_inlined_string<char16_t> fbA(u"a");
    basic_inlined_string<char16_t> fbB(u"b");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(U32FBString, compareToStdU32String) {
    ...
    auto stdA = U"a"s;
    auto stdB = U"b"s;
    basic_inlined_string<char32_t> fbA(U"a");
    basic_inlined_string<char32_t> fbB(U"b");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(WFBString, compareToStdWString) {
    ...
    auto stdA = L"a"s;
    auto stdB = L"b"s;
    basic_inlined_string<wchar_t> fbA(L"a");
    basic_inlined_string<wchar_t> fbB(L"b");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(InlinedString, compareToStdStringLong) {
    using turbo::inlined_string;
    using namespace std::string_literals;
    auto stdA = "1234567890a"s;
    auto stdB = "1234567890ab"s;
    inlined_string fbA("1234567890a");
    inlined_string fbB("1234567890ab");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(U16FBString, compareToStdU16StringLong) {
    ...
    basic_inlined_string<char16_t> fbA(u"1234567890a");
    basic_inlined_string<char16_t> fbB(u"1234567890ab");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(U32FBString, compareToStdU32StringLong) {
    ...
    auto stdA = U"1234567890a"s;
    auto stdB = U"1234567890ab"s;
    basic_inlined_string<char32_t> fbA(U"1234567890a");
    basic_inlined_string<char32_t> fbB(U"1234567890ab");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(WFBString, compareToStdWStringLong) {
    ...
    basic_inlined_string<wchar_t> fbA(L"1234567890a");
    basic_inlined_string<wchar_t> fbB(L"1234567890ab");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(InlinedString, OverLarge) {
    EXPECT_THROW(
            inlined_string().reserve((size_t) 0xFFFF'FFFF'FFFF'FFFF), std::length_error);
    EXPECT_THROW(
            inlined_string_core<char32_t>().reserve((size_t) 0x4000'0000'4000'0000),
            std::length_error);
}
