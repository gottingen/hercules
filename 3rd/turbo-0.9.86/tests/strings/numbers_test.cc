...
namespace {
    ...
    using turbo::numbers_internal::SixDigitsToBuffer;
    ...
    void CheckInt32(int32_t x) {
        ...
        char *actual = turbo::numbers_internal::FastIntToBuffer(x, buffer);
        std::string expected = std::to_string(x);
        EXPECT_EQ(expected, std::string(buffer, actual)) << " Input " << x;

        char *generic_actual = turbo::numbers_internal::FastIntToBuffer(x, buffer);
    ...
    void CheckInt64(int64_t x) {
        ...
        buffer[23] = '*';
        buffer[24] = '*';
        char *actual = turbo::numbers_internal::FastIntToBuffer(x, &buffer[1]);
        std::string expected = std::to_string(x);
        ...
        char *my_actual =
                turbo::numbers_internal::FastIntToBuffer(MyInt64(x), &buffer[1]);
    ...
    void CheckUInt64(uint64_t x) {
        ...
        char *generic_actual = turbo::numbers_internal::FastIntToBuffer(x, &buffer[1]);
        EXPECT_EQ(expected, std::string(&buffer[1], generic_actual))
                            << " Input " << x;

        char *my_actual =
                turbo::numbers_internal::FastIntToBuffer(MyUInt64(x), &buffer[1]);
        EXPECT_EQ(expected, std::string(&buffer[1], my_actual)) << " Input " << x;
    }
    ...
    TEST(stringtest, safe_strtou128_random) {
        ...
        // uint128 can be streamed, so this code must be custom
        ...
        for (size_t i = 0; i < kNumRandomTests; i++) {
            ...
            // Test underflow
            s.clear();
            turbo::strings_internal::OStringStream(&s) << "-" << value;
            EXPECT_FALSE(parse_func(s, &parsed_value, base));
        }
    }
    ...
    TEST(stringtest, safe_strto128_random) {
        ...
        // int128 can be streame, so this code must be custom
            ...
            for (size_t i = 0; i < mismatches.size(); ++i) {
                ...
                TURBO_RAW_LOG(
                        ...
                        turbo::format(
                                ...
                                PerfectDtoa(d), " ", (SixDigitsToBuffer(d, b2), b2),
                                (snprintf(b1, sizeof(b1), "%g", d), b1),
                                PerfectDtoa(after),
