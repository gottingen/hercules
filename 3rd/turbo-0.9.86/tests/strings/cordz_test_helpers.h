...
class CordzSamplingIntervalHelper {
 public:
  explicit CordzSamplingIntervalHelper(int32_t interval)
      : orig_mean_interval_(turbo::cord_internal::get_cordz_mean_interval()) {
    turbo::cord_internal::set_cordz_mean_interval(interval);
    turbo::cord_internal::cordz_set_next_sample_for_testing(interval);
  }

  ~CordzSamplingIntervalHelper() {
    turbo::cord_internal::set_cordz_mean_interval(orig_mean_interval_);
    turbo::cord_internal::cordz_set_next_sample_for_testing(orig_mean_interval_);
  }
