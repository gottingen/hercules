...
// of the full, over-large mantissa.
    ...
    void ExpectParsedFloat(std::string s, turbo::chars_format format_flags,
        ...
        if (open_bracket_pos != std::string::npos) {
            ...
            TURBO_RAW_CHECK(close_bracket_pos != std::string_view::npos,
        ...
        CHECK_NE(parsed.end, nullptr);
        if (parsed.end == nullptr) {
            return;  // The following tests are not useful if we fully failed to parse
        }
        CHECK_EQ(parsed.type, expected_type);
        if (begin_subrange == -1) {
            CHECK_EQ(parsed.subrange_begin, nullptr);
            CHECK_EQ(parsed.subrange_end, nullptr);
        } else {
            CHECK_EQ(parsed.subrange_begin, s.data() + begin_subrange);
            CHECK_EQ(parsed.subrange_end, s.data() + end_subrange);
        }
        if (parsed.type == FloatType::kNumber) {
            CHECK_EQ(parsed.mantissa, expected_mantissa);
            CHECK_EQ(parsed.exponent, expected_exponent);
            if (expected_literal_exponent != -999) {
                CHECK_EQ(parsed.literal_exponent, expected_literal_exponent);
            }
        }
        auto characters_matched = static_cast<int>(parsed.end - s.data());
        CHECK_EQ(characters_matched, expected_characters_matched);
    }
...
// Check that a given input string is not matched by Float.
    ...
    void ExpectFailedParse(std::string_view s, turbo::chars_format format_flags) {
        ParsedFloat parsed =
                ParseFloat<base>(s.data(), s.data() + s.size(), format_flags);
        CHECK_EQ(parsed.end, nullptr);
    }

    TEST_CASE("ParseFloat, SimpleValue") {
    ...
    TEST_CASE("ParseFloat, LargeDecimalMantissa") {
    ...
    TEST_CASE("ParseFloat, LargeHexadecimalMantissa") {
    ...
    TEST_CASE("ParseFloat, ScientificVsFixed") {
