...
void async(unsigned W) {
  ...
  for(int i=0; i<N; ++i) {
    if(auto r = i%3; r==0) {
      ...
      fus.emplace_back(executor.async(collie::tf::DefaultTaskParams{}, [&](){
        counter.fetch_add(1, std::memory_order_relaxed);
        return -2;
      }));
    }
    else {
      fus.emplace_back(executor.async(collie::tf::TaskParams{std::to_string(i)}, [&](){
        counter.fetch_add(1, std::memory_order_relaxed);
        return -2;
      }));
    }
...
void subflow_async(size_t W) {
  ...
  auto S1 = taskflow.emplace([&] (collie::tf::Subflow& sf){
    for(int i=0; i<1000; i++) {
      sf.async([&](){ counter.fetch_add(1, std::memory_order_relaxed); });
    }
  });

  auto S2 = taskflow.emplace([&] (collie::tf::Subflow& sf){
...
void nested_subflow_async(size_t W) {
  ...
  taskflow.emplace([&](collie::tf::Subflow& sf1){

    for(int i=0; i<100; i++) {
      sf1.async([&](){ counter.fetch_add(1, std::memory_order_relaxed); });
    }

    sf1.emplace([&](collie::tf::Subflow& sf2){
      for(int i=0; i<100; i++) {
      ...
      sf2.emplace([&](collie::tf::Subflow& sf3){
        for(int i=0; i<100; i++) {
          sf3.silent_async(
            [&](){ counter.fetch_add(1, std::memory_order_relaxed); }
          );
...
void runtime_async(size_t W) {
  ...
  auto S1 = taskflow.emplace([&] (collie::tf::Runtime& sf){
    for(int i=0; i<1000; i++) {
      sf.silent_async(
        [&](){counter.fetch_add(1, std::memory_order_relaxed);}
      );
  ...
  auto S2 = taskflow.emplace([&] (collie::tf::Runtime& sf){
  ...
  taskflow.emplace([&] (collie::tf::Runtime& sf){
    sf.silent_async([&](){ counter.fetch_add(1, std::memory_order_relaxed); });
    for(int i=0; i<1000; i++) {
      sf.async(
        [&](){ counter.fetch_add(1, std::memory_order_relaxed); }
