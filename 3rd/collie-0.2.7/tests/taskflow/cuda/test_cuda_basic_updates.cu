...
TEST_CASE("cudaFlowCapturer.Incrementality") {
  ...
  REQUIRE(cf.native_executable() != nullptr);
  REQUIRE(collie::tf::cuda_graph_get_num_nodes(cf.native_graph()) == cf.num_tasks());
...
TEST_CASE("cudaFlowCapturer.rebind.algorithms") {
  ...
  collie::tf::cudaFlowCapturer capturer;

  auto data = collie::tf::cuda_malloc_shared<int>(10000);
  auto res = collie::tf::cuda_malloc_shared<int>(1);
