...
#include <collie/taskflow/cuda/algorithm/reduce.h>
#include <collie/taskflow/cuda/algorithm/sort.h>
#include <collie/taskflow/cuda/algorithm/find.h>
...
void add2() {
  ...
  for(size_t N=1; N<=(1<<20); N <<= 1) {
    ...
    // standalone tramsform
      collie::tf::cudaDefaultExecutionPolicy p;

    auto input  = collie::tf::cuda_malloc_shared<T>(N);
    auto output = collie::tf::cuda_malloc_shared<T>(N);
    for(size_t n=0; n<N; n++) {
      input[n] = 1;
    }

    collie::tf::cuda_transform(p, input, input + N, output, 
...
void transform() {
  ...
  for(unsigned n=1; n<=1234567; n = n*2 + 1) {
    ...
    auto src1 = collie::tf::cuda_malloc_shared<int>(n);
    auto src2 = collie::tf::cuda_malloc_shared<int>(n);
    auto dest = collie::tf::cuda_malloc_shared<int>(n);
...
TEST_CASE("cudaflow.transform" * doctest::timeout(300)) {
  transform<collie::tf::cudaFlow>();
}

TEST_CASE("capture.transform" * doctest::timeout(300) ) {
  transform<collie::tf::cudaFlowCapturer>();
}
...
void scan() {
  ...
  for(int N=1; N<=1234567; N = N*2 + 1) {
    ...
    auto data1 = collie::tf::cuda_malloc_shared<T>(N);
    auto data2 = collie::tf::cuda_malloc_shared<T>(N);
    auto scan1 = collie::tf::cuda_malloc_shared<T>(N);
    auto scan2 = collie::tf::cuda_malloc_shared<T>(N);
    ...
    collie::tf::cudaDeviceVector<std::byte> temp(
      collie::tf::cuda_scan_buffer_size<collie::tf::cudaDefaultExecutionPolicy, T>(N)
    );

    collie::tf::cuda_inclusive_scan(collie::tf::cudaDefaultExecutionPolicy{},
      data1, data1+N, scan1, collie::tf::cuda_plus<T>{}, temp.data()
    );
    cudaStreamSynchronize(0);

    collie::tf::cuda_exclusive_scan(collie::tf::cudaDefaultExecutionPolicy{}, 
      data2, data2+N, scan2, collie::tf::cuda_plus<T>{}, temp.data()
    );
    cudaStreamSynchronize(0);
...
void transform_scan() {
  ...
  for(int N=1; N<=1234567; N = N*2 + 1) {
    ...
    collie::tf::cuda_transform_inclusive_scan(collie::tf::cudaDefaultExecutionPolicy{},
      data1, data1+N, scan1, 
      [] __device__ (T a, T b){ return a+b; },
      [] __device__ (T a) { return a*10; },
      temp.data()
    ...
    collie::tf::cuda_transform_exclusive_scan(collie::tf::cudaDefaultExecutionPolicy{},
      data2, data2+N, scan2, 
      [] __device__ (T a, T b){ return a+b; },
      [] __device__ (T a) { return a*10; },
      temp.data()
...
TEST_CASE("cudaflow.scan.int" * doctest::timeout(300)) {
  transform_scan<int, collie::tf::cudaFlow>();
}

TEST_CASE("cudaflow.scan.size_t" * doctest::timeout(300)) {
  transform_scan<size_t, collie::tf::cudaFlow>();
}

TEST_CASE("capture.transform_scan.int" * doctest::timeout(300)) {
  transform_scan<int, collie::tf::cudaFlowCapturer>();
}

TEST_CASE("capture.transform_scan.size_t" * doctest::timeout(300)) {
  transform_scan<size_t, collie::tf::cudaFlowCapturer>();
}
...
void merge_keys() {
  ...
  for(int N=0; N<=1234567; N = N*2 + 1) {
    ...
    auto a = collie::tf::cuda_malloc_shared<T>(N);
    auto b = collie::tf::cuda_malloc_shared<T>(N);
    auto c = collie::tf::cuda_malloc_shared<T>(2*N);
  
    collie::tf::cudaStream s;
    auto p = collie::tf::cudaDefaultExecutionPolicy{s};
    ...
    auto bufsz = collie::tf::cuda_merge_buffer_size<decltype(p)>(N, N);
    collie::tf::cudaDeviceVector<std::byte> buf(bufsz);

    collie::tf::cuda_merge(p, a, a+N, b, b+N, c, collie::tf::cuda_less<T>{}, buf.data());
    s.synchronize();
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.merge(a, a+N, b, b+N, c, collie::tf::cuda_less<T>{});
      run_and_wait(cudaflow);
    });
...
TEST_CASE("cudaflow.merge_keys.int" * doctest::timeout(300)) {
  merge_keys<int, collie::tf::cudaFlow>();
}

TEST_CASE("cudaflow.merge_keys.float" * doctest::timeout(300)) {
  merge_keys<float, collie::tf::cudaFlow>();
}
...
TEST_CASE("capture.merge_keys.float" * doctest::timeout(300)) {
  merge_keys<float, collie::tf::cudaFlowCapturer>();
}
...
void merge_keys_values() {
  ...
  for(int N=0; N<=1234567; N = N*2 + 1) {
    ...
    auto a_k = collie::tf::cuda_malloc_shared<T>(N);
    auto b_k = collie::tf::cuda_malloc_shared<T>(N);
    auto c_k = collie::tf::cuda_malloc_shared<T>(2*N);
    auto a_v = collie::tf::cuda_malloc_shared<int>(N);
    auto b_v = collie::tf::cuda_malloc_shared<int>(N);
    auto c_v = collie::tf::cuda_malloc_shared<int>(2*N);
    ...
    taskflow.emplace([&](){
      ...
      cudaflow.merge_by_key(
        a_k, a_k+N, a_v, b_k, b_k+N, b_v, c_k, c_v, collie::tf::cuda_less<T>{}
      );
      run_and_wait(cudaflow);
    });
...
void sort_keys() {
  ...
  for(int N=0; N<=1234567; N = N*2 + 1) {
    ...
    auto bufsz = collie::tf::cuda_sort_buffer_size<decltype(p), T>(N);
    collie::tf::cudaDeviceVector<std::byte> buf(bufsz);
    collie::tf::cuda_sort(p, a, a+N, collie::tf::cuda_less<T>{}, buf.data());
    s.synchronize();
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.sort(a, a+N, collie::tf::cuda_less<T>{});
      run_and_wait(cudaflow);
    });
...
void sort_keys_values() {
  ...
  for(int N=1; N<=1234567; N = N*2 + 1) {
    ...
    auto a = collie::tf::cuda_malloc_shared<T>(N);
    auto b = collie::tf::cuda_malloc_shared<int>(N);
    ...
    auto bufsz = collie::tf::cuda_sort_buffer_size<decltype(p), T, int>(N);
    collie::tf::cudaDeviceVector<std::byte> buf(bufsz);
    collie::tf::cuda_sort_by_key(p, a, a+N, b, collie::tf::cuda_less<T>{}, buf.data());
    s.synchronize();
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.sort_by_key(a, a+N, b, collie::tf::cuda_greater<T>{});
      run_and_wait(cudaflow);
    });
...
void find_if() {
  ...
  for(int N=0; N<=1234567; N += std::max(N/100, 1)) {
    ...
    auto a = collie::tf::cuda_malloc_shared<T>(N);
    auto r = collie::tf::cuda_malloc_shared<unsigned>(1);
    ...
    *r = 1234;
    ...
    collie::tf::cuda_find_if(p, a, a+N, r, []__device__(int v){ return v == 5000; });
    s.synchronize();
...
void min_element() {
  ...
  for(int N=0; N<=1234567; N += std::max(N/10, 1)) {
    ...
    *r = 1234;
    ...
    collie::tf::cudaDeviceVector<std::byte> buf(
      collie::tf::cuda_min_element_buffer_size<decltype(p), T>(N)
    );

    collie::tf::cuda_min_element(
      p, a, a+N, r, collie::tf::cuda_less<T>{}, buf.data()
    );
    s.synchronize();
    ...
    *r = 1234;
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.min_element(a, a+N, r, collie::tf::cuda_less<T>{});
      run_and_wait(cudaflow);
    });
...
void max_element() {
  ...
  for(int N=0; N<=1234567; N += std::max(N/10, 1)) {
    ...
    *r = 1234;
    ...
    collie::tf::cudaDeviceVector<std::byte> buf(
      collie::tf::cuda_max_element_buffer_size<decltype(p), T>(N)
    );

    collie::tf::cuda_max_element(p, a, a+N, r, collie::tf::cuda_less<T>{}, buf.data());
    s.synchronize();
    ...
    *r = 1234;
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.max_element(a, a+N, r, collie::tf::cuda_less<T>{});
      run_and_wait(cudaflow);
    });
  ...
  for(size_t rows = 1; rows <= 7999; rows*=2+3) {
    for(size_t cols = 1; cols <= 8021; cols*=3+5) {
      ...
      auto allocate = taskflow.emplace([&]() {
       ...
       //transpose
      auto cudaflow = taskflow.emplace([&](collie::tf::cudaFlow& cf) {
        auto h2d_input_t = cf.copy(dinput_mat, hinput_mat.data(), rows * cols).name("h2d");

        auto kernel_t = collie::tf::cudaBLAF(cf).transpose(
          dinput_mat,
          doutput_mat,
          rows,
          cols
  ...
  for(int m=1; m<=1992; m=2*m+1) {
    for(int k=1; k<=1012; k=2*k+3) {
      for(int n=1; n<=1998; n=2*n+8) {
        ...
        auto cuda = taskflow.emplace([&](collie::tf::cudaFlow& cf){
          auto pa = cf.copy(da, ha, m*k);
          auto pb = cf.copy(db, hb, k*n);

          auto op = collie::tf::cudaBLAF(cf).matmul(
            da, db, dc, m, k, n 
          ).name("op");
