...
TEST_CASE("DynamicTraversal" * doctest::timeout(300)) {
  ...
  std::function<void(Node*, collie::tf::Subflow&)> traverse;

  traverse = [&] (Node* n, collie::tf::Subflow& subflow) {
    REQUIRE(!n->visited);
    n->visited = true;
    ...
    for(size_t i=0; i<S; i++) {
      if(n->successors[i]->dependents.fetch_sub(1) == 1) {
        ...
        subflow.emplace([s=n->successors[i], &traverse](collie::tf::Subflow &subflow2){
          traverse(s, subflow2);
        });
      }
    }
  ...
  for(unsigned w=1; w<=4; w++) {
    ...
    for(size_t i=0; i<src.size(); i++) {
      tf.emplace([s=src[i], &traverse](collie::tf::Subflow& subflow){
        traverse(s, subflow);
      });
    }
...
//      if(n->successors[i]->dependents.fetch_sub(1) == 1) {
//        n->successors[i]->level = ++level;
//        subflow.emplace([s=n->successors[i], &traverse](collie::tf::Subflow &subflow){
//          traverse(s, subflow);
//        });
//      }
//    }
...
/*void parallel_traversal(unsigned num_threads) {
  ...
  collie::tf::Executor executor(num_threads);
