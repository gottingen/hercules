...
struct zip_test : zip_base<typename B::value_type, B::size>
{
    ...
    void zip_low()
    {
        ...
        B b_res_lo = collie::simd::zip_lo(b_lhs, b_rhs);
    ...
    void zip_hi()
    {
        ...
        B b_res_hi = collie::simd::zip_hi(b_lhs, b_rhs);
        CHECK_BATCH_EQ(b_res_hi, b_exp_hi);
    }
};

#if !COLLIE_SIMD_AVX512F || COLLIE_SIMD_AVX512BW
#define ZIP_BATCH_TYPES BATCH_TYPES
#else
#define ZIP_BATCH_TYPES collie::simd::batch<float>, collie::simd::batch<double>, collie::simd::batch<int32_t>, collie::simd::batch<int64_t>
#endif
...
struct slide_test : public init_slide_base<typename B::value_type, B::size>
{
    ...
    void slide_left()
    {
        ...
        B b_res_left0 = collie::simd::slide_left<0>(b_in);
        INFO("slide_left 0");
        CHECK_BATCH_EQ(b_res_left0, b_left0);

        B b_res_left_full = collie::simd::slide_left<full_slide>(b_in);
        INFO("slide_left full");
        CHECK_BATCH_EQ(b_res_left_full, b_left_full);

        B b_res_left_half = collie::simd::slide_left<half_slide>(b_in);
        INFO("slide_left half_slide");
        CHECK_BATCH_EQ(b_res_left_half, b_left_half);

        B b_res_left_one = collie::simd::slide_left<sizeof(value_type)>(b_in);
        INFO("slide_left one_slide");
        CHECK_BATCH_EQ(b_res_left_one, b_left_one);

        if (activate_above_below_checks)
        {
            B b_res_left_above_half = collie::simd::slide_left<above_half_slide>(b_in);
            INFO("slide_left above_half_slide");
            CHECK_BATCH_EQ(b_res_left_above_half, b_left_above_half);

            B b_res_left_below_half = collie::simd::slide_left<below_half_slide>(b_in);
    ...
    void slide_right()
    {
        ...
        B b_res_right0 = collie::simd::slide_right<0>(b_in);
        INFO("slide_right 0");
        CHECK_BATCH_EQ(b_res_right0, b_right0);

        B b_res_right_full = collie::simd::slide_right<full_slide>(b_in);
        INFO("slide_right full");
        CHECK_BATCH_EQ(b_res_right_full, b_right_full);

        B b_res_right_half = collie::simd::slide_right<half_slide>(b_in);
        INFO("slide_right half_slide");
        CHECK_BATCH_EQ(b_res_right_half, b_right_half);

        B b_res_right_one = collie::simd::slide_right<sizeof(value_type)>(b_in);
        INFO("slide_right one_slide");
        CHECK_BATCH_EQ(b_res_right_one, b_right_one);

        if (activate_above_below_checks)
        {
            B b_res_right_above_half = collie::simd::slide_right<above_half_slide>(b_in);
            INFO("slide_right above_half_slide");
            CHECK_BATCH_EQ(b_res_right_above_half, b_right_above_half);

            B b_res_right_below_half = collie::simd::slide_right<below_half_slide>(b_in);
...
TEST_CASE_TEMPLATE("[compress]", B, BATCH_FLOAT_TYPES, collie::simd::batch<uint32_t>, collie::simd::batch<int32_t>, collie::simd::batch<uint64_t>, collie::simd::batch<int64_t>)
{
...
TEST_CASE_TEMPLATE("[expand]", B, BATCH_FLOAT_TYPES, collie::simd::batch<uint32_t>, collie::simd::batch<int32_t>, collie::simd::batch<uint64_t>, collie::simd::batch<int64_t>)
{
    expand_test<B> Test;
    SUBCASE("empty")
    {
...
struct shuffle_test
{
    using batch_type = B;
    using value_type = typename B::value_type;
    using mask_batch_type = collie::simd::as_unsigned_integer_t<B>;
    static constexpr size_t size = B::size;
    ...
    void no_op()
    {
        ...
        INFO("no op lhs");
        B b_res_lhs = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, no_op_lhs_generator>());
        ...
        INFO("no op rhs");
        B b_res_rhs = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, no_op_rhs_generator>());
    ...
    void generic()
    {
        ...
        B b_res = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, generic_generator>());
    ...
    void pick()
    {
        ...
        B b_res = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, pick_generator>());
        CHECK_BATCH_EQ(b_res, b_ref);
    }
    ...
    void swizzle()
    {
...

        {
            ...
            B b_res = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, swizzle_lo_generator>());
            CHECK_BATCH_EQ(b_res, b_ref);
        }

        {
            ...
            B b_res = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, swizzle_hi_generator>());
            CHECK_BATCH_EQ(b_res, b_ref);
        }
    }
    ...
    void select()
    {
        ...
        INFO("select");
        B b_res = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, select_generator>());
        CHECK_BATCH_EQ(b_res, b_ref);
    }
    ...
    void zip()
    {
        ...
        INFO("zip_lo");
        B b_res_lo = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, zip_lo_generator>());
        ...
        INFO("zip_hi");
        B b_res_hi = collie::simd::shuffle(b_lhs, b_rhs, collie::simd::make_batch_constant<mask_batch_type, zip_hi_generator>());
        CHECK_BATCH_EQ(b_res_hi, b_ref_hi);
    }
};

TEST_CASE_TEMPLATE("[shuffle]", B, BATCH_FLOAT_TYPES, collie::simd::batch<uint32_t>, collie::simd::batch<int32_t>, collie::simd::batch<uint64_t>, collie::simd::batch<int64_t>)
{
    shuffle_test<B> Test;
    SUBCASE("no-op")
    {
