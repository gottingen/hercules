...
#ifndef COLLIE_SIMD_TYPES_SIMD_UTILS_H_
#define COLLIE_SIMD_TYPES_SIMD_UTILS_H_
...
namespace collie::simd {
    ...
    namespace kernel {
        namespace detail {
            ...
            template<class T, size_t S>
            using enable_sized_integral_t = typename std::enable_if<
                    std::is_integral<T>::value && sizeof(T) == S, int>::type;

            template<class T, size_t S>
            using enable_sized_t = typename std::enable_if<sizeof(T) == S, int>::type;

            template<class T, size_t S>
            using enable_max_sized_integral_t = typename std::enable_if<
                    std::is_integral<T>::value && sizeof(T) <= S, int>::type;
            ...
            template<class T, class U, class B = int>
            using stride_match_t = typename std::enable_if<
                    !std::is_same<T, U>::value && sizeof(T) == sizeof(U), B>::type;
        } // namespace detail
    } // namespace kernel
    ...
    // This is used in both simd_complex_base.hpp and simd_traits.hpp
    // However simd_traits.hpp indirectly includes simd_complex_base.hpp
    // so we cannot define is_complex in simd_traits.hpp. Besides, if
    // no file defining batches is included, we still need this definition
    // in simd_traits.hpp, so let's define it here.
...
#endif  // COLLIE_SIMD_TYPES_SIMD_UTILS_H_
