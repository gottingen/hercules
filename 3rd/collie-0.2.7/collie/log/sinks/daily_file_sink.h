...
struct daily_filename_calculator {
    ...
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm) {
        ...
        return fmt_lib::format(FMT_STRING(CLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}{}")),
...
public:
    ...
    daily_file_sink(filename_t base_filename,
        ...
        if (rotation_hour < 0 || rotation_hour > 23 || rotation_minute < 0 ||
            rotation_minute > 59) {
            throw_clog_ex("daily_file_sink: Invalid rotation time in ctor");
        }
...
private:
    ...
    // Throw CLogEx on failure to delete the old file.
    void delete_old_() {
        ...
        if (filenames_q_.full()) {
            ...
            if (!ok) {
                ...
                throw_clog_ex("Failed removing daily file " + filename_to_str(old_filename),
                                errno);
            }
        }
        filenames_q_.push_back(std::move(current_file));
