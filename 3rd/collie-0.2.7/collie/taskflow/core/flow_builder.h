...
#include <collie/taskflow/algorithm/partitioner.h>
...
namespace collie::tf {
    ...
    from which collie::tf::Taskflow and collie::tf::Subflow are derived.
    ...
    */
    ...
    public:
        ...
        */
        ...
        @return a collie::tf::Task handle

        The following example creates a static task.

        @code{.cpp}
        collie::tf::Task static_task = taskflow.emplace([](){});
        @endcode
        ...
        */
        ...
        @tparam C callable type constructible from std::function<void(collie::tf::Subflow&)>

        @param callable callable to construct a dynamic task

        @return a collie::tf::Task handle

        The following example creates a dynamic task (collie::tf::Subflow)
        that spawns two static tasks.

        @code{.cpp}
        collie::tf::Task dynamic_task = taskflow.emplace([](collie::tf::Subflow& sf){
          collie::tf::Task static_task1 = sf.emplace([](){});
          collie::tf::Task static_task2 = sf.emplace([](){});
        });
        @endcode
        ...
        */
        ...
        @tparam C callable type constructible from
                std::function<collie::InlinedVector<int>()>
        ...
        auto [init, cond, branch1, branch2, branch3] = taskflow.emplace(
          [] () { },
          [] () { return collie::InlinedVector{0, 2}; },
          [] () { std::cout << "branch1\n"; },
          [] () { std::cout << "branch2\n"; },
          [] () { std::cout << "branch3\n"; }
        );
        ...
        */
        ...
        @code{.cpp}
        collie::tf::Task A = taskflow.emplace([](){ std::cout << "A"; });
        collie::tf::Task B = taskflow.emplace([](){ std::cout << "B"; });
        collie::tf::Task C = taskflow.emplace([](){ std::cout << "C"; });
        collie::tf::Task D = taskflow.emplace([](){ std::cout << "D"; });
        A.precede(B, C, D);
        ...
        */
        ...
        @code{.cpp}
        collie::tf::Taskflow t1, t2;
        t1.emplace([](){ std::cout << "t1"; });

        // t2 is partially composed of t1
        collie::tf::Task comp = t2.composed_of(t1);
        collie::tf::Task init = t2.emplace([](){ std::cout << "t2"; });
        init.precede(comp);
        @endcode
        ...
        type collie::tf::Graph such that it can interact with the executor.
        For example:

        @code{.cpp}
        // custom struct
        struct MyObj {
          collie::tf::Graph graph;
          MyObj() {
            collie::tf::FlowBuilder builder(graph);
            collie::tf::Task task = builder.emplace([](){
              std::cout << "a task\n";  // static task
            });
          }
        ...
        MyObj obj;
        collie::tf::Task comp = taskflow.composed_of(obj);
        @endcode
        ...
        */
        ...
        collie::tf::Task placeholder = taskflow.placeholder();
        ...
        */
        ...
        collie::tf::Task C = taskflow.emplace([](){ std::cout << "C"; });
        collie::tf::Task D = taskflow.emplace([](){ std::cout << "D"; });
        std::vector<collie::tf::Task> tasks {A, B, C, D}
        ...
        */
        ...
        @tparam P partitioner type (default collie::tf::DefaultPartitioner)
        ...
        */
        ...
        @param part partitioning algorithm (default collie::tf::DefaultPartitioner)
    ...
    collie::tf::Subflow is a derived class from collie::tf::Runtime with a specialized mechanism
    to manage the execution of a child graph.
    By default, a subflow automatically @em joins its parent node.
    You may explicitly join or detach a subflow by calling collie::tf::Subflow::join
    or collie::tf::Subflow::detach, respectively.
    ...
    collie::tf::Task A = taskflow.emplace([](){}).name("A");
    collie::tf::Task C = taskflow.emplace([](){}).name("C");
    collie::tf::Task D = taskflow.emplace([](){}).name("D");

    // create a subflow graph (dynamic tasking)
    collie::tf::Task B = taskflow.emplace([] (collie::tf::Subflow& subflow) {
      collie::tf::Task B1 = subflow.emplace([](){}).name("B1");
      collie::tf::Task B2 = subflow.emplace([](){}).name("B2");
      collie::tf::Task B3 = subflow.emplace([](){}).name("B3");
      B1.precede(B3);
      B2.precede(B3);
    }).name("B");
