...
/*int main() {
  ...
  collie::tf::Task allocate = taskflow.emplace(
    [&](){ data = sycl::malloc_shared<int>(N, queue); }
  );

  // create a syclFlow task to add 2 to each element of the vector
  collie::tf::Task syclFlow = taskflow.emplace_on([&](collie::tf::syclFlow& sf){

    collie::tf::syclTask fill = sf.fill(data, 100, N);

    collie::tf::syclTask plus = sf.parallel_for(
  ...
  collie::tf::Task deallocate = taskflow.emplace([&](){
