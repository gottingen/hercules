...
int main(int argc, char **argv) {
    ...
    collie::App *start = app.add_subcommand("start", "A great subcommand");
    collie::App *stop = app.add_subcommand("stop", "Do you really want to stop?");
    app.require_subcommand();  // 1 or more

    std::string file;
    start->add_option("-f,--file", file, "File name");

    collie::Option *s = stop->add_flag("-c,--count", "Counter");
