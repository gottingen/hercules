...
class ConfigJSON : public collie::Config {
  public:
    std::string to_config(const collie::App *app, bool default_also, bool, std::string) const override {

        json j;

        for(const collie::Option *opt : app->get_options({})) {
        ...
        for(const collie::App *subcom : app->get_subcommands({}))
            j[subcom->get_name()] = json(to_config(subcom, default_also, false, ""));

        return j.dump(4);
    }

    std::vector<collie::ConfigItem> from_config(std::istream &input) const override {
        json j;
        input >> j;
        return _from_config(j);
    }

    std::vector<collie::ConfigItem>
    _from_config(json j, std::string name = "", std::vector<std::string> prefix = {}) const {
        std::vector<collie::ConfigItem> results;
        ...
        if(j.is_object()) {
            ...
            collie::ConfigItem &res = results.back();
            res.name = name;
            res.parents = prefix;
            if(j.is_boolean()) {
                ...
                throw collie::ConversionError("Failed to convert " + name);
            }
        } else {
            throw collie::ConversionError("You must make all top level values objects in json!");
        }
