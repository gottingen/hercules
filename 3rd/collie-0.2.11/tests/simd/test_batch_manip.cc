...
struct as_index
{
    using type = collie::simd::as_unsigned_integer_t<T>;
};

template <class T, class A>
struct as_index<collie::simd::batch<std::complex<T>, A>> : as_index<collie::simd::batch<T, A>>
{
};
...
struct insert_test
{
    ...
    void insert_first()
    {
        ...
        batch_type w = insert(v, sentinel_value, ::collie::simd::index<0>());
    ...
    void insert_last()
    {
        ...
        value_type sentinel_value = 1;
        batch_type v(fill_value);
        batch_type w = insert(v, sentinel_value, ::collie::simd::index<batch_type::size - 1>());
        std::array<value_type, batch_type::size> data;
        w.store_unaligned(data.data());
...
struct swizzle_test
{
    ...
    void rotate_right()
    {
        collie::simd::init_swizzle_base<value_type, size> swizzle_base;
        ...
        B b_res = collie::simd::rotate_right<1>(b_lhs);
    ...
    void rotate_left()
    {
        ...
        B b_res = collie::simd::rotate_left<1>(b_lhs);
        CHECK_BATCH_EQ(b_res, b_exped);
    }
    ...
    void swizzle_reverse()
    {
        ...
        auto index_batch = collie::simd::make_batch_constant<index_type, Reversor<typename index_type::value_type>>();

        B b_res = collie::simd::swizzle(b_lhs, index_batch);
        CHECK_BATCH_EQ(b_res, b_exped);

        B b_dyres = collie::simd::swizzle(b_lhs, (index_type)index_batch);
    ...
    void swizzle_fill()
    {
        ...
        using index_type = typename as_index<batch_type>::type;
        auto index_batch = collie::simd::make_batch_constant<index_type, Last<typename index_type::value_type>>();
    ...
    void swizzle_dup()
    {
        ...
        using index_type = typename as_index<batch_type>::type;
        auto index_batch = collie::simd::make_batch_constant<index_type, Dup<typename index_type::value_type>>();
