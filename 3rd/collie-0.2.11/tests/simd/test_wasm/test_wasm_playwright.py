...
async def playwright_run_page(page_url, headless=True, slow_mo=None):
    async with async_playwright() as p:
        ...
        status = await page.evaluate(
            f"""async () => {{
                let test_module = await test_simd_wasm();
                console.log("\\n\\n************************************************************");
                console.log("SIMD WASM TESTS:");
                console.log("************************************************************");
                let r = test_module.run_tests();
                if (r == 0) {{
                console.log("\\n\\n************************************************************");
                console.log("SIMD WASM TESTS PASSED");
                console.log("************************************************************");
                return r;
                }}
                else {{
                console.log("************************************************************");
                console.log("SIMD WASM TESTS FAILED");
                console.log("************************************************************");
                return r;
                }}
...
def main(build_dir):
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        shutil.copy(f"{build_dir}/test_simd.wasm", work_dir)
        shutil.copy(f"{build_dir}/test_simd.js", work_dir)
