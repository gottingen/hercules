...
TEST_CASE("ParallelTransform.Guided.1thread") {
  parallel_transform<std::vector<int>, collie::tf::GuidedPartitioner<>>(1);
  parallel_transform<std::list<int>, collie::tf::GuidedPartitioner<>>(1);
}

TEST_CASE("ParallelTransform.Guided.2threads") {
  parallel_transform<std::vector<int>, collie::tf::GuidedPartitioner<>>(2);
  parallel_transform<std::list<int>, collie::tf::GuidedPartitioner<>>(2);
}

TEST_CASE("ParallelTransform.Guided.3threads") {
  parallel_transform<std::vector<int>, collie::tf::GuidedPartitioner<>>(3);
  parallel_transform<std::list<int>, collie::tf::GuidedPartitioner<>>(3);
}

TEST_CASE("ParallelTransform.Guided.4threads") {
  parallel_transform<std::vector<int>, collie::tf::GuidedPartitioner<>>(4);
  parallel_transform<std::list<int>, collie::tf::GuidedPartitioner<>>(4);
}

// random
TEST_CASE("ParallelTransform.Random.1thread") {
  parallel_transform<std::vector<int>, collie::tf::RandomPartitioner<>>(1);
  parallel_transform<std::list<int>, collie::tf::RandomPartitioner<>>(1);
}

TEST_CASE("ParallelTransform.Random.2threads") {
  parallel_transform<std::vector<int>, collie::tf::RandomPartitioner<>>(2);
  parallel_transform<std::list<int>, collie::tf::RandomPartitioner<>>(2);
}

TEST_CASE("ParallelTransform.Random.3threads") {
  parallel_transform<std::vector<int>, collie::tf::RandomPartitioner<>>(3);
  parallel_transform<std::list<int>, collie::tf::RandomPartitioner<>>(3);
}

TEST_CASE("ParallelTransform.Random.4threads") {
  parallel_transform<std::vector<int>, collie::tf::RandomPartitioner<>>(4);
  parallel_transform<std::list<int>, collie::tf::RandomPartitioner<>>(4);
}

// static
TEST_CASE("ParallelTransform.Static.1thread") {
  parallel_transform<std::vector<int>, collie::tf::StaticPartitioner<>>(1);
  parallel_transform<std::list<int>, collie::tf::StaticPartitioner<>>(1);
}

TEST_CASE("ParallelTransform.Static.2threads") {
  parallel_transform<std::vector<int>, collie::tf::StaticPartitioner<>>(2);
  parallel_transform<std::list<int>, collie::tf::StaticPartitioner<>>(2);
}

TEST_CASE("ParallelTransform.Static.3threads") {
  parallel_transform<std::vector<int>, collie::tf::StaticPartitioner<>>(3);
  parallel_transform<std::list<int>, collie::tf::StaticPartitioner<>>(3);
}

TEST_CASE("ParallelTransform.Static.4threads") {
  parallel_transform<std::vector<int>, collie::tf::StaticPartitioner<>>(4);
  parallel_transform<std::list<int>, collie::tf::StaticPartitioner<>>(4);
}
...
TEST_CASE("ParallelTransform2.Guided.1thread") {
  parallel_transform2<std::vector<int>, collie::tf::GuidedPartitioner<>>(1);
  parallel_transform2<std::list<int>, collie::tf::GuidedPartitioner<>>(1);
}

TEST_CASE("ParallelTransform2.Guided.2threads") {
  parallel_transform2<std::vector<int>, collie::tf::GuidedPartitioner<>>(2);
  parallel_transform2<std::list<int>, collie::tf::GuidedPartitioner<>>(2);
}

TEST_CASE("ParallelTransform2.Guided.3threads") {
  parallel_transform2<std::vector<int>, collie::tf::GuidedPartitioner<>>(3);
  parallel_transform2<std::list<int>, collie::tf::GuidedPartitioner<>>(3);
}

TEST_CASE("ParallelTransform2.Guided.4threads") {
  parallel_transform2<std::vector<int>, collie::tf::GuidedPartitioner<>>(4);
  parallel_transform2<std::list<int>, collie::tf::GuidedPartitioner<>>(4);
}

// dynamic
TEST_CASE("ParallelTransform2.Dynamic.1thread") {
  parallel_transform2<std::vector<int>, collie::tf::DynamicPartitioner<>>(1);
  parallel_transform2<std::list<int>, collie::tf::DynamicPartitioner<>>(1);
}

TEST_CASE("ParallelTransform2.Dynamic.2threads") {
  parallel_transform2<std::vector<int>, collie::tf::DynamicPartitioner<>>(2);
  parallel_transform2<std::list<int>, collie::tf::DynamicPartitioner<>>(2);
}

TEST_CASE("ParallelTransform2.Dynamic.3threads") {
  parallel_transform2<std::vector<int>, collie::tf::DynamicPartitioner<>>(3);
  parallel_transform2<std::list<int>, collie::tf::DynamicPartitioner<>>(3);
}

TEST_CASE("ParallelTransform2.Dynamic.4threads") {
  parallel_transform2<std::vector<int>, collie::tf::DynamicPartitioner<>>(4);
  parallel_transform2<std::list<int>, collie::tf::DynamicPartitioner<>>(4);
}

// static
TEST_CASE("ParallelTransform2.Static.1thread") {
  parallel_transform2<std::vector<int>, collie::tf::StaticPartitioner<>>(1);
  parallel_transform2<std::list<int>, collie::tf::StaticPartitioner<>>(1);
}

TEST_CASE("ParallelTransform2.Static.2threads") {
  parallel_transform2<std::vector<int>, collie::tf::StaticPartitioner<>>(2);
  parallel_transform2<std::list<int>, collie::tf::StaticPartitioner<>>(2);
}

TEST_CASE("ParallelTransform2.Static.3threads") {
  parallel_transform2<std::vector<int>, collie::tf::StaticPartitioner<>>(3);
  parallel_transform2<std::list<int>, collie::tf::StaticPartitioner<>>(3);
}

TEST_CASE("ParallelTransform2.Static.4threads") {
  parallel_transform2<std::vector<int>, collie::tf::StaticPartitioner<>>(4);
  parallel_transform2<std::list<int>, collie::tf::StaticPartitioner<>>(4);
}

// random
TEST_CASE("ParallelTransform2.Random.1thread") {
  parallel_transform2<std::vector<int>, collie::tf::RandomPartitioner<>>(1);
  parallel_transform2<std::list<int>, collie::tf::RandomPartitioner<>>(1);
}

TEST_CASE("ParallelTransform2.Random.2threads") {
  parallel_transform2<std::vector<int>, collie::tf::RandomPartitioner<>>(2);
  parallel_transform2<std::list<int>, collie::tf::RandomPartitioner<>>(2);
}

TEST_CASE("ParallelTransform2.Random.3threads") {
  parallel_transform2<std::vector<int>, collie::tf::RandomPartitioner<>>(3);
  parallel_transform2<std::list<int>, collie::tf::RandomPartitioner<>>(3);
}

TEST_CASE("ParallelTransform2.Random.4threads") {
  parallel_transform2<std::vector<int>, collie::tf::RandomPartitioner<>>(4);
  parallel_transform2<std::list<int>, collie::tf::RandomPartitioner<>>(4);
}
...
void parallel_transform3(size_t W) {
  ...
  for(size_t N=0; N<1000; N=(N+1)*2) {
    ...
    auto to_tgt = taskflow.emplace([&](collie::tf::Subflow& subflow) {
...
TEST_CASE("ParallelTransform3.Guided.1thread") {
  parallel_transform3<collie::tf::GuidedPartitioner<>>(1);
  parallel_transform3<collie::tf::GuidedPartitioner<>>(1);
}

TEST_CASE("ParallelTransform3.Guided.2threads") {
  parallel_transform3<collie::tf::GuidedPartitioner<>>(2);
  parallel_transform3<collie::tf::GuidedPartitioner<>>(2);
}

TEST_CASE("ParallelTransform3.Guided.3threads") {
  parallel_transform3<collie::tf::GuidedPartitioner<>>(3);
  parallel_transform3<collie::tf::GuidedPartitioner<>>(3);
}

TEST_CASE("ParallelTransform3.Guided.4threads") {
  parallel_transform3<collie::tf::GuidedPartitioner<>>(4);
  parallel_transform3<collie::tf::GuidedPartitioner<>>(4);
}

// dynamic
TEST_CASE("ParallelTransform3.Dynamic.1thread") {
  parallel_transform3<collie::tf::DynamicPartitioner<>>(1);
  parallel_transform3<collie::tf::DynamicPartitioner<>>(1);
}

TEST_CASE("ParallelTransform3.Dynamic.2threads") {
  parallel_transform3<collie::tf::DynamicPartitioner<>>(2);
  parallel_transform3<collie::tf::DynamicPartitioner<>>(2);
}

TEST_CASE("ParallelTransform3.Dynamic.3threads") {
  parallel_transform3<collie::tf::DynamicPartitioner<>>(3);
  parallel_transform3<collie::tf::DynamicPartitioner<>>(3);
}

TEST_CASE("ParallelTransform3.Dynamic.4threads") {
  parallel_transform3<collie::tf::DynamicPartitioner<>>(4);
  parallel_transform3<collie::tf::DynamicPartitioner<>>(4);
}

// static
TEST_CASE("ParallelTransform3.Static.1thread") {
  parallel_transform3<collie::tf::StaticPartitioner<>>(1);
  parallel_transform3<collie::tf::StaticPartitioner<>>(1);
}

TEST_CASE("ParallelTransform3.Static.2threads") {
  parallel_transform3<collie::tf::StaticPartitioner<>>(2);
  parallel_transform3<collie::tf::StaticPartitioner<>>(2);
}

TEST_CASE("ParallelTransform3.Static.3threads") {
  parallel_transform3<collie::tf::StaticPartitioner<>>(3);
  parallel_transform3<collie::tf::StaticPartitioner<>>(3);
}

TEST_CASE("ParallelTransform3.Static.4threads") {
  parallel_transform3<collie::tf::StaticPartitioner<>>(4);
  parallel_transform3<collie::tf::StaticPartitioner<>>(4);
}

// random
TEST_CASE("ParallelTransform3.Random.1thread") {
  parallel_transform3<collie::tf::RandomPartitioner<>>(1);
  parallel_transform3<collie::tf::RandomPartitioner<>>(1);
}

TEST_CASE("ParallelTransform3.Random.2threads") {
  parallel_transform3<collie::tf::RandomPartitioner<>>(2);
  parallel_transform3<collie::tf::RandomPartitioner<>>(2);
}

TEST_CASE("ParallelTransform3.Random.3threads") {
  parallel_transform3<collie::tf::RandomPartitioner<>>(3);
  parallel_transform3<collie::tf::RandomPartitioner<>>(3);
}

TEST_CASE("ParallelTransform3.Random.4threads") {
  parallel_transform3<collie::tf::RandomPartitioner<>>(4);
  parallel_transform3<collie::tf::RandomPartitioner<>>(4);
}
...
{
  // Write a test case for using the taskwrapper on collie::tf::transform
  for (int tc = 1; tc < 16; tc++)
  {
    ...
    taskflow.transform(range.begin(), range.end(), begin(result), 
      ...
      collie::tf::StaticPartitioner(1, [&](auto&& task){
...
TEST_CASE("ClosureWrapper.transform.Dynamic" * doctest::timeout(300))
{
  for (int tc = 1; tc < 16; tc++)
  {
    ...
    taskflow.transform(range.begin(), range.end(), begin(result), 
      ...
      collie::tf::DynamicPartitioner(1, [&](auto&& task) {
        wrapper_called_count++;
        GetThreadSpecificContext() = tc;
        task();
        GetThreadSpecificContext() = 0;
