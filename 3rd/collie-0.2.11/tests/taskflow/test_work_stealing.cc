...
void tsq_owner() {
  ...
  for(size_t N=1; N<=777777; N=N*2+1) {
    collie::tf::TaskQueue<void*> queue;
...
void priority_tsq_owner() {
  ...
  collie::tf::TaskQueue<void*, P> queue;
...
void starvation_loop_test(size_t W) {
  ...
  collie::tf::Task prev, curr, second;
  ...
  for(size_t b=0; b<B; b++) {
    collie::tf::Task task = taskflow.emplace([&](){
      // record the threads
      {
...
void subflow_starvation_test(size_t W) {
  ...
  taskflow.emplace([&](collie::tf::Subflow& subflow){
    ...
    // fork
    for(size_t b=0; b<B; b++) {
      collie::tf::Task task = subflow.emplace([&](){
        // record the threads
        {
          std::scoped_lock lock(mutex);
          set.insert(executor.this_worker_id());
...
void embarrasing_starvation_test(size_t W) {
  ...
  collie::tf::Taskflow taskflow, parent;
  collie::tf::Executor executor(W);
...
void skewed_starvation(size_t W) {
  ...
  collie::tf::Task parent = taskflow.emplace([&](){ 
    set.clear();
    count.store(0, std::memory_order_relaxed);
    stop.store(false, std::memory_order_relaxed);
  }).name("root");

  collie::tf::Task left, right;
...
void nary_starvation(size_t W) {
  ...
  for(size_t w=0; w<W; w++) {
    ...
    for(size_t i=0; i<N; i++) {
      collie::tf::Task task = taskflow.emplace([&, w, p, i](){
...
void wavefront_starvation(size_t W) {
  ...
  std::vector<std::vector<collie::tf::Task>> G;
  G.resize(W);
