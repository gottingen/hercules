...
TEST_CASE("moved_run") {
  ...
  collie::tf::Executor().run(taskflow).wait();
  REQUIRE(counter == 3*N);
  ...
  // run the moved taskflow
  collie::tf::Executor().run(std::move(taskflow)).wait();
  REQUIRE(counter == 6*N);
  REQUIRE(taskflow.num_tasks() == 0);
  ...
  make_taskflow();
  REQUIRE(taskflow.num_tasks() == N);
  collie::tf::Executor().run_n(std::move(taskflow), 3).wait();
  REQUIRE(counter == 10*N);
  REQUIRE(taskflow.num_tasks() == 0);

  // run the moved empty taskflow with callable
  collie::tf::Executor().run(std::move(taskflow), [&](){
...
TEST_CASE("moved_taskflows") {
  ...
  auto make_taskflow = [&counter](collie::tf::Taskflow& taskflow, int N){
    for(int i=0; i<N; i++) {
  ...
  {
    collie::tf::Taskflow taskflow1;
    collie::tf::Taskflow taskflow2;
  ...
  {
    collie::tf::Taskflow taskflow1;
    collie::tf::Taskflow taskflow2(std::move(taskflow1));

    REQUIRE(taskflow1.num_tasks() == 0);
    REQUIRE(taskflow2.num_tasks() == 0);

    make_taskflow(taskflow1, N);
    collie::tf::Taskflow taskflow3(std::move(taskflow1));
