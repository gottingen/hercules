...
void multicondition_task_exception(unsigned W) {
  ...
  auto A = taskflow.emplace([&](){ 
    collie::InlinedVector<int> ret = {0};
    counter.fetch_add(1, std::memory_order_relaxed); 
    return ret;
  });
  auto B = taskflow.emplace([&](){ 
...
void runtime_async_task_exception(unsigned W) {
  ...
  // runtime async
  auto A = taskflow.emplace([](collie::tf::Runtime& rt){
  ...
  flag = 0;
  taskflow.clear();
  A = taskflow.emplace([&](collie::tf::Runtime& rt){
...
void joined_subflow_exception_1(unsigned W) {
  ...
  taskflow.emplace([&] (collie::tf::Subflow& sf0) {
    for (int i = 0; i < 16; ++i) {
      sf0.emplace([&] (collie::tf::Subflow& sf1) {
        for (int j = 0; j < 16; ++j) {
          sf1.emplace([] () {
            throw std::runtime_error("x");
          });
...
void executor_corun_exception(unsigned W) {
  ...
  for(size_t i=0; i<100; i++) {
    taskflow1.emplace([](collie::tf::Subflow& sf){
...
void runtime_corun_exception(unsigned W) {
  ...
  auto task = taskflow2.emplace([&](collie::tf::Runtime& rt){
  ...
  task.work([&](collie::tf::Runtime& rt){
    rt.corun(taskflow1);
  });
