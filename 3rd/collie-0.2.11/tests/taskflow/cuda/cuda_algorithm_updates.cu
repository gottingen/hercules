...
#include <collie/taskflow/cuda/algorithm/sort.hpp>
...
TEST_CASE("cudaFlow.offload" * doctest::timeout(300)) {
  offload_n<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.offload" * doctest::timeout(300)) {
  offload_n<collie::tf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.join" * doctest::timeout(300)) {
  join<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.join" * doctest::timeout(300)) {
  join<collie::tf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.update.for_each_index" * doctest::timeout(300)) {
  update_for_each_index<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.for_each_index" * doctest::timeout(300)) {
  update_for_each_index<collie::tf::cudaFlowCapturer>();
}
...
void update_reduce() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto data = collie::tf::cuda_malloc_shared<int>(N);
    auto soln = collie::tf::cuda_malloc_shared<int>(1);
...
TEST_CASE("cudaFlow.update.uninitialized_reduce" * doctest::timeout(300)) {
  update_uninitialized_reduce<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.uninitialized_reduce" * doctest::timeout(300)) {
  update_uninitialized_reduce<collie::tf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.update.transform_reduce" * doctest::timeout(300)) {
  update_transform_reduce<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.transform_reduce" * doctest::timeout(300)) {
  update_transform_reduce<collie::tf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.update.transform_uninitialized_reduce" * doctest::timeout(300)) {
  update_transform_uninitialized_reduce<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.transform_uninitialized_reduce" * doctest::timeout(300)) {
  update_transform_uninitialized_reduce<collie::tf::cudaFlowCapturer>();
}
...
void update_transform() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto input1 = collie::tf::cuda_malloc_shared<int>(N);
    auto input2 = collie::tf::cuda_malloc_shared<int>(N);
    ...
    collie::tf::cuda_free(input1);
    collie::tf::cuda_free(input2);
    collie::tf::cuda_free(output);
  }
}

TEST_CASE("cudaFlow.update.transform" * doctest::timeout(300)) {
  update_transform<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.transform" * doctest::timeout(300)) {
  update_transform<collie::tf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.update.binary_transform" * doctest::timeout(300)) {
  update_binary_transform<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.binary_transform" * doctest::timeout(300)) {
  update_binary_transform<collie::tf::cudaFlowCapturer>();
}
...
void update_scan() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto input1 = collie::tf::cuda_malloc_shared<int>(N);
    auto input2 = collie::tf::cuda_malloc_shared<int>(N);
    auto output1 = collie::tf::cuda_malloc_shared<int>(N);
    auto output2 = collie::tf::cuda_malloc_shared<int>(N);
    ...
    collie::tf::cuda_free(input1);
    collie::tf::cuda_free(input2);
    collie::tf::cuda_free(output1);
    collie::tf::cuda_free(output2);
  }
}

TEST_CASE("cudaFlow.update.scan" * doctest::timeout(300)) {
  update_scan<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.scan" * doctest::timeout(300)) {
  update_scan<collie::tf::cudaFlowCapturer>();
}
...
void update_merge() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto input1 = collie::tf::cuda_malloc_shared<int>(N);
    auto input2 = collie::tf::cuda_malloc_shared<int>(2*N);
    auto output1 = collie::tf::cuda_malloc_shared<int>(3*N);
    auto output2 = collie::tf::cuda_malloc_shared<int>(3*N);
    ...
    // merge
    auto merge = cf.merge(
      input1, input1+N, input2, input2+2*N, output1, collie::tf::cuda_less<int>()
    );
    cf.offload();
    ...
    // update merge
    cf.merge(merge,
      input1, input1+N, input2, input2+N, output2, collie::tf::cuda_less<int>()
    );
    cf.offload();
...
TEST_CASE("cudaFlow.update.merge" * doctest::timeout(300)) {
  update_merge<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.merge" * doctest::timeout(300)) {
  update_merge<collie::tf::cudaFlowCapturer>();
}
...
void update_sort() {
  ...
  for(int N=1; N<=100000; N += (N/100+1)) {
    ...
    cf.sort(sort, input1, input1+N, collie::tf::cuda_greater<int>());
    cf.offload();
...
TEST_CASE("cudaFlow.update.sort" * doctest::timeout(300)) {
  update_sort<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.sort" * doctest::timeout(300)) {
  update_sort<collie::tf::cudaFlowCapturer>();
}
...
void update_sort_by_key() {
  ...
  for(int N=1; N<=100000; N += (N/100+1)) {
    ...
    auto input1 = collie::tf::cuda_malloc_shared<int>(N);
    auto input2 = collie::tf::cuda_malloc_shared<int>(N);
    auto index1 = collie::tf::cuda_malloc_shared<int>(N);
    auto index2 = collie::tf::cuda_malloc_shared<int>(N);
    ...
    collie::tf::cuda_free(input1);
    collie::tf::cuda_free(input2);
    collie::tf::cuda_free(index1);
    collie::tf::cuda_free(index2);
  }
}

TEST_CASE("cudaFlow.update.sort_by_key" * doctest::timeout(300)) {
  update_sort_by_key<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.sort_by_key" * doctest::timeout(300)) {
  update_sort_by_key<collie::tf::cudaFlowCapturer>();
}
...
void update_find() {
  ...
  for(unsigned N=1; N<=100000; N += (N/100+1)) {
    ...
    auto input1 = collie::tf::cuda_malloc_shared<int>(N);
    auto input2 = collie::tf::cuda_malloc_shared<int>(N);
    auto index1 = collie::tf::cuda_malloc_shared<unsigned>(1);
    auto index2 = collie::tf::cuda_malloc_shared<unsigned>(1);
...
TEST_CASE("cudaFlow.update.find" * doctest::timeout(300)) {
  update_find<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.find" * doctest::timeout(300)) {
  update_find<collie::tf::cudaFlowCapturer>();
}
...
void update_minmax_element() {
  ...
  for(unsigned N=1; N<=100000; N += (N/100+1)) {
    ...
    // create find
    auto find_min = cf.min_element(input1, input1+N, index1, collie::tf::cuda_less<int>());
    auto find_max = cf.max_element(input2, input2+N, index2, collie::tf::cuda_less<int>());
    cf.offload();
    ...
    // update find
    cf.min_element(find_min, input2, input2+N, index2, collie::tf::cuda_less<int>());
    cf.max_element(find_max, input1, input1+N, index1, collie::tf::cuda_less<int>());
    cf.offload();
...
TEST_CASE("cudaFlow.update.minmax_element" * doctest::timeout(300)) {
  update_minmax_element<collie::tf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.minmax_element" * doctest::timeout(300)) {
  update_minmax_element<collie::tf::cudaFlowCapturer>();
}
