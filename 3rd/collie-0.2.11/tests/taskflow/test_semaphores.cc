...
void critical_section(size_t W) {
  ...
  collie::tf::CriticalSection section(1);

  int N = 1000;
  int counter = 0;

  for(int i=0; i<N; ++i) {
    collie::tf::Task task = taskflow.emplace([&](){ counter++; })
...
void overlapped_semaphore(size_t W) {
  ...
  collie::tf::Executor executor(W);
  collie::tf::Taskflow taskflow;
  collie::tf::Semaphore semaphore1(1);
  collie::tf::Semaphore semaphore4(4);
...
void conflict_graph(size_t W) {
  ...
  collie::tf::Executor executor(W);
  collie::tf::Taskflow taskflow;
  collie::tf::Semaphore conflict_AB(1);
  collie::tf::Semaphore conflict_AC(1);

  int counter {0};
  std::mutex mutex;

  collie::tf::Task A = taskflow.emplace([&](){ counter++; });

  // B and C can run together
  collie::tf::Task B = taskflow.emplace([&](){
