...
// --------------------------------------------------------
void pipeline_1P(size_t L, unsigned w, collie::tf::PipeType type) {
  ...
  for (size_t N = 0; N <= maxN; N++) {
    ...
    if (type == collie::tf::PipeType::SERIAL) {
      ...
      size_t j = 0;
      collie::tf::Pipeline pl (L, collie::tf::Pipe{type, [L, N, &j, &source](auto& pf) mutable {
        if (j == N) {
          pf.stop();
          return;
        }
    ...
    // parallel pipe
    //else if(type == collie::tf::PipeType::PARALLEL) {
    //
    //  collie::tf::Taskflow taskflow;
    ...
    //  collie::tf::Pipeline pl(L, collie::tf::Pipe{type,
...
void pipeline_2P_SS(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(
      L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j1, &mybuffer, L](auto& pf) mutable {
        if(j1 == N) {
          pf.stop();
          return;
        }
      ...
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j2, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j2 < N);
        REQUIRE(pf.token() % L == pf.line());
...
void pipeline_2P_SP(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(L,
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL,
...
void pipeline_3P_SSS(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(L,
      ...
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j3, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j3 < N);
...
void pipeline_3P_SSP(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(L,
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [N, &j3, &mutex, &collection, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j3++ < N);
        {
          std::scoped_lock<std::mutex> lock(mutex);
          REQUIRE(pf.token() % L == pf.line());
...
void pipeline_3P_SPS(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(L,
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [N, &j2, &mutex, &collection, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j2++ < N);
...
void pipeline_3P_SPP(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    collie::tf::Pipeline pl(L,
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [N, &j2, &mutex2, &collection2, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j2++ < N);
        //*pf.output() = *pf.input() + 1;
        {
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [N, &j3, &mutex3, &collection3, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j3++ < N);
        {
...
void three_parallel_pipelines(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    // pipeline 1 is SSSS
    collie::tf::Pipeline pl1(L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j1_1, &mybuffer1, L](auto& pf) mutable {
        if(j1_1 == N) {
          pf.stop();
          return;
        }
      ...
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j1_2, &mybuffer1, L](auto& pf) mutable {
        REQUIRE(j1_2 < N);
        REQUIRE(pf.token() % L == pf.line());
      ...
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j1_3, &mybuffer1, L](auto& pf) mutable {
        REQUIRE(j1_3 < N);
        REQUIRE(pf.token() % L == pf.line());
      ...
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j1_4, &mybuffer1, L](auto& pf) mutable {
        REQUIRE(j1_4 < N);
        REQUIRE(pf.token() % L == pf.line());
    ...
    // pipeline 2 is SSP
    collie::tf::Pipeline pl2(L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j2_1, &mybuffer2, L](auto& pf) mutable {
        if(j2_1 == N) {
          pf.stop();
          return;
        }
      ...
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j2_2, &mybuffer2, L](auto& pf) mutable {
        REQUIRE(j2_2 < N);
      ...
      collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [N, &j2_3, &mutex2_3, &collection2_3, &mybuffer2, L](auto& pf) mutable {
        REQUIRE(j2_3++ < N);
        {
    ...
    // pipeline 3 is SP
    collie::tf::Pipeline pl3(L,
      collie::tf::Pipe{collie::tf::PipeType::SERIAL, [N, &source, &j3_1, &mybuffer3, L](auto& pf) mutable {
        if(j3_1 == N) {
          pf.stop();
          return;
        }
...
void looping_pipelines(size_t L, unsigned w) {
  ...
  collie::tf::Pipeline pl(L,
    ...
    collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [&N, &j4, &mutex4, &collection4, &mybuffer, L](auto& pf) mutable {
      REQUIRE(j4++ < N);
      {
...
void ifelse_pipeline(size_t L, unsigned w) {
  ...
  for(size_t N = 1; N < maxN; ++N) {
    ...
    collie::tf::Pipeline pl(L,
      // pipe 1
      collie::tf::Pipe(collie::tf::PipeType::SERIAL, [&, N](auto& pf){
        if(pf.token() == N) {
          pf.stop();
          return;
        }
      ...
      // pipe 2
      collie::tf::Pipe(collie::tf::PipeType::PARALLEL, [&](auto& pf){
      ...
      // pipe 3
      collie::tf::Pipe(collie::tf::PipeType::SERIAL, [&](auto& pf){
  ...
  for (size_t N = 1; N < maxN; ++N) {
    for(size_t subN = 1; subN < maxsubN; ++subN) {
      ...
      collie::tf::Pipeline pl(L,
        ...
        // begin of pipe 1 -----------------------------
        collie::tf::Pipe{collie::tf::PipeType::SERIAL, [&, N, subN, subL](auto& pf) mutable {
          if(j1 == N) {
            pf.stop();
            return;
          }
          ...
          // subpipeline
          collie::tf::Pipeline subpl(subL,

            // subpipe 1
            collie::tf::Pipe{collie::tf::PipeType::SERIAL, [&, subN](auto& subpf) mutable {
              if(subj1 == subN) {
                subpf.stop();
                return;
              }
            ...
            // subpipe 2
            collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [&, subN](auto& subpf) mutable {
              REQUIRE(subj2++ < subN);
              REQUIRE(subpf.token() % subL == subpf.line());
        ...
        // end of pipe 1 -----------------------------
         ...
         //begin of pipe 2 ---------------------------
        collie::tf::Pipe{collie::tf::PipeType::PARALLEL, [&, N, subN, subL](auto& pf) mutable {
        ...
        // begin of pipe 4 ---------------------------
        collie::tf::Pipe{collie::tf::PipeType::SERIAL, [&, subN](auto& pf) mutable {
