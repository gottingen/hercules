...
#pragma GCC diagnostic pop
        ...
        std::string to_string(T &&variable) {
            ...
            while (cval != end) {
                defaults.emplace_back(collie::detail::to_string(*cval));
                ++cval;
            }
...
/// String and similar direct assignment
        ...
        struct classify_object<
                T,
                typename std::enable_if<
                        !std::is_floating_point<T>::value && !std::is_integral<T>::value && WIDE_STRING_CHECK &&
...
/// Convert to an unsigned integral
        ...
        template<
                typename T,
                enable_if_t<classify_object<T>::value ==
                            object_category::wstring_assignable, detail::enabler> = detail::dummy>
        bool lexical_cast(const std::string &input, T &output) {
            output = widen(input);
            return true;
        }
        ...
        template<
                typename T,
                enable_if_t<classify_object<T>::value ==
                            object_category::wstring_constructible, detail::enabler> = detail::dummy>
        bool lexical_cast(const std::string &input, T &output) {
