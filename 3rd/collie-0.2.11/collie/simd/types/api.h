...
#ifndef COLLIE_SIMD_TYPES_API_H_
#define COLLIE_SIMD_TYPES_API_H_
...
namespace collie::simd {
     ...
     */
    template<class T, class A>
    inline complex_batch_type_t<batch<T, A>>
     ...
     *
     * @warning Unlike most simd function, this does not return the same result as the
    ...
    template<class T, class A>
    inline batch<T, A>
    select(batch_bool<T, A> const &cond, batch<T, A> const &true_br, batch<T, A> const &false_br) noexcept {
        detail::static_check_supported_config<T, A>();
    ...
    inline batch<T, A> select(batch_bool_constant<batch<T, A>, Values...> const &cond, batch<T, A> const &true_br,
    ...
    swizzle(batch<std::complex<T>, A> const &x, batch_constant<batch<Vt, A>, Values...> mask) noexcept {
        static_assert(sizeof(T) == sizeof(Vt), "consistent mask");
        detail::static_check_supported_config<T, A>();
        return kernel::swizzle<A>(x, mask, A{});
    }
...
#endif  // COLLIE_SIMD_TYPES_API_H_
