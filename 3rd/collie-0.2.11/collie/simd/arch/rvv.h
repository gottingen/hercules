...
#ifndef COLLIE_SIMD_ARCH_RVV_H_
#define COLLIE_SIMD_ARCH_RVV_H_
...
#include <collie/simd/arch/constants.h>
...
//
#define COLLIE_SIMD_RVV_JOIN_(x, y) x##y
#define COLLIE_SIMD_RVV_JOIN(x, y) COLLIE_SIMD_RVV_JOIN_(x, y)
#define COLLIE_SIMD_RVV_PREFIX_T(T, S, then) COLLIE_SIMD_RVV_JOIN(T, then)
#define COLLIE_SIMD_RVV_PREFIX_S(T, S, then) COLLIE_SIMD_RVV_JOIN(S, then)
#define COLLIE_SIMD_RVV_PREFIX_M(T, S, then) COLLIE_SIMD_RVV_JOIN(m1, then)
#define COLLIE_SIMD_RVV_PREFIX(T, S, then) then
//
// COLLIE_SIMD_RVV_IDENTIFIER accepts type and size parameters, and a template for
// the identifier.  The template is a comma-separated list of alternating
// literal and parameter segments.  Each parameter is appended to COLLIE_SIMD_RVV_PREFIX to
...
//
#define COLLIE_SIMD_RVV_IDENTIFIER9(T, S, t, ...) t
#define COLLIE_SIMD_RVV_IDENTIFIER8(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER9(T, S, __VA_ARGS__)))
#define COLLIE_SIMD_RVV_IDENTIFIER7(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER8(T, S, __VA_ARGS__)))
#define COLLIE_SIMD_RVV_IDENTIFIER6(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER7(T, S, __VA_ARGS__)))
#define COLLIE_SIMD_RVV_IDENTIFIER5(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER6(T, S, __VA_ARGS__)))
#define COLLIE_SIMD_RVV_IDENTIFIER4(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER5(T, S, __VA_ARGS__)))
#define COLLIE_SIMD_RVV_IDENTIFIER3(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER4(T, S, __VA_ARGS__)))
#define COLLIE_SIMD_RVV_IDENTIFIER2(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER3(T, S, __VA_ARGS__)))
#define COLLIE_SIMD_RVV_IDENTIFIER1(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER2(T, S, __VA_ARGS__)))
#define COLLIE_SIMD_RVV_IDENTIFIER0(T, S, t, p, ...) COLLIE_SIMD_RVV_JOIN(t, COLLIE_SIMD_RVV_PREFIX##p(T, S, COLLIE_SIMD_RVV_IDENTIFIER1(T, S, __VA_ARGS__)))
//
// UNBRACKET and REPARSE force the preprocessor to handle expansion in a
// specific order.  COLLIE_SIMD_RVV_UNBRACKET strips the parentheses from the template
// (which were necessary to keep the template as a single, named macro
// parameter up to this point).  COLLIE_SIMD_RVV_ARG_LIST then forms the new parameter list
// to pass to COLLIE_SIMD_RVV_IDENTIFIER0, with trailing commas to ensure the unrolled
// COLLIE_SIMD_RVV_IDENTIFIER loop runs to completion adding empty strings.
//
// However COLLIE_SIMD_RVV_IDENTIFIER0 is not expanded immediately because it does not
// match a function-like macro in this pass.  COLLIE_SIMD_RVV_REPARSE forces another
// evaluation after the expansion of COLLIE_SIMD_RVV_ARG_LIST, where COLLIE_SIMD_RVV_IDENTIFIER0 will
// now match as a function-like macro, and the cycle of substitutions and
// insertions can begin.
//
#define COLLIE_SIMD_RVV_REPARSE(v) (v)
#define COLLIE_SIMD_RVV_UNBRACKET(...) __VA_ARGS__
#define COLLIE_SIMD_RVV_ARG_LIST(T, S, name) (T, S, COLLIE_SIMD_RVV_UNBRACKET name, , , , , , , , , , , , , , , , , , , , , )
#define COLLIE_SIMD_RVV_IDENTIFIER(T, S, name) COLLIE_SIMD_RVV_REPARSE(COLLIE_SIMD_RVV_IDENTIFIER0 COLLIE_SIMD_RVV_ARG_LIST(T, S, name))
//
...
//
#define COLLIE_SIMD_RVV_T , _T,
#define COLLIE_SIMD_RVV_S , _S,
#define COLLIE_SIMD_RVV_M , _M,
#define COLLIE_SIMD_RVV_TSM COLLIE_SIMD_RVV_T COLLIE_SIMD_RVV_S COLLIE_SIMD_RVV_M

// COLLIE_SIMD_RVV_OVERLOAD, below, expands to a head section, a number of body sections
...
// three macro names COLLIE_SIMD_RVV_WRAPPER_HEAD, COLLIE_SIMD_RVV_WRAPPER, and COLLIE_SIMD_RVV_WRAPPER_TAIL and
// specified as an argument to COLLIE_SIMD_RVV_OVERLOAD (the empty string is the default,
// but still needs an extra comma to hold its place).
//
// The default COLLIE_SIMD_RVV_WRAPPER_HEAD provides a class containing convenient names
// for the function signature argument(s) to COLLIE_SIMD_RVV_OVERLOAD.  That signature can
// also reference the template argument T, because it's a text substitution
// into the template.
#define COLLIE_SIMD_RVV_WRAPPER_HEAD(NAME, SIGNATURE, ...)                      \
    namespace NAME##_cruft                                                \
    {                                                                     \
        template <class T>                                                \
        struct ctx                                                        \
        {                                                                 \
            static constexpr size_t width = COLLIE_SIMD_RVV_BITS;               \
...
#define COLLIE_SIMD_RVV_WRAPPER_HEAD_NOVL(...) COLLIE_SIMD_RVV_WRAPPER_HEAD(__VA_ARGS__)
#define COLLIE_SIMD_RVV_WRAPPER_HEAD_DROP_1ST(...) COLLIE_SIMD_RVV_WRAPPER_HEAD(__VA_ARGS__)
#define COLLIE_SIMD_RVV_WRAPPER_HEAD_DROP_1ST_CUSTOM_ARGS(...) COLLIE_SIMD_RVV_WRAPPER_HEAD(__VA_ARGS__)
#define COLLIE_SIMD_RVV_WRAPPER_HEAD_DROP_1ST_CUSTOM_ARGS_NOVL(...) COLLIE_SIMD_RVV_WRAPPER_HEAD(__VA_ARGS__)
...
//
#define COLLIE_SIMD_RVV_WRAPPER(KEY, CALLEE, ...)                   \
...
#define COLLIE_SIMD_RVV_WRAPPER_NOVL(KEY, CALLEE, ...)              \
    template <class Ret, class... Args>                       \
    struct impl<KEY, Ret(Args...)>                            \
    {                                                         \
        constexpr Ret operator()(Args... args) const noexcept \
...
#define COLLIE_SIMD_RVV_WRAPPER_DROP_1ST(KEY, CALLEE, ...)                 \
...
#define COLLIE_SIMD_RVV_WRAPPER_DROP_1ST_CUSTOM_ARGS(KEY, CALLEE, SIGNATURE, ...) \
    template <class Ret, class First, class... Args>                        \
    struct impl<KEY, Ret(First, Args...)>                                   \
    {                                                                       \
        using ctx = ctx<KEY>;                                               \
...
#define COLLIE_SIMD_RVV_WRAPPER_DROP_1ST_CUSTOM_ARGS_NOVL(KEY, CALLEE, SIGNATURE, ...) \
    template <class Ret, class First, class... Args>                             \
    struct impl<KEY, Ret(First, Args...)>                                        \
    {                                                                            \
        constexpr Ret operator()(First, Args... args) const noexcept             \
...
#define COLLIE_SIMD_RVV_WRAPPER_TAIL(NAME, ...)                     \
    } /* namespace NAME##_cruft */                            \
...
#define COLLIE_SIMD_RVV_WRAPPER_TAIL_NOVL(...) COLLIE_SIMD_RVV_WRAPPER_TAIL(__VA_ARGS__)
#define COLLIE_SIMD_RVV_WRAPPER_TAIL_DROP_1ST(...) COLLIE_SIMD_RVV_WRAPPER_TAIL(__VA_ARGS__)
#define COLLIE_SIMD_RVV_WRAPPER_TAIL_DROP_1ST_CUSTOM_ARGS(...) COLLIE_SIMD_RVV_WRAPPER_TAIL(__VA_ARGS__)
#define COLLIE_SIMD_RVV_WRAPPER_TAIL_DROP_1ST_CUSTOM_ARGS_NOVL(...) COLLIE_SIMD_RVV_WRAPPER_TAIL(__VA_ARGS__)

// clang-format off

#define COLLIE_SIMD_RVV_OVERLOAD_head(my_name, variant, ...) \
    COLLIE_SIMD_RVV_WRAPPER_HEAD##variant(my_name, __VA_ARGS__)
#define COLLIE_SIMD_RVV_OVERLOAD_i(name, variant, ...)                                        \
    COLLIE_SIMD_RVV_WRAPPER##variant(int8_t, COLLIE_SIMD_RVV_IDENTIFIER(i, 8, name), __VA_ARGS__)   \
    COLLIE_SIMD_RVV_WRAPPER##variant(int16_t, COLLIE_SIMD_RVV_IDENTIFIER(i, 16, name), __VA_ARGS__) \
    COLLIE_SIMD_RVV_WRAPPER##variant(int32_t, COLLIE_SIMD_RVV_IDENTIFIER(i, 32, name), __VA_ARGS__) \
    COLLIE_SIMD_RVV_WRAPPER##variant(int64_t, COLLIE_SIMD_RVV_IDENTIFIER(i, 64, name), __VA_ARGS__)
#define COLLIE_SIMD_RVV_OVERLOAD_u(name, variant, ...)                                         \
    COLLIE_SIMD_RVV_WRAPPER##variant(uint8_t, COLLIE_SIMD_RVV_IDENTIFIER(u, 8, name), __VA_ARGS__)   \
    COLLIE_SIMD_RVV_WRAPPER##variant(uint16_t, COLLIE_SIMD_RVV_IDENTIFIER(u, 16, name), __VA_ARGS__) \
    COLLIE_SIMD_RVV_WRAPPER##variant(uint32_t, COLLIE_SIMD_RVV_IDENTIFIER(u, 32, name), __VA_ARGS__) \
    COLLIE_SIMD_RVV_WRAPPER##variant(uint64_t, COLLIE_SIMD_RVV_IDENTIFIER(u, 64, name), __VA_ARGS__)
#define COLLIE_SIMD_RVV_OVERLOAD_f(name, variant, ...)                                      \
    COLLIE_SIMD_RVV_WRAPPER##variant(float, COLLIE_SIMD_RVV_IDENTIFIER(f, 32, name), __VA_ARGS__) \
    COLLIE_SIMD_RVV_WRAPPER##variant(double, COLLIE_SIMD_RVV_IDENTIFIER(f, 64, name), __VA_ARGS__)
#define COLLIE_SIMD_RVV_OVERLOAD_tail(my_name, variant, ...) \
    COLLIE_SIMD_RVV_WRAPPER_TAIL##variant(my_name, __VA_ARGS__)
...
// templates.
//
#define COLLIE_SIMD_RVV_OVERLOAD2(my_name, name_i, name_u, variant, ...) \
    COLLIE_SIMD_RVV_OVERLOAD_head(my_name, variant, __VA_ARGS__)         \
    COLLIE_SIMD_RVV_OVERLOAD_i(name_i, variant, __VA_ARGS__)         \
    COLLIE_SIMD_RVV_OVERLOAD_u(name_u, variant, __VA_ARGS__)     \
    COLLIE_SIMD_RVV_OVERLOAD_tail(my_name, variant, __VA_ARGS__)

#define COLLIE_SIMD_RVV_OVERLOAD3(my_name, name_i, name_u, name_f, variant, ...) \
    COLLIE_SIMD_RVV_OVERLOAD_head(my_name, variant, __VA_ARGS__)                 \
    COLLIE_SIMD_RVV_OVERLOAD_i(name_i, variant, __VA_ARGS__)                     \
    COLLIE_SIMD_RVV_OVERLOAD_u(name_u, variant, __VA_ARGS__)                     \
    COLLIE_SIMD_RVV_OVERLOAD_f(name_f, variant, __VA_ARGS__)                     \
    COLLIE_SIMD_RVV_OVERLOAD_tail(my_name, variant, __VA_ARGS__)

#define COLLIE_SIMD_RVV_OVERLOAD(my_name, name, ...) COLLIE_SIMD_RVV_OVERLOAD3(my_name, name, name, name, __VA_ARGS__)
#define COLLIE_SIMD_RVV_OVERLOAD_INTS(my_name, name, ...) COLLIE_SIMD_RVV_OVERLOAD2(my_name, name, name, __VA_ARGS__)

#define COLLIE_SIMD_RVV_OVERLOAD_SINTS(my_name, name, variant, ...) \
    COLLIE_SIMD_RVV_OVERLOAD_head(my_name, variant, __VA_ARGS__)    \
    COLLIE_SIMD_RVV_OVERLOAD_i(name, variant, __VA_ARGS__)          \
    COLLIE_SIMD_RVV_OVERLOAD_tail(my_name, variant, __VA_ARGS__)

#define COLLIE_SIMD_RVV_OVERLOAD_UINTS(my_name, name, variant, ...) \
    COLLIE_SIMD_RVV_OVERLOAD_head(my_name, variant, __VA_ARGS__)    \
    COLLIE_SIMD_RVV_OVERLOAD_u(name, variant, __VA_ARGS__)          \
    COLLIE_SIMD_RVV_OVERLOAD_tail(my_name, variant, __VA_ARGS__)

#define COLLIE_SIMD_RVV_OVERLOAD_FLOATS(my_name, name, variant, ...) \
    COLLIE_SIMD_RVV_OVERLOAD_head(my_name, variant, __VA_ARGS__)     \
    COLLIE_SIMD_RVV_OVERLOAD_f(name, variant, __VA_ARGS__)           \
    COLLIE_SIMD_RVV_OVERLOAD_tail(my_name, variant, __VA_ARGS__)
...
namespace collie::simd {
    ...
    namespace kernel {
        namespace detail {
            ...
            template<class T, size_t Width = COLLIE_SIMD_RVV_BITS>
            ...
            rvv_reg_t<T, U::width> rvvreinterpret(U const &arg) noexcept {
                return rvv_reg_t<T, U::width>(arg, types::detail::COLLIE_SIMD_RVV_BITCAST);
            }
            ...
            COLLIE_SIMD_RVV_OVERLOAD(rvvid,
                               (__riscv_vid_v_u COLLIE_SIMD_RVV_S COLLIE_SIMD_RVV_M), _DROP_1ST, uvec(T))

            COLLIE_SIMD_RVV_OVERLOAD3(rvvmv_splat,
                                (__riscv_vmv_v_x_ COLLIE_SIMD_RVV_TSM),
                                (__riscv_vmv_v_x_ COLLIE_SIMD_RVV_TSM),
                                (__riscv_vfmv_v_f_ COLLIE_SIMD_RVV_TSM), , vec(T))

            COLLIE_SIMD_RVV_OVERLOAD3(rvvmv_lane0,
                                (__riscv_vmv_x),
                                (__riscv_vmv_x),
                                (__riscv_vfmv_f), _NOVL, T(vec))

            COLLIE_SIMD_RVV_OVERLOAD(rvvmerge, (__riscv_vmerge), , vec(vec, vec, bvec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvmerge_splat,
                                (__riscv_vmerge),
                                (__riscv_vmerge),
                                (__riscv_vfmerge), , vec(vec, T, bvec))

            // count active lanes in a predicate
            COLLIE_SIMD_RVV_OVERLOAD(rvvcpop, (__riscv_vcpop), , size_t(bvec));
            ...
            template<class T>
            using rvv_enable_signed_int_t = typename std::enable_if<
                    std::is_integral<T>::value && std::is_signed<T>::value, int>::type;

            // enable for unsigned integers
            template<class T>
            using rvv_enable_unsigned_int_t = typename std::enable_if<
        ...
        namespace detail {
            ...
            inline detail::rvv_reg_t<T, Width> broadcast(T arg) noexcept {
                ...
                return detail::rvv_reg_t<T, Width>(splat.get_bytes(), types::detail::COLLIE_SIMD_RVV_BITCAST);
            }
        }
        ...
        namespace detail {
            COLLIE_SIMD_RVV_OVERLOAD(rvvle, (__riscv_vle COLLIE_SIMD_RVV_S _v_ COLLIE_SIMD_RVV_TSM), , vec(T const*))
            COLLIE_SIMD_RVV_OVERLOAD(rvvse, (__riscv_vse COLLIE_SIMD_RVV_S _v_ COLLIE_SIMD_RVV_TSM), , void(T*, vec))
        }
        ...
        namespace detail {
            ...
            COLLIE_SIMD_RVV_OVERLOAD(rvvget_lo_, (__riscv_vget_ COLLIE_SIMD_RVV_TSM), _DROP_1ST_CUSTOM_ARGS_NOVL, vec(T, wide_vec),
                               args..., 0)
            COLLIE_SIMD_RVV_OVERLOAD(rvvget_hi_, (__riscv_vget_ COLLIE_SIMD_RVV_TSM), _DROP_1ST_CUSTOM_ARGS_NOVL, vec(T, wide_vec),
                               args..., 1)
            ...
            load_complex(batch<T, A> const &lo, batch<T, A> const &hi, requires_arch<rvv>) noexcept {
        ...
        namespace detail {
            template<class T, class U>
            using rvv_enable_sg_t = typename std::enable_if<(sizeof(T) == sizeof(U) &&
                                                             (sizeof(T) == 4 || sizeof(T) == 8)), int>::type;
            COLLIE_SIMD_RVV_OVERLOAD(rvvloxei, (__riscv_vloxei COLLIE_SIMD_RVV_S), , vec(T const*, uvec))
            COLLIE_SIMD_RVV_OVERLOAD(rvvsoxei, (__riscv_vsoxei COLLIE_SIMD_RVV_S), , void(T*, uvec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvmul_splat,
                                (__riscv_vmul),
                                (__riscv_vmul),
        ...
        inline void
        scatter(batch<T, A> const &vals, T *dst, batch<U, A> const &index, kernel::requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, class U, detail::rvv_enable_sg_t<T, U> = 0>
        inline batch<T, A>
        gather(batch<T, A> const &, T const *src, batch<U, A> const &index, kernel::requires_arch<rvv>) noexcept {
            using UU = as_unsigned_integer_t<U>;
            const auto uindex = detail::rvv_to_unsigned_batch(index);
        ...
        namespace detail {
            COLLIE_SIMD_RVV_OVERLOAD3(rvvadd,
                                (__riscv_vadd),
                                (__riscv_vadd),
                                (__riscv_vfadd), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD2(rvvsadd,
                                (__riscv_vsadd),
                                (__riscv_vsaddu), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvsub,
                                (__riscv_vsub),
                                (__riscv_vsub),
                                (__riscv_vfsub), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD2(rvvssub,
                                (__riscv_vssub),
                                (__riscv_vssubu), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD2(rvvaadd,
                                (__riscv_vaadd),
                                (__riscv_vaaddu), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvmul,
                                (__riscv_vmul),
                                (__riscv_vmul),
                                (__riscv_vfmul), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvdiv,
                                (__riscv_vdiv),
                                (__riscv_vdivu),
                                (__riscv_vfdiv), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvmax,
                                (__riscv_vmax),
                                (__riscv_vmaxu),
                                (__riscv_vfmax), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvmin,
                                (__riscv_vmin),
                                (__riscv_vminu),
                                (__riscv_vfmin), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvneg,
                                (__riscv_vneg),
                                (abort),
                                (__riscv_vfneg), , vec(vec))
            COLLIE_SIMD_RVV_OVERLOAD_FLOATS(rvvabs,
                                      (__riscv_vfabs), , vec(vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvmacc,
                                (__riscv_vmacc),
                                (__riscv_vmacc),
                                (__riscv_vfmacc), , vec(vec, vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvnmsac,
                                (__riscv_vnmsac),
                                (__riscv_vnmsac),
                                (__riscv_vfnmsac), , vec(vec, vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvmadd,
                                (__riscv_vmadd),
                                (__riscv_vmadd),
                                (__riscv_vfmadd), , vec(vec, vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvnmsub,
                                (__riscv_vnmsub),
                                (__riscv_vnmsub),
                                (__riscv_vfnmsub), , vec(vec, vec, vec))

#define RISCV_VMSXX(XX)                                      \
    COLLIE_SIMD_RVV_OVERLOAD3(rvvms##XX,                           \
                        (__riscv_vms##XX),                   \
                        (__riscv_vms##XX##u),                \
                        (__riscv_vmf##XX), , bvec(vec, vec)) \
    COLLIE_SIMD_RVV_OVERLOAD3(rvvms##XX##_splat,                   \
                        (__riscv_vms##XX),                   \
                        (__riscv_vms##XX##u),                \
...
#undef RISCV_VMSXX
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch<T, A>
        fma(batch<T, A> const &x, batch<T, A> const &y, batch<T, A> const &z, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch<T, A>
        fnma(batch<T, A> const &x, batch<T, A> const &y, batch<T, A> const &z, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch<T, A>
        fms(batch<T, A> const &x, batch<T, A> const &y, batch<T, A> const &z, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch<T, A>
        fnms(batch<T, A> const &x, batch<T, A> const &y, batch<T, A> const &z, requires_arch<rvv>) noexcept {
        ...
        namespace detail {
            COLLIE_SIMD_RVV_OVERLOAD_INTS(rvvand, (__riscv_vand), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD_INTS(rvvor, (__riscv_vor), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD_INTS(rvvor_splat, (__riscv_vor), , vec(vec, T))
            COLLIE_SIMD_RVV_OVERLOAD_INTS(rvvxor, (__riscv_vxor), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD_INTS(rvvnot, (__riscv_vnot), , vec(vec))
            COLLIE_SIMD_RVV_OVERLOAD(rvvmand, (__riscv_vmand_mm_b COLLIE_SIMD_RVV_S), , bvec(bvec, bvec))
            COLLIE_SIMD_RVV_OVERLOAD(rvvmor, (__riscv_vmor_mm_b COLLIE_SIMD_RVV_S), , bvec(bvec, bvec))
            COLLIE_SIMD_RVV_OVERLOAD(rvvmxor, (__riscv_vmxor_mm_b COLLIE_SIMD_RVV_S), , bvec(bvec, bvec))
            COLLIE_SIMD_RVV_OVERLOAD(rvvmandn, (__riscv_vmandn_mm_b COLLIE_SIMD_RVV_S), , bvec(bvec, bvec))
            COLLIE_SIMD_RVV_OVERLOAD(rvvmnot, (__riscv_vmnot), , bvec(bvec))
        }
        ...
        bitwise_and(batch_bool<T, A> const &lhs, batch_bool<T, A> const &rhs, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch_bool<T, A>
        bitwise_andnot(batch_bool<T, A> const &lhs, batch_bool<T, A> const &rhs, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch_bool<T, A>
        bitwise_or(batch_bool<T, A> const &lhs, batch_bool<T, A> const &rhs, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch_bool<T, A>
        bitwise_xor(batch_bool<T, A> const &lhs, batch_bool<T, A> const &rhs, requires_arch<rvv>) noexcept {
        ...
        namespace detail {
            COLLIE_SIMD_RVV_OVERLOAD_INTS(rvvsll_splat, (__riscv_vsll), , vec(vec, size_t))
            COLLIE_SIMD_RVV_OVERLOAD_INTS(rvvsll, (__riscv_vsll), , vec(vec, uvec))
            COLLIE_SIMD_RVV_OVERLOAD2(rvvsr_splat,
                                (__riscv_vsra),
                                (__riscv_vsrl), , vec(vec, size_t))
            COLLIE_SIMD_RVV_OVERLOAD2(rvvsr,
                                (__riscv_vsra),
        ...
        namespace detail {
            COLLIE_SIMD_RVV_OVERLOAD3(rvvredsum,
                                (__riscv_vredsum),
                                (__riscv_vredsum),
                                (__riscv_vfredosum), // or __riscv_vfredusum
            , scalar_vec(vec, scalar_vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvredmax,
                                (__riscv_vredmax),
                                (__riscv_vredmaxu),
                                (__riscv_vfredmax), , scalar_vec(vec, scalar_vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvredmin,
                                (__riscv_vredmin),
                                (__riscv_vredminu),
                                (__riscv_vfredmin), , scalar_vec(vec, scalar_vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvslide1up,
                                (__riscv_vslide1up),
                                (__riscv_vslide1up),
                                (__riscv_vfslide1up), , vec(vec, vec))
            COLLIE_SIMD_RVV_OVERLOAD3(rvvslide1down,
                                (__riscv_vslide1down),
                                (__riscv_vslide1down),
            ...
            inline T reduce_scalar(rvv_reg_t<T, types::detail::rvv_width_m1> const &arg) {
                return detail::rvvmv_lane0(rvv_reg_t<T, A::width>(arg.get_bytes(), types::detail::COLLIE_SIMD_RVV_BITCAST));
            }
        }
...
#pragma unroll size
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch_bool<T, A>
        eq(batch_bool<T, A> const &lhs, batch_bool<T, A> const &rhs, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch_bool<T, A>
        neq(batch_bool<T, A> const &lhs, batch_bool<T, A> const &rhs, requires_arch<rvv>) noexcept {
            return detail::rvvmxor(lhs, rhs);
        }
         ...
         *************/
        namespace detail {
            COLLIE_SIMD_RVV_OVERLOAD(rvvcompress, (__riscv_vcompress), , vec(vec, bvec))
        }
         ...
         ***************/
        namespace detail {
            COLLIE_SIMD_RVV_OVERLOAD(rvvrgather, (__riscv_vrgather), , vec(vec, uvec))
            COLLIE_SIMD_RVV_OVERLOAD(rvvslideup, (__riscv_vslideup), , vec(vec, vec, size_t))
            COLLIE_SIMD_RVV_OVERLOAD(rvvslidedown, (__riscv_vslidedown), , vec(vec, size_t))
        }

        // swizzle
        template<class A, class T, class I, I... idx>
        inline batch<T, A>
        swizzle(batch<T, A> const &arg, batch_constant<batch<I, A>, idx...>, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch<T, A>
        extract_pair(batch<T, A> const &lhs, batch<T, A> const &rhs, size_t n, requires_arch<rvv>) noexcept {
        ...
        template<class A, class T, detail::rvv_enable_all_t<T> = 0>
        inline batch<T, A>
        select(batch_bool<T, A> const &cond, batch<T, A> const &a, batch<T, A> const &b, requires_arch<rvv>) noexcept {
            return detail::rvvmerge(b, a, cond);
        }

        template<class A, class T, bool... b>
        inline batch<T, A> select(batch_bool_constant < batch<T, A>, b...
        > const&,
        batch<T, A> const &true_br, batch<T, A>
        const& false_br, requires_arch<rvv>) noexcept {
        return
        select(batch_bool<T, A> {b...}, true_br, false_br, rvv{}
        );
    }
    ...
    template<class A, class T, detail::rvv_enable_floating_point_t<T> = 0>
    inline void
    store_complex_aligned(std::complex<T> *dst, batch<std::complex<T>, A> const &src, requires_arch <rvv>) noexcept {
    ...
    template<class A, class T, detail::rvv_enable_floating_point_t<T> = 0>
    inline void
    store_complex_unaligned(std::complex<T> *dst, batch<std::complex<T>, A> const &src, requires_arch <rvv>) noexcept {
    ...
    namespace detail {
        COLLIE_SIMD_RVV_OVERLOAD_FLOATS(rvvfsqrt, (__riscv_vfsqrt), , vec(vec))
        COLLIE_SIMD_RVV_OVERLOAD_FLOATS(rvvfrec7, (__riscv_vfrec7), , vec(vec))
        COLLIE_SIMD_RVV_OVERLOAD_FLOATS(rvvfrsqrt7, (__riscv_vfrsqrt7), , vec(vec))
    }
    ...
    // fast_cast
    namespace detail {
        COLLIE_SIMD_RVV_OVERLOAD2(rvvfcvt_rtz, // truncating conversion, like C.
                            (__riscv_vfcvt_rtz_x),
                            (__riscv_vfcvt_rtz_xu), _DROP_1ST, vec(T, fvec))
        COLLIE_SIMD_RVV_OVERLOAD2(rvvfcvt_rne, // round to nearest, ties to even
                            (__riscv_vfcvt_x),
                            (__riscv_vfcvt_xu), _DROP_1ST_CUSTOM_ARGS, vec(T, fvec), args..., __RISCV_FRM_RNE)
        COLLIE_SIMD_RVV_OVERLOAD2(rvvfcvt_rmm, // round to nearest, ties to max magnitude
                            (__riscv_vfcvt_x),
                            (__riscv_vfcvt_xu), _DROP_1ST_CUSTOM_ARGS, vec(T, fvec), args..., __RISCV_FRM_RMM)
        COLLIE_SIMD_RVV_OVERLOAD2(rvvfcvt, // round to current rounding mode.
                            (__riscv_vfcvt_x),
                            (__riscv_vfcvt_xu), _DROP_1ST, vec(T, fvec))
        COLLIE_SIMD_RVV_OVERLOAD_INTS(rvvfcvt_f, (__riscv_vfcvt_f), , fvec(vec))

        template<class T, class U>
        using rvv_enable_ftoi_t = typename std::enable_if<(sizeof(T) == sizeof(U) && std::is_floating_point<T>::value &&
                                                           !std::is_floating_point<U>::value), int>::type;
        template<class T, class U>
        using rvv_enable_itof_t = typename std::enable_if<(sizeof(T) == sizeof(U) &&
    ...
    inline batch<R, A> bitwise_cast(batch<T, A> const &arg, batch<R, A> const &, requires_arch <rvv>) noexcept {
        return detail::rvv_reg_t<R, A::width>(arg.data.get_bytes(), types::detail::COLLIE_SIMD_RVV_BITCAST);
    }

    // batch_bool_cast
    template<class A, class T_out, class T_in, detail::rvv_enable_all_t<T_in> = 0>
    inline batch_bool<T_out, A>
    batch_bool_cast(batch_bool<T_in, A> const &arg, batch_bool<T_out, A> const &, requires_arch <rvv>) noexcept {
    ...
    inline batch<T, A> slide_right(batch<T, A> const &arg, requires_arch <rvv>) noexcept {
        ...
        const auto bytes = arg.data.get_bytes();
        return reg_t(detail::rvvslidedownbytes<A::width>(bytes, N), types::detail::COLLIE_SIMD_RVV_BITCAST);
    }
