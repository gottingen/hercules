...
#ifndef COLLIE_SIMD_ARCH_GENERIC_TRIGO_H_
#define COLLIE_SIMD_ARCH_GENERIC_TRIGO_H_
...
namespace collie::simd {
    ...
    namespace kernel {
        ...
        inline batch<double, A> asin(batch<double, A> const &self, requires_arch<generic>) noexcept {
            ...
            batch_type vp = zz1 *
                            detail::horner<batch_type, 0x403c896240f3081dull, 0xc03991aaac01ab68ull, 0x401bdff5baf33e6aull, 0xbfe2079259f9290full, 0x3f684fc3988e9f08ull>(
                                    zz1) /
                            detail::horner1<batch_type, 0x40756709b0b644beull, 0xc077fe08959063eeull, 0x40626219af6a7f42ull, 0xc035f2a2b6bf5d8cull>(
                                    zz1);
            zz1 = sqrt(zz1 + zz1);
            ...
            z = zz2 *
                detail::horner<batch_type, 0xc020656c06ceafd5ull, 0x40339007da779259ull, 0xc0304331de27907bull, 0x4015c74b178a2dd9ull, 0xbfe34341333e5c16ull, 0x3f716b9b0bd48ad3ull>(
                        zz2) /
                detail::horner1<batch_type, 0xc04898220a3607acull, 0x4061705684ffbf9dull, 0xc06265bb6d3576d7ull, 0x40519fc025fe9054ull, 0xc02d7b590b5e0eabull>(
                        zz2);
            zz2 = fma(x, z, x);
        ...
        namespace detail {
            ...
            template<class A>
            static inline batch<double, A>
            kernel_atan(const batch<double, A> &x, const batch<double, A> &recx) noexcept {
                using batch_type = batch<double, A>;
                const auto flag1 = x < constants::tan3pio8<batch_type>();
                    ...
                    if (all(x <= constants::pio4<B>())) {
                        ...
                        for (std::size_t i = 0; i < size; ++i) {
                            ...
                            if (arg == std::numeric_limits<value_type>::infinity()) {
                                ...
                                double y[2];
                                std::int32_t n = ::collie::simd::detail::__ieee754_rem_pio2(arg, y);
        ...
        namespace detail {
            ...
            inline batch<T, A> sin(batch<T, A> const &self, Tag = Tag()) noexcept {
                ...
                auto tmp = select(n >= batch_type(2.), batch_type(1.), batch_type(0.));
                auto swap_bit = fma(batch_type(-2.), tmp, n);
                auto sign_bit = bitofsign(self) ^
                                select(tmp != batch_type(0.), constants::signmask<batch_type>(), batch_type(0.));
                const batch_type z = xr * xr;
                const batch_type se = detail::sin_eval(z, xr);
                const batch_type ce = detail::cos_eval(z);
                const batch_type z1 = select(swap_bit == batch_type(0.), se, ce);
        ...
        inline std::pair<batch<T, A>, batch<T, A>> sincos(batch<T, A> const &self, requires_arch<generic>) noexcept {
            ...
            const batch_type ce = detail::cos_eval(z);
            auto sin_sign_bit =
                    bitofsign(self) ^ select(tmp != batch_type(0.), constants::signmask<batch_type>(), batch_type(0.));
            const batch_type sin_z1 = select(swap_bit == batch_type(0.), se, ce);
            auto cos_sign_bit = select((swap_bit ^ tmp) != batch_type(0.), constants::signmask<batch_type>(),
                                       batch_type(0.));
...
#endif  // COLLIE_SIMD_ARCH_GENERIC_TRIGO_H_
