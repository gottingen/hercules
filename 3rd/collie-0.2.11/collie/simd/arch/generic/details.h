...
#ifndef COLLIE_SIMD_GENERIC_DETAILS_H_
#define COLLIE_SIMD_GENERIC_DETAILS_H_

#include <complex>

#include <collie/simd/math/rem_pio2.h>
...
namespace collie::simd {
    ...
    select(batch_bool<T, A> const &, batch<std::complex<T>, A> const &, batch<std::complex<T>, A> const &) noexcept;
    ...
    namespace kernel {
        ...
        namespace detail {
            ...
            fast_cast(batch<int8_t, A> const &self, batch<uint8_t, A> const &, requires_arch<generic>) noexcept {
                return bitwise_cast<uint8_t>(self);
            }

            template<class A>
            inline batch<uint16_t, A>
            fast_cast(batch<int16_t, A> const &self, batch<uint16_t, A> const &, requires_arch<generic>) noexcept {
                return bitwise_cast<uint16_t>(self);
            }

            template<class A>
            inline batch<uint32_t, A>
            fast_cast(batch<int32_t, A> const &self, batch<uint32_t, A> const &, requires_arch<generic>) noexcept {
                return bitwise_cast<uint32_t>(self);
            }

            template<class A>
            inline batch<uint64_t, A>
            fast_cast(batch<int64_t, A> const &self, batch<uint64_t, A> const &, requires_arch<generic>) noexcept {
                return bitwise_cast<uint64_t>(self);
            }

            template<class A>
            inline batch<int8_t, A>
            fast_cast(batch<uint8_t, A> const &self, batch<int8_t, A> const &, requires_arch<generic>) noexcept {
                return bitwise_cast<int8_t>(self);
            }

            template<class A>
            inline batch<int16_t, A>
            fast_cast(batch<uint16_t, A> const &self, batch<int16_t, A> const &, requires_arch<generic>) noexcept {
                return bitwise_cast<int16_t>(self);
            }

            template<class A>
            inline batch<int32_t, A>
            fast_cast(batch<uint32_t, A> const &self, batch<int32_t, A> const &, requires_arch<generic>) noexcept {
                return bitwise_cast<int32_t>(self);
            }

            template<class A>
            inline batch<int64_t, A>
            fast_cast(batch<uint64_t, A> const &self, batch<int64_t, A> const &, requires_arch<generic>) noexcept {
            ...
            fast_cast(batch<uint32_t, A> const &v, batch<float, A> const &, requires_arch<generic>) noexcept {
                ...
                auto v_lo = batch_cast<int32_t>(
                        v & msk_lo); /* extract the 16 lowest significant bits of self                             */
                auto v_hi = batch_cast<int32_t>(
                        v >> 16); /* 16 most significant bits of v                                                 */
                auto v_lo_flt = batch_cast<float>(
                        v_lo); /* No rounding                                                                */
                auto v_hi_flt = batch_cast<float>(
                        v_hi); /* No rounding                                                                */
                v_hi_flt = cnst65536f *
                           v_hi_flt; /* No rounding                                                            */
                return v_hi_flt +
                       v_lo_flt; /* Rounding may occur here, mul and add may fuse to fma for haswell and newer   */
            }
            ...
            inline batch<uint32_t, A>
            fast_cast(batch<float, A> const &v, batch<uint32_t, A> const &, requires_arch<generic>) noexcept {
        ...
        namespace detail {
            ...
            using collie::simd::detail::void_t;
...
#endif  // COLLIE_SIMD_GENERIC_DETAILS_H_
