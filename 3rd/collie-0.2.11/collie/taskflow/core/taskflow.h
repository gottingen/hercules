...
namespace collie::tf {
    ...
    dependency between two tasks. A task is one of the following types:
      ...
      2. subflow task        : the callable constructible from
                               @c std::function<void(collie::tf::Subflow&)>
      3. condition task      : the callable constructible from
                               @c std::function<int()>
      4. multi-condition task: the callable constructible from
                               @c %std::function<collie::InlinedVector<int>()>
      5. module task         : the task constructed from collie::tf::Taskflow::composed_of
                               @c std::function<void(collie::tf::Runtime&)>
    ...
    collie::tf::Task A = taskflow.emplace([](){ std::cout << "TaskA\n"; });
    collie::tf::Task B = taskflow.emplace([](){ std::cout << "TaskB\n"; });
    collie::tf::Task C = taskflow.emplace([](){ std::cout << "TaskC\n"; });
    collie::tf::Task D = taskflow.emplace([](){ std::cout << "TaskD\n"; });
    ...
    */
    ...
    public:
        ...
        @code{.cpp}
        collie::tf::Taskflow taskflow("My Taskflow");
        ...
        */
        ...
        @code{.cpp}
        collie::tf::Taskflow taskflow1(std::move(taskflow2));
        ...
        */
        ...
        @code{.cpp}
        collie::tf::Task parent = taskflow.emplace([](collie::tf::Subflow sf){
        ...
        */
        ...
        This method is similar to collie::tf::Taskflow::dump(std::ostream& ostream),
        ...
        */
        ...
        collie::tf::Taskflow::num_tasks is zero.
        */
        bool empty() const;
        ...
        */
        ...
        A visitor is a callable that takes an argument of type collie::tf::Task
        and returns nothing. The following example iterates each task in a
        taskflow and prints its name:

        @code{.cpp}
        taskflow.for_each_task([](collie::tf::Task task){
          std::cout << task.name() << '\n';
        });
        @endcode
        */
        ...
        */
        ...
        A graph object (of type collie::tf::Graph) is the ultimate storage for the
    ...
    collie::tf::Future is a derived class from std::future that will eventually hold the
    execution result of a submitted taskflow (collie::tf::Executor::run)
    In addition to the base methods inherited from std::future,
    you can call collie::tf::Future::cancel to cancel the execution of the running taskflow
    ...
    // submit the taskflow
    collie::tf::Future fu = executor.run(taskflow);
    ...
    */
    ...
    public:
        ...
        */
        ...
        (non-preemptive).
        You can call collie::tf::Future::wait to wait for the cancellation to complete.
        */
        bool cancel();
