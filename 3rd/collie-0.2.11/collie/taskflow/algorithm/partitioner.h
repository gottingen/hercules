...
to schedule parallel iterations (e.g., collie::tf::Taskflow::for_each).

An partitioner defines the scheduling method for running parallel algorithms,
such collie::tf::Taskflow::for_each, collie::tf::Taskflow::reduce, and so on.
By default, we provide the following partitioners: 

+ collie::tf::GuidedPartitioner  to enable guided scheduling algorithm of adaptive chunk size
+ collie::tf::DynamicPartitioner to enable dynamic scheduling algorithm of equal chunk size
+ collie::tf::StaticPartitioner  to enable static scheduling algorithm of static chunk size
+ collie::tf::RandomPartitioner  to enable random scheduling algorithm of random chunk size
...
iteration, collie::tf::StaticPartitioner can deliver the best performance.
On the other hand, if the work unit per iteration is irregular and unbalanced,
collie::tf::GuidedPartitioner or collie::tf::DynamicPartitioner can outperform collie::tf::StaticPartitioner.
In most situations, collie::tf::GuidedPartitioner can deliver decent performance and
...
taskflow.for_each_index(0, 100, 1, 
  ...
  collie::tf::StaticPartitioner(0, [](auto&& closure){
...
@note
The default closure wrapper (collie::tf::DefaultClosureWrapper) does nothing but invoke
...
@tparam C closure wrapper type (default collie::tf::DefaultClosureWrapper)
...
taskflow.for_each_index(0, 100, 1, 
  ...
  collie::tf::GuidedPartitioner(0, [](auto&& closure){
    // do something before invoking the partitioned task
    // ...
...
taskflow.for_each_index(0, 100, 1, 
  ...
  collie::tf::DynamicPartitioner(0, [](auto&& closure){
    // do something before invoking the partitioned task
    // ...
...
Similar to collie::tf::DynamicPartitioner,
...
taskflow.for_each_index(0, 100, 1, 
  ...
  collie::tf::RandomPartitioner(0, [](auto&& closure){
    // do something before invoking the partitioned task
    // ...
...
/**
@brief default partitioner set to collie::tf::GuidedPartitioner
...
A partitioner is a derived type from collie::tf::PartitionerBase.
*/
template <typename P>
