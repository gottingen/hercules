...
(specified by collie::tf::PipeType) and is associated with a callable to invoke
by the pipeline scheduler.

You need to use the template function, collie::tf::make_data_pipe, to create
a data pipe. The input and output types of a collie::tf::DataPipe should be decayed types
...
@code{.cpp}
collie::tf::make_data_pipe<int, std::string>(
...
of collie::tf::Pipeflow in the second argument to probe the runtime information
for a stage task, such as its line number and token number:

@code{.cpp}
collie::tf::make_data_pipe<int, std::string>(
  collie::tf::PipeType::SERIAL,
  [](int& input, collie::tf::Pipeflow& pf) {
...
class DataPipe {
  ...
  */
  ...
  You should use the helper function, collie::tf::make_data_pipe,
  to create a DataPipe object, especially when you need collie::tf::DataPipe
  ...
  */
  ...
  A data pipe can be either parallel (collie::tf::PipeType::PARALLEL) or serial
  (collie::tf::PipeType::SERIAL).
  */
  PipeType type() const {
    return _type;
...
/**
@brief function to construct a data pipe (collie::tf::DataPipe)

@tparam Input input data type
@tparam Output output data type
@tparam C callable type

collie::tf::make_data_pipe is a helper function to create a data pipe (collie::tf::DataPipe)
in a data-parallel pipeline (collie::tf::DataPipeline).
The first argument specifies the direction of the data pipe,
either collie::tf::PipeType::SERIAL or collie::tf::PipeType::PARALLEL,
...
The callable can additionally take a reference of collie::tf::Pipeflow,
...
Similar to collie::tf::Pipeline, a collie::tf::DataPipeline is a composable graph object
for users to create a <i>data-parallel pipeline scheduling framework</i> 
using a module task in a taskflow.
The only difference is that collie::tf::DataPipeline provides a data abstraction
...
#include <collie/taskflow/algorithm/data_pipeline.hpp>
...
int main() {
  ...
  collie::tf::DataPipeline pl(num_lines,
    collie::tf::make_data_pipe<void, int>(collie::tf::PipeType::SERIAL, [&](collie::tf::Pipeflow& pf) -> int{
      if(pf.token() == 5) {
        pf.stop();
        return 0;
      }
...
class DataPipeline {
  ...
  */
  ...
  The first pipe must define a serial direction (collie::tf::PipeType::SERIAL)
...
void DataPipeline<Ps...>::_on_pipe(Pipeflow& pf, Runtime&) {
  ...
  visit_tuple([&](auto&& pipe){
    ...
    if constexpr (std::is_invocable_v<callable_t, Pipeflow&>) {
      // [](collie::tf::Pipeflow&) -> void {}, i.e., we only have one pipe
      if constexpr (std::is_void_v<output_t>) {
        pipe._callable(pf);
      // [](collie::tf::Pipeflow&) -> output_t {}
      } else {
    ...
    // other pipes with pipeflow in the second argument
      ...
      // [](input_t&, collie::tf::Pipeflow&) -> void {}
      if constexpr (std::is_void_v<output_t>) {
        pipe._callable(std::get<input_t>(_buffer[pf._line].data), pf);
      // [](input_t&, collie::tf::Pipeflow&) -> output_t {}
      } else {
        _buffer[pf._line].data = pipe._callable(
...
void DataPipeline<Ps...>::_build() {
  ...
  for(size_t l = 0; l < num_lines(); l++) {
    ...
    _tasks[l + 1] = fb.emplace([this, l] (collie::tf::Runtime& rt) mutable {
