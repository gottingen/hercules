...
Pipeflow can only be created privately by the collie::tf::Pipeline and
...
@em parallel direction or @em serial direction (specified by collie::tf::PipeType)
and is coupled with a callable to invoke by the pipeline scheduler.
The callable must take a referenced collie::tf::Pipeflow object in the first argument:

@code{.cpp}
Pipe{PipeType::SERIAL, [](collie::tf::Pipeflow&){}}
@endcode
...
*/
template <typename C = std::function<void(collie::tf::Pipeflow&)>>
class Pipe {
  ...
  */
  ...
  @param d pipe type (collie::tf::PipeType)
  @param callable callable type

  The constructor constructs a pipe with the given direction
  (collie::tf::PipeType::SERIAL or collie::tf::PipeType::PARALLEL) and the given callable.
  The callable must take a referenced collie::tf::Pipeflow object in the first argument.
  ...
  */
  ...
  @param type a collie::tf::PipeType variable
  */
  ...
  @param callable a callable object constructible from std::function<void(collie::tf::Pipeflow&)>
...
collie::tf::Pipeline pipeline(num_lines,
...
collie::tf::Task task = taskflow.composed_of(pipeline)
                        .name("pipeline");
...
Internally, collie::tf::Pipeline uses std::tuple to store the given sequence of pipes.
...
If applications need to change these pipes, please use collie::tf::ScalablePipeline.
*/
template <typename... Ps>
class Pipeline {
...
<i>pipeline scheduling framework</i> using a module task in a taskflow.
Unlike collie::tf::Pipeline that instantiates all pipes upon the construction time,
collie::tf::ScalablePipeline allows variable assignments of pipes using range iterators.
...
auto pipe_callable = [&buffer] (collie::tf::Pipeflow& pf) mutable {
  switch(pf.pipe()) {
...
for(size_t i=0; i<3; i++) {
  pipes.emplace_back(collie::tf::PipeType::SERIAL, pipe_callable);
}

// create a pipeline of four parallel lines based on the given vector of pipes
collie::tf::ScalablePipeline pl(num_lines, pipes.begin(), pipes.end());
...
Each pipe has the same type of `%collie::tf::Pipe<%std::function<void(%collie::tf::Pipeflow&)>>`
