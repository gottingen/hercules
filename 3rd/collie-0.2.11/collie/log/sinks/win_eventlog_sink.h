...
/** Windows error */
struct win32_error : public CLogEx {
...
public:
    ...
    static sid_t duplicate_sid(PSID psid) {
        if (!::IsValidSid(psid)) {
            throw_clog_ex("sid_t::sid_t(): invalid SID received");
        }
        ...
        if (!::CopySid(sid_length, (PSID)result.as_sid(), psid)) {
            CLOG_THROW(win32_error("CopySid"));
        }
    ...
    static sid_t get_current_user_sid() {
        ...
        struct process_token_t {
            ...
            explicit process_token_t(HANDLE process) {
                if (!::OpenProcessToken(process, TOKEN_QUERY, &token_handle_)) {
                    CLOG_THROW(win32_error("OpenProcessToken"));
                }
            }
        ...
        if (::GetTokenInformation(current_process_token.token_handle_, TokenUser, NULL, 0,
                                  &tusize)) {
            CLOG_THROW(win32_error("GetTokenInformation should fail"));
        }
        ...
        if (!::GetTokenInformation(current_process_token.token_handle_, TokenUser,
            ...
            CLOG_THROW(win32_error("GetTokenInformation"));
        }
    ...
    HANDLE event_log_handle() {
        if (!hEventLog_) {
            ...
            if (!hEventLog_ || hEventLog_ == (HANDLE)ERROR_ACCESS_DENIED) {
                CLOG_THROW(internal::win32_error("RegisterEventSource"));
            }
        }
...
protected:
    void sink_it_(const details::log_msg &msg) override {
        ...
        if (!succeeded) {
            CLOG_THROW(win32_error("ReportEvent"));
        }
    }
