...
#include "hercules/cir/attribute.h"
...
namespace hercules::ir {
    ...
    public:
        ...
        bool is() const { return isConvertible(Target::nodeId()); }
        ...
        void setSrcInfo(hercules::SrcInfo s) {
            setAttribute(std::make_unique<SrcInfoAttribute>(std::move(s)));
        }

        /// @return the src info
        hercules::SrcInfo getSrcInfo() const {
        ...
        class AcceptorExtend;

        template<typename> friend
        class ReplaceableNodeBase;
    ...
    Desired *cast(Node *other) {
        return other != nullptr ? other->as<Desired>() : nullptr;
    }

    template<typename Desired>
    const Desired *cast(const Node *other) {
        return other != nullptr ? other->as<Desired>() : nullptr;
    }
