...
public:
  ...
  DesiredType *N(hercules::SrcInfo s, Args &&...args) {
  ...
  /// @return the new node
  template <typename DesiredType, typename... Args>
  DesiredType *N(const hercules::SrcObject *s, Args &&...args) {
  ...
  template <typename DesiredType, typename... Args> DesiredType *Nr(Args &&...args) {
    return N<DesiredType>(hercules::SrcInfo(), std::forward<Args>(args)...);
  }
...
template <> struct fmt::formatter<hercules::ir::Module> : fmt::ostream_formatter {};
