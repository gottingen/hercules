...
#include "hercules/cir/util/side_effect.h"

namespace hercules::ir::analyze::dataflow {
    namespace {

        template<typename S, typename T>
        bool contains(const S &x, T i) {
            for (auto a: x) {
                if (a == i)
                    return true;
            }
        ...
        template<>
        bool shouldTrack(const types::Type *x) { return x && !x->isAtomic(); }
...
// "i" (typically) we would not care about.
        struct ExtractVars : public util::ConstVisitor {
            ...
            void process(const Node *v) { v->accept(*this); }
