...
namespace hercules::ir {
    namespace {
        ...
        translateGenerics(hercules::ast::Cache *cache, std::vector<types::Generic> &generics) {
            std::vector<hercules::ast::types::TypePtr> ret;
            for (auto &g: generics) {
                seqassertn(g.isStatic() || g.getTypeValue(), "generic must be static or a type");
                ret.push_back(std::make_shared<hercules::ast::types::LinkType>(
                        g.isStatic()
                        ? std::make_shared<hercules::ast::types::StaticType>(cache, g.getStaticValue())
                        : (g.isStaticStr() ? std::make_shared<hercules::ast::types::StaticType>(
        ...
        std::vector<hercules::ast::types::TypePtr>
        translateArgs(hercules::ast::Cache *cache, std::vector<types::Type *> &types) {
            std::vector<hercules::ast::types::TypePtr> ret = {
                    std::make_shared<hercules::ast::types::LinkType>(
                            cache, hercules::ast::types::LinkType::Kind::Unbound, 0)};
            for (auto *t: types) {
                seqassertn(t->getAstType(), "{} must have an ast type", *t);
                if (auto f = t->getAstType()->getFunc()) {
                    ...
                    ret.push_back(std::make_shared<hercules::ast::types::PartialType>(
    ...
    Func *Module::getOrRealizeMethod(types::Type *parent, const std::string &methodName,
        ...
        try {
            ...
            for (int i = 0; i < e.messages.size(); i++) LOG_IR("getOrRealizeMethod parser error at {}: {}",
                                                               e.locations[i],
                                                               e.messages[i]);
            return nullptr;
        }
    ...
    Func *Module::getOrRealizeFunc(const std::string &funcName,
        ...
        try {
            ...
            for (int i = 0; i < e.messages.size(); i++) LOG_IR("getOrRealizeFunc parser error at {}: {}",
                                                               e.locations[i], e.messages[i]);
            return nullptr;
        }
    }
    ...
    types::Type *Module::getOrRealizeType(const std::string &typeName,
        ...
        try {
            ...
            for (int i = 0; i < e.messages.size(); i++) LOG_IR("getOrRealizeType parser error at {}: {}",
                                                               e.locations[i], e.messages[i]);
            return nullptr;
        }
    }
    ...
    types::Type *Module::getFuncType(types::Type *rType,
        ...
        args[0] = std::make_shared<hercules::ast::types::LinkType>(rType->getAstType());
