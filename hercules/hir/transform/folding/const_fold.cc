...
namespace hercules::ir::transform::folding {
    ...
    namespace {
        ...
        class IntFloatBinaryRule : public RewriteRule {
        private:
            Func f;
            std::string magic;
            types::Type *out;
...
/// Binary rule that requires two constants.
        ...
        To convert(From x) { return To(x); }

        template<typename... Args>
        auto intSingleRule(Module *m, Args &&...args) {
        ...
        auto intToIntBinary(Module *m, Func f, std::string magic) {
        ...
        template<typename Func>
        auto floatToBoolBinary(Module *m, Func f, std::string magic) {
        ...
        template<typename Func>
        auto intToIntUnary(Module *m, Func f, std::string magic) {
            return std::make_unique<SingleConstantUnaryRule<int64_t, Func>>(
                    std::move(f), std::move(magic), m->getIntType(), m->getIntType());
        }

        template<typename Func>
        auto floatToFloatUnary(Module *m, Func f, std::string magic) {
            return std::make_unique<SingleConstantUnaryRule<double, Func>>(
                    std::move(f), std::move(magic), m->getFloatType(), m->getFloatType());
        }

        template<typename Func>
        auto boolToBoolUnary(Module *m, Func f, std::string magic) {
