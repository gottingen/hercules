...
#include <hercules/hir/transform/parallel/para.h>
...
namespace hercules::ir::transform::parallel {
    ...
    namespace {
        const std::string ompModule = "std.para";
...
// we create the locks lazily to avoid them when they're not needed
        struct ReductionLocks {
            ...
            Var *createLock(Module *M) {
                ...
                seqassertn(lockType, "para.Lock type not found");
                ...
                auto *init = (*lockType)();
                seqassertn(init, "could not initialize para.Lock");
        ...
        template<typename T>
        ParaTransformData setupParaTransform(T *v, BodiedFunc *parent, bool gpu) {
            if (!v->isParallel())
                return unpar(v);
            auto *M = v->getModule();
        ...
        ForkCallData createForkCall(Module *M, OMPTypes &types, Value *rawTemplateFunc,
                                    ...
                                    transform::parallel::ParaSched *sched) {
            ForkCallData result;
        ...
        CollapseResult collapseLoop(BodiedFunc *parent, ImperativeForFlow *v, int64_t levels) {
            ...
            auto *body = M->Nr<SeriesFlow>();
            auto sched = std::make_unique<ParaSched>(*v->getSchedule());
            sched->collapse = 0;
    ...
    const std::string ParaPass::KEY = "core-parallel-para";

    void ParaPass::handle(ForFlow *v) {
        auto data = setupParaTransform(v, cast<BodiedFunc>(getParentFunc()), /*gpu=*/false);
        if (!v->isParallel())
            return;
    ...
    void ParaPass::handle(ImperativeForFlow *v) {
        ...
        auto data =
                setupParaTransform(v, parent, (v->isParallel() && v->getSchedule()->gpu));
        if (!v->isParallel())
            return;
