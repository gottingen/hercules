...
namespace hercules::ir::transform::parallel {
    ...
    ParaSched::ParaSched(int code, bool dynamic, Value *threads, Value *chunk, bool ordered,
    ...
    ParaSched::ParaSched(const std::string &schedule, Value *threads, Value *chunk,
                       bool ordered, int64_t collapse, bool gpu)
            : ParaSched(getScheduleCode(schedule, nullIfNeg(chunk) != nullptr, ordered),
                       (schedule != "static") || ordered, threads, chunk, ordered, collapse,
                       gpu) {}

    std::vector<Value *> ParaSched::getUsedValues() const {
    ...
    int ParaSched::replaceUsedValue(id_t id, Value *newValue) {
        auto count = 0;
