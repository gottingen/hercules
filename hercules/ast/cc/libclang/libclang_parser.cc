...
#include <hercules/ast/cc/libclang/parse_error.h>
#include <hercules/ast/cc/libclang/parse_functions.h>
#include <hercules/ast/cc/libclang/preprocessor.h>
...
namespace hercules::ccast {
    ...
    bool detail::libclang_compile_config_access::write_preprocessed(const libclang_compile_config &config) {
        return config.write_preprocessed_;
    }

    bool detail::libclang_compile_config_access::fast_preprocessing(const libclang_compile_config &config) {
    ...
    libclang_compile_config::libclang_compile_config() : libclang_compile_config("cc") {}
    ...
    libclang_compile_config::libclang_compile_config(std::string clang_binary)
        ...
        define_macro("__ccast__", "libclang");
        define_macro("__ccast_version_major__", std::to_string(HERCULES_VERSION_MAJOR));
        define_macro("__ccast_version_minor__", std::to_string(HERCULES_VERSION_MINOR));
    }
    ...
    libclang_compile_config::libclang_compile_config(const libclang_compilation_database &database,
            ...
            : libclang_compile_config("cc", database, file) {}
...
#    define CPPAST_DETAIL_WINDOWS 0
    ...
    void libclang_compile_config::do_set_flags(cpp_standard standard, compile_flags flags) {
        switch (standard) {
            ...
            case cpp_standard::cpp_2b:
                if (libclang_parser::libclang_minor_version() >= 61) { // Corresponds to Clang version 12
                    if (flags & compile_flag::gnu_extensions)
                        add_flag("-std=gnu++2a");
                    else
                        add_flag("-std=c++2a");
            ...
            case cpp_standard::c_17:
                if (libclang_parser::libclang_minor_version() >= 45) { // Corresponds to Clang version 6
                    if (flags & compile_flag::gnu_extensions)
                        add_flag("-std=gnu17");
                    else
                        add_flag("-std=c17");
    ...
    collie::ts::optional<libclang_compile_config> find_config_for(
        ...
        static const char *extensions[]
                = {".h", ".h", ".cpp", ".h++", ".c++", ".hxx", ".cxx", ".hh", ".cc", ".H", ".C", ".c"};
    ...
    int libclang_parser::libclang_minor_version() {
    ...
    libclang_parser::libclang_parser(collie::ts::object_ref<const diagnostic_logger> logger)
    ...
    namespace {
        ...
        collie::ts::optional<severity> get_severity(const CXDiagnostic &diag) {
