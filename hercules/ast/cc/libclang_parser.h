...
namespace hercules::ccast {
    ...
    /// Compilation config for the [hercules::ccast::libclang_parser]().
    ...
    public:
        ...
        /// It will also define `__ccast__` with the value `"libclang"` as well as `__ccast_major__`
        /// and `__ccast_minor__`.
        ...
        /// will also define `__ccast__` with the value `"libclang"` as well as `__ccast_major__` and
        /// `__ccast_minor__`.
    ...
    /// [hercules::ccast::parse_files](standardese://hercules::ccast::parse_files_basic/).
    ...
    public:
        ...
        explicit libclang_parser(collie::ts::object_ref<const diagnostic_logger> logger);
    ...
    /// Parses multiple files using a [hercules::ccast::libclang_parser]() and a compilation database.
    ///
    /// \effects Invokes [hercules::ccast::parse_files](standardese://parse_files_basic/) passing it the parser
    /// and file names, and a `get_config` function using [hercules::ccast::find_config_for]().
    ///
    /// \throws [hercules::ccast::libclang_error]() if no configuration for a given file could be found in the
    /// database.
    ///
    /// \requires `FileParser` must use the libclang parser.
    /// i.e. `FileParser::parser` must be an alias of [hercules::ccast::libclang_parser]().
    ...
    /// Parses the files specified in a compilation database using a [hercules::ccast::libclang_parser]().
    ///
    ...
    /// [hercules::ccast::parse_files](standardese://parse_files_basic/). It must also use the libclang parser,
    /// i.e. `FileParser::parser` must be an alias of [hercules::ccast::libclang_parser]().
