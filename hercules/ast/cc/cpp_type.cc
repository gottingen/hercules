...
const cpp_type& hercules::ccast::remove_cv(const cpp_type& type) noexcept
{
...
const cpp_type& hercules::ccast::remove_const(const cpp_type& type) noexcept
{
    if (type.kind() == cpp_type_kind::cv_qualified_t)
    {
        auto& cv = static_cast<const cpp_cv_qualified_type&>(type);
...
const cpp_type& hercules::ccast::remove_volatile(const cpp_type& type) noexcept
{
    if (type.kind() == cpp_type_kind::cv_qualified_t)
    {
        auto& cv = static_cast<const cpp_cv_qualified_type&>(type);
...
std::string hercules::ccast::to_string(const cpp_type& type)
{
    ...
    to_string_generator::output output(collie::ts::ref(generator), collie::ts::ref(*dummy_entity),
                                       cpp_public);
