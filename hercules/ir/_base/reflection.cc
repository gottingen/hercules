...
class AttrGetter : public AttrVisitor {
 public:
  ...
  void Visit(const char* key, uint64_t* value) final {
    HSCHECK_LE(value[0], static_cast<uint64_t>(std::numeric_limits<int64_t>::max()))
...
ObjectPtr<Object> ReflectionVTable::CreateInitObject(const String& type_key,
  ...
  if (tindex >= fcreate_.size() || fcreate_[tindex] == nullptr) {
    HSLOG(FATAL) << "TypeError: " << type_key
...
class NodeAttrSetter : public AttrVisitor {
 ...
 private:
  RTValue GetAttr(const char* key) {
    ...
    if (it == attrs.end()) {
      HSLOG(FATAL) << type_key << ": require field " << key;
    }
    RTValue v = it->second;
    attrs.erase(it);
    return v;
...
HERCULES_REGISTER_GLOBAL("runtime.NodeGetAttr").set_body(NodeGetAttr);

HERCULES_REGISTER_GLOBAL("runtime.NodeListAttrNames").set_body(NodeListAttrNames);

HERCULES_REGISTER_GLOBAL("runtime.MakeNode").set_body(MakeNode);
