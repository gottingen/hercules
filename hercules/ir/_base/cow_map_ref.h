...
#include "hercules/runtime/container/inplace_array_base.h"
...
#include <hercules/ir/_base/object_hash.h>
...
class DenseMapNode : public MapNode {
 ...
 public:
  ...
  /*! \return end iterator */
  iterator end() const {
    return slots_ == 0 ? iterator(0, this) : iterator(slots_ + 1, this);
  }
 ...
 private:
   ...
   */
  bool IsFull() const {
    return size_ + 1 > (slots_ + 1) * kMaxLoadFactor;
  }
  /*!
   * \brief Increment the pointer
   * \param index The pointer to be incremented
   ...
   */
  KVType* DeRefItr(uint64_t index) const {
    return &ListNode(index, this).Data();
  }
  ...
  struct ListNode {
    ...
    /*! \brief Move the entry to the next entry on the linked list */
    bool MoveToNext(const DenseMapNode* self) {
      return MoveToNext(self, Meta());
    }
...
class Map : public ObjectRef {
 public:
  ...
  bool contains(const K& key) const {
    MapNode* n = GetMapNode();
    return n == nullptr ? false : GetMapNode()->count(key) > 0;
  }
  ...
  /*! \return begin iterator */
  iterator begin() const {
    return iterator(GetMapNode()->begin());
  }
  /*! \return end iterator */
  iterator end() const {
    return iterator(GetMapNode()->end());
  }
