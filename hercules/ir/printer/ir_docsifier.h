...
#include <hercules/ir/printer/ir_docsifier_functor.h>
 ...
 * when printing IR graph into text-format code.
 */
class FrameNode : public Object {
 ...
 public:
   ...
   */
  void AddDispatchToken(const IRDocsifier& d, const StringRef& token);
  /*!
...
class IRDocsifierNode : public Object {
 public:
   ...
   */
  Array<StringRef> dispatch_tokens;
  /*! \brief Mapping from a var to its info */
  std::unordered_map<ObjectRef, VariableInfo, runtime::ObjectPtrHash, runtime::ObjectPtrEqual>
      obj2info;
  /*! \brief Metadata printing */
  std::unordered_map<StringRef, Array<ObjectRef>> metadata;
  /*! \brief The variable names used already */
  std::unordered_set<StringRef> defined_names;
 ...
 public:
   ...
   * in the table.
   */
  IdDoc Define(const ObjectRef& obj, const Frame& frame, const StringRef& name_hint);
   ...
   * different `source_path`. Currently there isn't a good way to deep copy an Object
  ...
  /*! \brief Add an object to the metadata section*/
   ...
   */
  void SetCommonPrefix(const ObjectRef& root, runtime::TypedNativeFunction<bool(ObjectRef)> is_var);
  /*!
...
inline void FrameNode::EnterWithScope() {
  if (d != nullptr) {
    d->frames.push_back(runtime::GetRef<Frame>(this));
  }
}
...
template <class TDoc>
inline static void AddDocDecoration(const Doc& d,
                                    const ObjectRef& obj,
...
inline TDoc IRDocsifierNode::AsDoc(const ObjectRef& obj, const ObjectPath& path) const {
  if (obj.defined()) {
    Doc d = IRDocsifier::vtable()(
        dispatch_tokens.back(), obj, path, runtime::GetRef<IRDocsifier>(this));
    d->source_paths.push_back(path);
    AddDocDecoration<TDoc>(d, obj, path, cfg);
    return runtime::Downcast<TDoc>(d);
  }
  return runtime::Downcast<TDoc>(LiteralDoc::None(path));
}

inline void FrameNode::AddDispatchToken(const IRDocsifier& d, const StringRef& token) {
