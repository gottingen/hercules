...
size_t MoveBack(size_t pos, size_t distance) {
  return distance > pos ? 0 : pos - distance;
}
...
std::string DecorateText(const std::string& text,
  ...
  std::string ret;
  if (underlines.empty()) {
    PrintChunk({0, num_lines},
               {underlines.begin(), underlines.begin()},
               text,
               line_starts,
               options,
               line_number_width,
  ...
  PrintChunk(cur_chunk,
             {first_underline_in_cur_chunk, underlines.end()},
             text,
             line_starts,
             options,
             line_number_width,
...
StringRef DocPrinter::GetString() const {
  ...
  return StringRef(DecorateText(
      text, line_starts_, options_, MergeAndExemptSpans(underlines_, underlines_exempted_)));
}
...
void DocPrinter::PrintDoc(const Doc& doc) {
  ...
  } else if (const auto* doc_node = doc.as<ModuleDocNode>()) {
    PrintTypedDoc(GetRef<ModuleDoc>(doc_node));
