...
namespace hercules {
    ...
    public:
        ...
        void handle(hercules::ir::AssignInstr *v) {
            auto *M = v->getModule();
            ...
            auto debug_str = collie::format(FMT_STRING("{} called [{}:{}] at [{}:{}]"), foo->getUnmangledName(), f, l, cf, cl);
    ...
    llvm::Error load_builtin(hercules::ir::transform::PassManager *pm, bool debug) {
        if(debug) {
            pm->registerPass(std::make_unique<BuiltinTrace>());
        }
