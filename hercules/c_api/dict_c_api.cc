...
HERCULES_REGISTER_GLOBAL("runtime.Dict").set_body([](PyArgs args) -> RTValue {
...
HERCULES_REGISTER_GLOBAL("runtime.FTDict").set_body([](PyArgs args) -> RTValue {
  HSCHECK_EQ(args.size() % 2, 0);
...
HERCULES_REGISTER_GLOBAL("runtime.DictEqual").set_body([](PyArgs args) -> RTValue {
  HSCHECK(2 == args.size()) << "dict.__eq__ expect " << 2 << " arguments but get " << args.size();
  ...
  switch (self.type_code()) {
    ...
    default: {
      HSTHROW << "expect 'dict' but get '" << self.type_name();
    } break;
  }
  return false;
});

HERCULES_REGISTER_GLOBAL("runtime.Dict_Iter").set_body([](PyArgs args) -> RTValue {
  HSCHECK(1 == args.size()) << "dict.__iter__ expect " << 1 << " arguments but get " << args.size();
  const auto& self = args[0];
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
...
HERCULES_REGISTER_GLOBAL("runtime.Dict_KeyIter").set_body([](PyArgs args) -> RTValue {
  HSCHECK(1 == args.size()) << "dict.keys expect " << 1 << " arguments but get " << args.size();
  const auto& self = args[0];
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
      return self.AsObjectRefNoCheck<Dict>().key_iter();
...
HERCULES_REGISTER_GLOBAL("runtime.Dict_ValueIter").set_body([](PyArgs args) -> RTValue {
  HSCHECK(1 == args.size()) << "dict.values expect " << 1 << " arguments but get " << args.size();
  const auto& self = args[0];
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
...
HERCULES_REGISTER_GLOBAL("runtime.Dict_ItemIter").set_body([](PyArgs args) -> RTValue {
  HSCHECK(1 == args.size()) << "dict.items expect " << 1 << " arguments but get " << args.size();
  const auto& self = args[0];
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
...
HERCULES_REGISTER_GLOBAL("runtime.DictSize").set_body([](PyArgs args) -> RTValue {
  HSCHECK(1 == args.size()) << "dict.__len__ expect " << 1 << " arguments but get " << args.size();
  const auto& self = args[0];
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
...
HERCULES_REGISTER_GLOBAL("runtime.DictContains").set_body([](PyArgs args) -> RTValue {
  HSCHECK(2 == args.size()) << "dict.__contains__ expect " << 2 << " arguments but get "
                            << args.size();
  const auto& self = args[0];
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
...
HERCULES_REGISTER_GLOBAL("runtime.DictGetItem").set_body([](PyArgs args) -> RTValue {
  HSCHECK(2 == args.size()) << "dict.__getitem__ expect " << 2 << " arguments but get "
                            << args.size();
  const auto& self = args[0];
  ...
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
      ...
      HSCHECK(obj.contains(key.As<RTValue>())) << "cannot find the corresponding key in the Dict";
      return obj[key.As<RTValue>()];
    } break;
    case TypeIndex::kRuntimeFTDict: {
      auto obj = self.AsObjectRefNoCheck<FTObjectBase>();
      HSCHECK(obj.generic_call_attr("__contains__", {key.As<RTView>()}).As<bool>())
...
HERCULES_REGISTER_GLOBAL("runtime.DictClear").set_body([](PyArgs args) -> RTValue {
  HSCHECK(1 == args.size()) << "dict.clear expect " << 1 << " arguments but get " << args.size();
  const auto& self = args[0];
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
...
HERCULES_REGISTER_GLOBAL("runtime.DictSetItem").set_body([](PyArgs args) -> RTValue {
  const auto& key = args[1];
  const auto& val = args[2];

  HSCHECK(3 == args.size()) << "dict.__contains__ expect " << 3 << " arguments but get "
                            << args.size();
  const auto& self = args[0];
  switch (self.type_code()) {
    case TypeIndex::kRuntimeDict: {
...
HERCULES_REGISTER_GLOBAL("runtime.DictReserve").set_body([](PyArgs args) -> RTValue {
  Dict obj = args[0].As<Dict>();
  obj.reserve(args[1].As<int64_t>());
  return None;
});

HERCULES_REGISTER_GLOBAL("runtime.DictBucketCount").set_body([](PyArgs args) -> RTValue {
  Dict obj = args[0].As<Dict>();
  return static_cast<int64_t>(obj.bucket_count());
});

HERCULES_REGISTER_GLOBAL("runtime.DictGetDefault").set_body([](PyArgs args) -> RTValue {
  HSCHECK(2 == args.size() || 3 == args.size())
  ...
  switch (self.type_code()) {
    ...
    default: {
      HSTHROW << "expect 'dict' bug get '" << self.type_name();
    } break;
  }
  return None;
});

HERCULES_REGISTER_GLOBAL("runtime.DictPop").set_body([](PyArgs args) -> RTValue {
  HSCHECK(2 == args.size() || 3 == args.size())
