...
namespace hercules::ast {
    ...
    /// @param defines   User-defined static values (typically passed as `hercules run -DX=Y`).
...
#define N std::make_shared
        ...
        if (!in(cache->imports, STDLIB_IMPORT)) {
            ...
            const std::string initFile = "__init__.hs";
        ...
        for (auto &d: defines) {
            // Load compile-time defines (e.g., hercules run -DFOO=1 ...)
