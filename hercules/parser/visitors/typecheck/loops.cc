...
namespace hercules::ast {
    ...
    std::pair<bool, std::vector<std::shared_ptr<hercules::SrcObject>>>
    TypecheckVisitor::transformStaticLoopCall(
            const std::vector<std::string> &vars, ExprPtr iter,
            std::function<std::shared_ptr<hercules::SrcObject>(StmtPtr)> wrap) {
        if (!iter->getCall())
            return {false, {}};
        ...
        std::vector<std::shared_ptr<hercules::SrcObject>> block;
