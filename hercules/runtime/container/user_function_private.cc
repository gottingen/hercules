 ...
 * under the License.
 */
#include <hercules/runtime/container/user_function_private.h>

namespace hercules {
namespace runtime {

RTView UserFunction::__getattr__(string_view var_name) const {
  HSCHECK(false) << "[UserFunction] get_attr is disabled";
  return None;
}

void UserFunction::__setattr__(string_view var_name, const Any& val) {
  HSCHECK(false) << "[UserFunction] set_attr is disabled";
}
