...
#include <hercules/runtime/_is_comparable.h>
...
template <typename T>
class HERCULES_DLL FTSet;

template <typename T>
class HERCULES_DLL FTSetNode : public FTObjectBaseNode {
 public:
  // data holder
 ...
 public:
  HERCULES_INLINE_VISIBILITY ~FTSetNode() = default;
  // constructors
  HERCULES_INLINE_VISIBILITY FTSetNode()
      : FTObjectBaseNode(&function_table_, &std_type_index_, type_tag_) {
  }
  HERCULES_INLINE_VISIBILITY FTSetNode(FTSetNode&& other) = default;
  HERCULES_INLINE_VISIBILITY FTSetNode(const FTSetNode& other) = default;
  HERCULES_INLINE_VISIBILITY FTSetNode& operator=(FTSetNode&& other) = default;
  HERCULES_INLINE_VISIBILITY FTSetNode& operator=(const FTSetNode& other) = default;

  template <typename IterType>
  HERCULES_INLINE_VISIBILITY FTSetNode(IterType first, IterType last)
      : FTObjectBaseNode(&function_table_, &std_type_index_, type_tag_), data_(first, last) {
  }
  HERCULES_INLINE_VISIBILITY FTSetNode(std::initializer_list<value_type> init)
      : FTObjectBaseNode(&function_table_, &std_type_index_, type_tag_), data_(init) {
  }
  HERCULES_INLINE_VISIBILITY FTSetNode(const std::vector<value_type>& init)
      : FTObjectBaseNode(&function_table_, &std_type_index_, type_tag_),
 ...
 public:
  ...
  HERCULES_DECLARE_FINAL_OBJECT_INFO(FTSetNode, FTObjectBaseNode);
...
template <typename T>
class HERCULES_DLL FTSet : public FTObjectBase {
 public:
  // data holder
  using value_type = typename std::remove_cv<typename std::remove_reference<T>::type>::type;
 ...
 public:
  ...
  HERCULES_INLINE_VISIBILITY ~FTSet() noexcept = default;

  // constructors
  HERCULES_INLINE_VISIBILITY FTSet() : FTObjectBase(make_object<FTSetNode<value_type>>()) {
  }
  HERCULES_INLINE_VISIBILITY FTSet(FTSet&& other) noexcept = default;
  HERCULES_INLINE_VISIBILITY FTSet(const FTSet& other) noexcept = default;
  HERCULES_INLINE_VISIBILITY FTSet& operator=(FTSet&& other) noexcept = default;
  HERCULES_INLINE_VISIBILITY FTSet& operator=(const FTSet& other) noexcept = default;

  /*!
   * \brief constructor from pointer
   * \param n the container pointer
   */
  HERCULES_INLINE_VISIBILITY explicit FTSet(ObjectPtr<Object> n) noexcept
   ...
   */
  template <typename IterType>
  HERCULES_INLINE_VISIBILITY FTSet(IterType first, IterType last)
   ...
   */
  HERCULES_INLINE_VISIBILITY FTSet(std::initializer_list<value_type> init)
   ...
   */
  HERCULES_INLINE_VISIBILITY FTSet(const std::vector<value_type>& init)
      : FTObjectBase(make_object<FTSetNode<value_type>>(init)) {
  }

  template <class U>
  HERCULES_INLINE_VISIBILITY bool operator==(const FTSet<U>& other) const {
 ...
 public:
  ...
  HERCULES_INLINE_VISIBILITY std::pair<iterator, bool> emplace(Args&&... args) const {
    return MutableImpl().emplace(std::forward<Args>(args)...);
  }

  HERCULES_INLINE_VISIBILITY void add(value_type item) const {
    MutableImpl().emplace(std::move(item));
  }

  template <class U>
  HERCULES_INLINE_VISIBILITY void add(U&& item) const {
 ...
 public:
  ...
  HERCULES_INLINE_VISIBILITY bool __eq__(const FTSet<KEY_U>& o, std::true_type) const {
    return MutableImpl() == o.MutableImpl();
  }

  template <class KEY_U>
  HERCULES_INLINE_VISIBILITY bool __eq__(const FTSet<KEY_U>& o, std::false_type) const {
    return Iterator::all_items_equal(__iter__(), o.__iter__());
  }

  template <class KEY_U>
  HERCULES_INLINE_VISIBILITY bool __eq__(const FTSet<KEY_U>& o) const {
 ...
 public:
  HERCULES_INLINE_VISIBILITY void difference_update(PyArgs args) const {
  ...
  HERCULES_INLINE_VISIBILITY FTSet difference(PyArgs args) const {
    FTSet ret(*this);
    ret.difference_update(args);
    return ret;
  }

  HERCULES_INLINE_VISIBILITY void update(PyArgs args) const {
    for (const auto& val : args) {
      if (val.type_code() == TypeIndex::kRuntimeIterator) {
  ...
  HERCULES_INLINE_VISIBILITY FTSet set_union(PyArgs args) const {
    FTSet ret(*this);
    ret.update(args);
    return ret;
  }
  ...
  template <class KEY_U>
  HERCULES_INLINE_VISIBILITY void discard(const KEY_U& key, std::true_type) const {
  ...
  template <class KEY_U>
  HERCULES_INLINE_VISIBILITY void discard(const KEY_U& key, std::false_type) const {
    SmartHash{}(key);
    return;
  }

  template <class KEY_U>
  HERCULES_INLINE_VISIBILITY void discard(const KEY_U& key) const {
    using KEY_U_TYPE = typename std::remove_cv<typename std::remove_reference<KEY_U>::type>::type;
 ...
 private:
  ...
  HERCULES_INLINE_VISIBILITY FTSetNode<value_type>* MutableNode() const {
    return static_cast<FTSetNode<value_type>*>(data_.get());
  }

  HERCULES_INLINE_VISIBILITY void difference_update_iter(const Iterator& iter) const {
  ...
  HERCULES_INLINE_VISIBILITY void update_iter(const Iterator& iter) const {
...
// python methods
#define HERCULES_CHECK_FT_SET_ARGS(FuncName, NumArgs)                               \
  HSCHECK(NumArgs == args.size()) << "[" << DemangleType(typeid(FTSetNode<T>).name()) \
...
const FTObjectBaseNode::FunctionTable FTSetNode<T>::function_table_ = {
    ...
    {"discard",
     [](RTView self, PyArgs args) -> RTValue {
       HERCULES_CHECK_FT_SET_ARGS(discard, 1);
...
#undef HERCULES_CHECK_FT_SET_ARGS
