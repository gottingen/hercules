...
NativeFunction WrapPackedFunc(HerculesBackendPackedCFunc faddr,
  ...
  if (capture_session_handle) {
    return NativeFunction([faddr, sptr_to_self](PyArgs args) -> RTValue {
      ...
      HSCHECK_EQ(ret, 0) << HerculesAPIGetLastError();
...
class LibraryModuleNode final : public ModuleNode {
 public:
  explicit LibraryModuleNode(ObjectPtr<Library> lib) : lib_(std::move(lib)) {
    auto* func_reg = reinterpret_cast<HerculesFuncRegistry*>(
        lib_->GetSymbol(runtime::symbol::library_func_registry));
    HSCHECK(func_reg != nullptr) << "Symbol " << runtime::symbol::library_func_registry
                                 << " is not presented";
    ...
    HSCHECK(closures_names_ptr != nullptr)
  ...
  NativeFunction GetFunction(const String& name, const ObjectPtr<Object>& sptr_to_self) final {
    ...
    HerculesBackendPackedCFunc faddr;
    ...
    if (name == runtime::symbol::library_func_registry) {
      ...
      faddr = reinterpret_cast<HerculesBackendPackedCFunc>(lib_->GetSymbol(name.c_str()));
    }
    if (faddr == nullptr)
 ...
 private:
  ...
  ska::flat_hash_map<String, HerculesBackendPackedCFunc> func_regs_;
