 ...
 * https://github.com/python/cpython/blob/3.8/Python/dtoa.c
 *
...
/* The code below assumes that the endianness of integers matches the
   endianness of the two 32-bit words of a double.  Check this. */
    ...
    (defined(DOUBLE_IS_LITTLE_ENDIAN_IEEE754) || defined(DOUBLE_IS_ARM_MIXED_ENDIAN_IEEE754))
...
static Bigint* Balloc(int k) {
  ...
  if (k <= Kmax && (rv = freelist[k]))
    ...
    if (k <= Kmax && pmem_next - private_mem + len <= (intptr_t)PRIVATE_mem) {
      rv = (Bigint*)pmem_next;
      pmem_next += len;
    } else {
...
char* _Py_dg_dtoa(double dd, int mode, int ndigits, int* decpt, int* sign, char** rve) {
  ...
  */
  ...
  int bbits, b2, b5, be, dig, i, ieps, ilim, ilim0, ilim1, j, j1, k, k0, k_check, leftright, m2, m5,
