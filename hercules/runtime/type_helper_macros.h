...
struct TypeAsConverterNoCheck {
 private:
  ...
  template <typename U>
  static HERCULES_ALWAYS_INLINE TO_TYPE run(U&& v, std::integral_constant<bool, true>) {
    return v.template MoveToObjectRefNoCheck<TO_TYPE>();
  }

  template <typename U>
  static HERCULES_ALWAYS_INLINE TO_TYPE run(U&& v, std::integral_constant<bool, false>) {
    return v.template AsNoCheck<TO_TYPE>();
  }

 public:
  template <typename U>
  static HERCULES_ALWAYS_INLINE TO_TYPE run(U&& v) {
...
struct TypeAsConverterNoCheck<String> {
  template <typename U>
  static HERCULES_ALWAYS_INLINE String run(U&& v) {
    return v.template AsNoCheck<String>();
  }
  static HERCULES_ALWAYS_INLINE String run(RTValue&& v) {
...
struct TypeAsConverterNoCheck<Unicode> {
  template <typename U>
  static HERCULES_ALWAYS_INLINE Unicode run(U&& v) {
    return v.template AsNoCheck<Unicode>();
  }
  static HERCULES_ALWAYS_INLINE Unicode run(RTValue&& v) {
...
struct TypeAsHelper {
  ...
  static HERCULES_ALWAYS_INLINE TO_TYPE
...
#define HERCULES_TYPE_AS(o, TYPE)                         \
  ::hercules::runtime::internal::TypeAsHelper<TYPE>::run( \
      (o), __FILE__, __LINE__, nullptr, "expect '" #o "' is '" #TYPE "' type")

#define HERCULES_TYPE_AS_V2(o, TYPE, TYPE_REPR)           \
  ::hercules::runtime::internal::TypeAsHelper<TYPE>::run( \
      (o), __FILE__, __LINE__, nullptr, "expect '" #o "' is '" TYPE_REPR "' type")

// this is for python only
#define HERCULES_TYPE_AS_WITH_PY_INFO(o, TYPE, PY_INFO)   \
  ::hercules::runtime::internal::TypeAsHelper<TYPE>::run( \
