...
SEQ_FUNC void seq_terminate(void *exc) {
  ...
  if ((seq_flags & SEQ_FLAG_DEBUG) && (seq_flags & SEQ_FLAG_STANDALONE)) {
    ...
    if (bt->count > 0) {
      ...
      for (unsigned i = 0; i < bt->count; i++) {
        ...
        buf << "  "
            << hercules::runtime::makeBacktraceFrameString(
  ...
  if (seq_flags & SEQ_FLAG_STANDALONE) {
    ...
    hercules::runtime::JITError e(output, msg, type, file, (int)hdr->line, (int)hdr->col,
                               backtrace);
    if (jitErrorCallback)
      jitErrorCallback(e);
    else
...
std::string hercules::runtime::makeBacktraceFrameString(uintptr_t pc,
...
void hercules::runtime::setJITErrorCallback(
    std::function<void(const hercules::runtime::JITError &)> callback) {
