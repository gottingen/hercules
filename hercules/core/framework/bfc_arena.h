...
#include "arena.h"
#include "hercules/core//common/common.h"
#include "hercules/core//framework/brt_mutex.h"
...
class BFCArena : public IArenaAllocator {
 ...
 private:
  ...
  struct Bin {
    ...
    Bin(BFCArena* allocator, size_t bs) : bin_size(bs), free_chunks(ChunkComparator(allocator)) {
    }
  };
  ...
  class AllocationRegion {
   public:
    ...
    AllocationRegion(AllocationRegion&& other) noexcept {
      Swap(other);
    }
   ...
   private:
    ...
    // (May be used by the client to track which allocation region was allocated first, second, and
    // so on)
    int64_t id_ = -1;
