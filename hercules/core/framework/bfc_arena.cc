...
BFCArena::BFCArena(std::unique_ptr<IAllocator> resource_allocator,
                   size_t total_memory,
                   bool initial_max_extend,
                   ArenaExtendStrategy arena_extend_strategy,
                   int initial_chunk_size_bytes,
                   int max_dead_bytes_per_chunk,
                   int initial_growth_chunk_size_bytes)
    : IArenaAllocator(BrtMemoryInfo(resource_allocator->Info().name,
                                    BrtAllocatorType::BrtArenaAllocator,
  ...
  // static_cast<std::underlying_type_t<ArenaExtendStrategy>>(arena_extend_strategy); doesn't work
  // on this compiler

  curr_region_allocation_bytes_ =
      RoundedBytes(std::min(total_memory, static_cast<size_t>(initial_chunk_size_bytes_)));
  ...
  // The user selected initial allocation chunk is only valid for the arena extend strategy
  // kNextPowerOfTwo and the user has likely chosen this initial value so that any ad-hoc arena
  // extensions/shrinkages could potentially be avoided. So we do not consider the initial
  // allocation for shrinkage whatever its usage status. On the other hand, if the arena extension
  // strategy is kSameAsRequested, any initial chunk set by the user or otherwise, is moot and the
  // arena will only extend based on the request size. In these cases, we consider any allocation
  // for shrinkage if it is left unused (even if it is the first allocation).
...
Status BFCArena::Extend(size_t rounded_bytes) {
  ...
  auto safe_alloc = [this](size_t alloc_bytes) {
    ...
    BRT_CATCH(const std::bad_alloc&) {
      // attempted allocation can throw std::bad_alloc. we want to treat this the same as if it
      // returned nullptr so swallow the exception
    }
    BRT_CATCH(const BrtException& brt_exception) {
      ...
      BRT_HANDLE_EXCEPTION([&brt_exception]() {
        if (std::string(brt_exception.what()).find("cudaMalloc") == std::string::npos &&
            std::string(brt_exception.what()).find("hipMalloc") == std::string::npos) {
          BRT_RETHROW;
        }
      });
    }
  ...
  auto get_extend_bytes = [this, available_bytes](const size_t bytes) -> size_t {
    ...
    if (arena_extend_strategy_ == ArenaExtendStrategy::kNextPowerOfTwo) {
      ...
      BRT_THROW("Incorrect arena extend strategy.", static_cast<int32_t>(arena_extend_strategy_));
    }
  ...
  while (mem_addr == nullptr) {
    ...
    // give up if we can't satisfy the requested size, or we're attempting an allocation of less
    // than 8K.
    //
    // the latter protects against an infinite loop that occurs when bytes is less than 2560. at
    // that point the 10% reduction to 2304 bytes is undone by rounding to a 256 boundary in
    // RoundedBytes, leading to an infinite loop. the 8K value is just to give up a little earlier
    // vs. getting all the way down to 2560 bytes. If we can't allocate 8K, we're pretty much dead.
  ...
  if (mem_addr == nullptr) {
    return BRT_MAKE_STATUS(
        BRT, FAIL, "Failed to allocate memory for requested buffer of size ", rounded_bytes);
  }
...
void* BFCArena::Reserve(size_t size) {
  ...
  stats_.max_bytes_in_use =
      std::max<int64_t>(static_cast<int64_t>(stats_.max_bytes_in_use), stats_.bytes_in_use);
    ...
    for (auto citer = b->free_chunks.begin(); citer != b->free_chunks.end(); ++citer) {
      ...
      if (chunk->size >= rounded_bytes) {
        ...
        if (chunk->size >= rounded_bytes * 2 ||
            static_cast<int64_t>(chunk->size) - static_cast<int64_t>(rounded_bytes) >=
...
Status BFCArena::Shrink() {
  ...
  // In case the extend strategy is kSameAsRequested, the arena growth is exactly the size of the
  // memory request itself
...
void BFCArena::RemoveFreeChunkIterFromBin(BFCArena::Bin::FreeChunkSet* free_chunks,
...
void BFCArena::RemoveFreeChunkFromBin(BFCArena::ChunkHandle h) {
  Chunk* c = ChunkFromHandle(h);
  BRT_ENFORCE(!c->in_use() && (c->bin_num != kInvalidBinNum));
  BRT_ENFORCE(BinFromIndex(c->bin_num)->free_chunks.erase(h) > 0, "Could not find chunk in bin");
  c->bin_num = kInvalidBinNum;
}
...
std::array<BFCArena::BinDebugInfo, BFCArena::kNumBins> BFCArena::get_bin_debug_info() {
